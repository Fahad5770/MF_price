...
@WebServlet(description = "Executes Sampling Workflow Request", urlPatterns = { "/mobile/MobileMDESamplingRequestExecute" })
public class MobileMDESamplingRequestExecute extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public MobileMDESamplingRequestExecute() {
        super();
    }

	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
	System.out.println("Hello I am in MDE servlet!!!");
		...
		*/
		...
		//System.out.println("Brand"+Arrays.toString(PerCaseBrand));
				// Utilities.parseInt(Utilities.filterString(mr.getParameterValues("PerCaseBrand"), 0, MaxLength.CURRENCY));
		...
		int PerCaseVolume[] = Utilities.parseInt(Utilities.filterString(mr.getParameterValues("PerCaseVolume"), 0, MaxLength.CURRENCY));
		...
		*/
		...
		try {
			...
			*/
			...
			String query = "INSERT INTO sampling (request_id, outlet_id, outlet_name, business_type, address, region, market, vehicle, advance_company_share, advance_agency_share, advance_valid_from, advance_valid_to, fixed_company_share, fixed_agency_share, fixed_deduction_term, fixed_valid_from, fixed_valid_to, active, activated_on, deactivated_on, fixed_company_share_offpeak, fixed_agency_share_offpeak, fixed_deduction_term_offpeak)"+
					"VALUES"+
			...
			if (updated > 0){
				...
				//	System.out.println("P[ I]"+PerCasePackage[i] );
					if (PerCasePackage[i] != 0){
						...
						// System.out.println("REPLACE S "+SamplingID+", "+PerCasePackage[i]+", "+PerCaseAgencyShare[i]+", "+PerDiscountCaseCompanyShare[i]+", "+PerCaseDeductionTerm[i]+","+Utilities.getSQLDate(PerCaseValidFrom[i])+", "+Utilities.getSQLDate(PerCaseValidTo[i])+", "+PerCaseHandToHand[i];
