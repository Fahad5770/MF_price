 ...
 */
@WebServlet(description = "Mobile Outlet order info json", urlPatterns = { "/mobile/MobileOutletOrderStatusInfoJson" })
public class MobileOutletOrderStatusInfoJson extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MobileOutletOrderStatusInfoJson() {
        super();
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			System.out.println("I am here, servelet called - Mobile Outlet Order Info!!! ");
		...
		//if (!mr.isExpired()){
			...
			try{
				...
				ResultSet rs2 = s.executeQuery("SELECT * FROM pep.mobile_order where  mobile_timestamp between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate));
				while(rs2.next()){
					OrderPlaced=1;
				...
				ResultSet rs3 = s.executeQuery("SELECT * FROM pep.mobile_order_zero where mobile_timestamp between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" order by id desc");
				while(rs3.next()){
				...
				ResultSet rs4 = s.executeQuery("SELECT * FROM mobile_outlet_closed where mobile_timestamp between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" order by id desc");
				while(rs4.next()){
