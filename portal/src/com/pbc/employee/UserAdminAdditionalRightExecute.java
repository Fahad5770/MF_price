 ...
 */
@WebServlet(description = "User Admin Additional Rights", urlPatterns = { "/employee/UserAdminAdditionalRightExecute" })
public class UserAdminAdditionalRightExecute extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UserAdminAdditionalRightExecute() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			query = "delete from `user_access_admin_warehouses` where user_id = "+UserID+" and feature_id="+FeatureID;
			s.executeUpdate(query);
				...
				if(request.getParameterValues("Warehousehiddenfield") != null)
				{
					...
					for(int ii=0;ii<SelectedWareHouses.length;ii++)
					{
						...
						query2 = "insert into " +
				                "`user_access_admin_warehouses`(`user_id`,`feature_id`,`warehouse_id`)" +
				...
				query = "delete from `user_access_admin_regions` where user_id = "+UserID+" and feature_id="+FeatureID;
				s.executeUpdate(query);
				...
				if(request.getParameterValues("RegionIDhiddenfield") != null)
				{
					...
					for(int ii=0;ii<SelectedRegions.length;ii++)
					{
						...
						query2 = "insert into " +
				                "user_access_admin_regions(user_id,feature_id,region_id)" +
				...
				query = "delete from `user_access_admin_distributors` where user_id = "+UserID+" and feature_id="+FeatureID;
				s.executeUpdate(query);
				...
				if(request.getParameterValues("DistributorIDhiddenfield") != null)
				{
					...
					for(int ii=0;ii<SelectedDistributor.length;ii++)
					{
						...
						query2 = "insert into " +
				                "`user_access_admin_distributors`(`user_id`,`feature_id`,`distributor_id`)" +
				...
				query = "delete from `user_access_admin_distributor_groups` where user_id = "+UserID+" and feature_id="+FeatureID;
				s.executeUpdate(query);
				...
				if(request.getParameterValues("GroupDistributorhiddenfield") != null)
				{
					...
					for(int ii=0;ii<SelectedDistributorGroups.length;ii++)
					{
						...
						query2 = "insert into " +
				                "user_access_admin_distributor_groups(user_id,feature_id,group_id)" +
				                "values("+UserID+","+FeatureID+","+FeatureIDndRegionID[0]+")";						
						s.executeUpdate(query2);
					}
				}
