...
@WebServlet(description = "Uploads a file", urlPatterns = { "/employee/UploadFilePSRTargetsT2" })
public class UploadFilePSRTargetsT2 extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public UploadFilePSRTargetsT2() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("PSR T2 Targets Against PJP");
			...
			try {
				...
				while (iterator.hasNext()) {
					...
					if (!item.isFormField()) {
						...
						ResultSet rs3 = s.executeQuery("select month,year from pep.employee_targets_t2 where month=" + Month
								+ " and year=" + Year + "");
						if (rs3.first()) {
...
//									+ ")");
							s2.executeUpdate("INSERT INTO " + ds.logDatabaseName()
									+ ".`employee_targets_logs_t2`(`target_id`,`pjp_id`,`employee_id`,`month`,`year`,`created_by`,`created_on`,`updated_on`,`updated_by`) (select id,pjp_id,employee_id,month,year,created_by,created_on,now(),"
									+ created_by + " from pep.employee_targets_t2 where month=" + Month + " and year=" + Year
									+ ")");
...
//									+ Month + " and year=" + Year + "))");
							s2.executeUpdate("INSERT INTO " + ds.logDatabaseName()
									+ ".`employee_targets_packages_logs_t2`(`id`,`package_id`,`quantity`,`updated_on`,`updated_by`) (SELECT id,package_id,quantity ,now(),"
									+ created_by
									+ " FROM pep.employee_targets_packages_t2 where id in(SELECT  id FROM pep.employee_targets_t2 where month="
									+ Month + " and year=" + Year + "))");
...
//									+ Month + " and year=" + Year + "))");
							s2.executeUpdate("INSERT INTO " + ds.logDatabaseName()
									+ ".`employee_targets_packages_brands_logs_t2`(`id`,`package_id`,`brand_id`,`quantity`,`updated_on`,`updated_by`) (SELECT id,package_id,brand_id,quantity,now(),"
									+ created_by
									+ " FROM pep.employee_targets_packages_brands_t2 where id in(SELECT  id FROM pep.employee_targets_t2 where month="
									+ Month + " and year=" + Year + "))");
							s.executeUpdate(
									"delete from pep.employee_targets_packages_brands_t2 where id in (SELECT id FROM pep.employee_targets_t2 where month="
											+ Month + " and year=" + Year + ")");
							s.executeUpdate(
									"delete FROM pep.employee_targets_packages_t2 where id in (SELECT id FROM pep.employee_targets_t2 where month="
											+ Month + " and year=" + Year + ")");
							s.executeUpdate(
									"delete FROM pep.employee_targets_t2 where month=" + Month + " and year=" + Year);
									...
									if (j == 0) {
										...
										if (!pjpid.equals("")) {
											psrid = (psrid.equals("")) ? "0" : psrid;
											  ...
											  System.out.println(
											  "insert into pep.employee_targets_t2 (employee_id,month,year,created_by,created_on,pjp_id) values("
											  + psrid + "," + Month + "," + Year + "," + created_by + ",now()," + pjpid
											  + ")");
											 
											s2.executeUpdate("insert into pep.employee_targets_t2 (employee_id,month,year,created_by,created_on,pjp_id) values("
...
//																+ targetFor.format(target) + "')");
												s3.executeUpdate(
														"insert into pep.employee_targets_packages_t2(id,package_id,quantity) values('"
																+ AutoID + "','" + rsData.getInt("package_id") + "','"
																+ targetFor.format(target) + "')");
//												System.out.println(
...
//																+ targetFor.format(target) + "')");
												s3.executeUpdate(
														"insert into pep.employee_targets_packages_brands_t2(id,package_id,brand_id,quantity) values('"
																+ AutoID + "','" + rsData.getInt("package_id") + "','"
																+ rsData.getInt("brand_id") + "','"
																+ targetFor.format(target) + "')");
