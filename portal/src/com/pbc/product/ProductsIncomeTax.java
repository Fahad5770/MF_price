...
@WebServlet(description = "Products Income Tax List", urlPatterns = { "/product/ProductsIncomeTax" })
public class ProductsIncomeTax extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public ProductsIncomeTax() {
		super();
	}
		...
		try {
			...
			for (int i = 0; i < PriceLTaxSapCode.length; i++) {
				...
				if (rsProductIds.next()) {
					...
					s1.executeUpdate(
							"INSERT INTO `peplogs`.`inventory_product_income_tax_logs`(`product_id`,`register_filer`,`register_non_filer`,`unregister_filer`,`unregister_non_filer`,`created_on`,`created_by`,`is_active`,`updated_on`,`updated_by`) select *, now(), "+UserID+" from inventory_product_income_tax where product_id="+productId);
					s1.executeUpdate("delete from inventory_product_income_tax where product_id="+productId);

					s1.executeUpdate(
							"INSERT INTO inventory_product_income_tax (product_id, register_filer, register_non_filer, unregister_filer, unregister_non_filer, created_on, created_by, is_active) "
