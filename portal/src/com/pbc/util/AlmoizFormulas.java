...
public class AlmoizFormulas {
	...
	public static HashMap<String, Double> ProductsTax_2(int productId, long outlet_id) {
		HashMap<String, Double> ProductTax = new HashMap<String, Double>();
		Datasource ds = new Datasource();
		try {
			ds.createConnection();
		...
		try {
			...
			String SelectText = (isRegister && isFiler) ? "FR"
					: (isRegister && !isFiler) ? "FUR"
							: (!isRegister && isFiler) ? "NFR" : (!isRegister && !isFiler) ? "NFUR" : "";

			System.out.println("SELECT " + SelectText + " FROM inventory_sales_tax where isActive=1");
			ResultSet rsSaleTax = s2
					.executeQuery("SELECT " + SelectText + " FROM inventory_sales_tax where isActive=1");
			ProductTax2.put("sales_tax", (rsSaleTax.first()) ? rsSaleTax.getDouble(1) : 0);
			System.out.println("SELECT " + SelectText + " FROM inventory_income_tax where isActive=1");
			ResultSet rsIncomeTax = s2
					.executeQuery("SELECT " + SelectText + " FROM inventory_income_tax where isActive=1");
