...
public class Report340DistributorSalesExcel {
	int FeatureID;
	...
	public Report340DistributorSalesExcel()
	...
	public void createExcel(final String filename, final Date StartDate, final Date EndDate, final long SessionUserID,
		...
		XSSFCell cellGroupheader3 = rowP1.createCell(col3 + 9 + col6);
		cellGroupheader3.setCellValue("FLOOR STOCKS AT DISTRIBUTIONS - (TONS");
		this.Set2ndHeaderBackColor(workbook, cellGroupheader3);
		spreadsheet.addMergedRegion(new CellRangeAddress(0, 0, col3 + 9 + col6, col3 + 9 + col6 + col0));
...
//		// Floor Stock
		...
		if (total_Clm == 0) {
			...
			String ASM_NAME="";
			System.out.println("Select distributor_id,name as DISTRIBUTOR_NAME,snd_id,city,tdm_id,rsm_id,region_id,(Select DISPLAY_NAME from users where users.id=cd.snd_id) as SND_NAME,(Select DISPLAY_NAME from users where users.distributor_id=cd.distributor_id) as RSM_NAME, (Select region_name from common_regions cr where cr.region_id=cd.Region_id) as region_name from common_distributors cd where distributor_id in ("+ DistributorID + ") ");
			...
			//	System.out.println("SELECT asm_id, (SELECT DISPLAY_NAME FROM users where users.ID=asm_id) as ASMName FROM pep.distributor_beat_plan where id in (SELECT distinct beat_plan_id FROM inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id=isap.id join inventory_products_view ipv on ipv.product_id=isap.product_id where isa.created_on between "+Utilities.getSQLDate(CurrentDate)+" and "+Utilities.getSQLDateNext(CurrentDate)+"  and  distributor_id="+Distributor_id+"  and isap.product_id in (select distinct package_id from inventory_products where lrb_type_id in (select id from inventory_products_lrb_types)))");
				ResultSet rsAsmName = s2.executeQuery("SELECT asm_id, (SELECT DISPLAY_NAME FROM users where users.ID=asm_id) as ASMName FROM pep.distributor_beat_plan where id in (SELECT distinct beat_plan_id FROM inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id=isap.id join inventory_products_view ipv on ipv.product_id=isap.product_id where isa.created_on between "+Utilities.getSQLDate(CurrentDate)+" and "+Utilities.getSQLDateNext(CurrentDate)+"  and  distributor_id="+rs13.getLong("distributor_id")+"  and isap.product_id in (select distinct package_id from inventory_products where lrb_type_id in (select id from inventory_products_lrb_types)))");
				if (rsAsmName.next()) {
					ASM_NAME = rsAsmName.getString("ASMName");
				}
				...
				while (true) {
					...
					headercell2 = headerrow22.createCell((int) (short) SerialColumns);
					headercell2.setCellValue(rs13.getString("DISTRIBUTOR_NAME"));
					this.SetNormalCellBackColor(workbook, headercell2);
					++SerialColumns;
					...
					headercell2 = headerrow22.createCell((int) (short) SerialColumns);
					headercell2.setCellValue(ASM_NAME);
					this.SetNormalCellBackColor(workbook, headercell2);
					++SerialColumns;

					headercell2 = headerrow22.createCell((int) (short) SerialColumns);
					headercell2.setCellValue(rs13.getString("RSM_NAME"));
					this.SetNormalCellBackColor(workbook, headercell2);
					++SerialColumns;

					headercell2 = headerrow22.createCell((int) (short) SerialColumns);
					headercell2.setCellValue(rs13.getString("SND_NAME"));
					this.SetNormalCellBackColor(workbook, headercell2);
					++SerialColumns;

					headercell2 = headerrow22.createCell((int) (short) SerialColumns);
					headercell2.setCellValue(rs13.getString("region_name"));
					this.SetNormalCellBackColor(workbook, headercell2);
					++SerialColumns;
					 ...
					 for (int i = 0; i < lrbIds.size(); i++) {
						 ...
						 System.out.println(
									"SELECT sum(total_units), ipv.liquid_in_ml FROM inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id=isap.id join inventory_products_view ipv on ipv.product_id=isap.product_id where isa.created_on between "
											...
											+ Distributor_id + "  and lrb_type_id=" + lrbIds.get(i)+ " and ipv.is_visible=1");
						...
						while (rs1.next()) {
							...
							headercell2 = headerrow22.createCell((int) (short) SerialColumns);
							headercell2.setCellValue(rs1.getDouble(1) * rs1.getDouble(2));
							this.SetNormalCellBackColor(workbook, headercell2);
							++SerialColumns;
						};
						...
						System.out.println(
								"SELECT (OV / OT) * 100 FROM(SELECT COUNT(DISTINCT outlet_id) AS OV, (SELECT COUNT(DISTINCT outlet_id)FROM  distributor_beat_plan_log WHERE distributor_id ="
										...
										+ Distributor_id + " AND log_date BETWEEN " + Utilities.getSQLDate(CurrentDate)
										+ " and " + Utilities.getSQLDateNext(CurrentDate) + ") AND  ipv.lrb_type_id="
										+ lrbIds.get(i) + ")     UP");
					...
					double TotalSecondarysales = 0;
					System.out.println(
							"SELECT sum(total_units), ipv.liquid_in_ml FROM  inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id=isap.id join inventory_products_view ipv on ipv.product_id=isap.product_id where ipv.is_visible=1 and isa.created_on between "
									+ Utilities.getSQLDate(CurrentDate) + " and "
									+ Utilities.getSQLDateNext(CurrentDate) + " and   distributor_id="
									+ Distributor_id);
					ResultSet rs27 = s2.executeQuery(
							"SELECT sum(total_units), ipv.liquid_in_ml FROM  inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id=isap.id join inventory_products_view ipv on ipv.product_id=isap.product_id where ipv.is_visible=1 and isa.created_on between "
									+ Utilities.getSQLDate(CurrentDate) + " and "
									+ Utilities.getSQLDateNext(CurrentDate) + " and   distributor_id="
									+ Distributor_id);
					...
					while (rs27.next()) {
						TotalSecondarysales = rs27.getLong(1);

						headercell2 = headerrow22.createCell((int) (short) SerialColumns);
						headercell2.setCellValue(TotalSecondarysales);
						this.SetNormalCellBackColor(workbook, headercell2);
						++SerialColumns;
					};
					...
					System.out.println(
							"SELECT (OV / OT) * 100 FROM(SELECT COUNT(DISTINCT outlet_id) AS OV, (SELECT COUNT(DISTINCT outlet_id)FROM  distributor_beat_plan_log WHERE distributor_id = "
									...
									+ " AND outlet_id IN (SELECT DISTINCT outlet_id FROM  distributor_beat_plan_log WHERE distributor_id ="
									+ Distributor_id + " AND log_date BETWEEN " + Utilities.getSQLDate(CurrentDate)
									+ " and " + Utilities.getSQLDateNext(CurrentDate) + ") )  UP");

					ResultSet rs35 = s2.executeQuery(
							"SELECT (OV / OT) * 100 FROM(SELECT COUNT(DISTINCT outlet_id) AS OV, (SELECT COUNT(DISTINCT outlet_id)FROM  distributor_beat_plan_log WHERE distributor_id = "
									+ Distributor_id + " AND log_date BETWEEN " + Utilities.getSQLDate(CurrentDate)
									+ " and " + Utilities.getSQLDateNext(CurrentDate)
									+ ") AS OT FROM  inventory_sales_adjusted isa JOIN inventory_sales_adjusted_products isap ON isa.id = isap.id JOIN inventory_products_view ipv ON ipv.product_id = isap.product_id WHERE isa.created_on BETWEEN "
									...
									+ " and " + Utilities.getSQLDateNext(CurrentDate) + ") )  UP");
					while (rs35.next()) {
						long TotalSecondarysalesUP = rs35.getLong(1);

						headercell2 = headerrow22.createCell((int) (short) SerialColumns);
						headercell2.setCellValue(TotalSecondarysalesUP);
						this.SetNormalCellBackColor(workbook, headercell2);
						++SerialColumns;
					...
					// PRIMARY SALES
					 for (int i = 0; i < lrbIds.size(); i++) {
						 ...
						 System.out.println(
							"SELECT sum(total_units), ipv.liquid_in_ml FROM  inventory_delivery_note isa join inventory_delivery_note_products isap on isa.delivery_id=isap.delivery_id join inventory_products_view ipv on ipv.product_id=isap.product_id where ipv.is_visible=1 and isa.created_on between "
							+ Utilities.getSQLDate(CurrentDate) + " and "
							+ Utilities.getSQLDateNext(CurrentDate) + "  and   distributor_id="
							+ Distributor_id + "   and lrb_type_id=" + lrbIds.get(i));
						...
						if (rs28.first()) {
							headercell2 = headerrow22.createCell((int) (short) SerialColumns);
							headercell2.setCellValue(rs28.getDouble(1) * rs28.getDouble(2));
							this.SetNormalCellBackColor(workbook, headercell2);
							++SerialColumns;
					...
					while (rs217.next()) {
						...
						headercell2 = headerrow22.createCell((int) (short) SerialColumns);
						headercell2.setCellValue(rs217.getLong(1) * rs217.getLong(2));
						this.SetNormalCellBackColor(workbook, headercell2);
						++SerialColumns;
					}
					;

					// long Total_Floor_Stock = Total_LMP + Total_SMP + Total_Gurr + Total_Shakkar +
					// Total_Beeta;
					// Total
					...
					for (int i = 0; i < lrbIds.size(); i++) {
						System.out.println("select product_id, liquid_in_ml from inventory_products_view where lrb_type_id=" + lrbIds.get(i)+"  and is_visible=1");
						ResultSet rs1446 = s2.executeQuery("select product_id, liquid_in_ml from inventory_products_view where lrb_type_id=" + lrbIds.get(i)+"  and is_visible=1");
						while (rs1446.next()) {
							Total_LMP += sp.getClosingBalance(Distributor_id, rs1446.getInt("product_id"), CurrentDate) * rs1446.getDouble("liquid_in_ml");
						}
						headercell2 = headerrow22.createCell((int) (short) SerialColumns);
						headercell2.setCellValue(Total_LMP);
						this.SetNormalCellBackColor(workbook, headercell2);
						++SerialColumns;
						GrandTotal += Total_LMP;
