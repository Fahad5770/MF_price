...
public class Report340DistributorSalesWithStockExcel {
	int FeatureID;
	...
	public Report340DistributorSalesWithStockExcel()
	...
	public void createExcel(final String filename, final Date StartDate, final Date EndDate, final long SessionUserID,
		...
		cellGroupheader.setCellValue("Brand Wise Sales & Stock Report (Tons) With Stock");
...
//		// Floor Stock
		...
		if (total_Clm == 0) {
			...
			// distributor_id,name,snd_id,city,tdm_id,rsm_id,region_id from
			// common_distributors where distributor_id in ("+ DistributorID + ") ");
			ResultSet rs13 = s.executeQuery(
					"Select distributor_id,name,snd_id,city,tdm_id,rsm_id,region_id from common_distributors where distributor_id in ("
							+ DistributorID + ") ");
			...
			while (rs13.next()) {
				...
				ResultSet rs151 = s2.executeQuery(
						"Select region_name from common_regions where region_id=" + rs13.getInt("region_id"));
				if (rs151.next()) {
					Region = rs151.getString("region_name");
				}

				// System.out.println("SELECT asm_id, (SELECT DISPLAY_NAME FROM users where
				// users.ID=asm_id) as ASMName FROM pep.distributor_beat_plan where id in
				// (SELECT distinct beat_plan_id FROM inventory_sales_adjusted isa join
				// inventory_sales_adjusted_products isap on isa.id=isap.id join
				// inventory_products_view ipv on ipv.product_id=isap.product_id where
				// isa.created_on between "+Utilities.getSQLDate(CurrentDate)+" and
				// "+Utilities.getSQLDateNext(CurrentDate)+" and
				// distributor_id="+Distributor_id+" and isap.product_id in (select distinct
				// package_id from inventory_products where lrb_type_id in (select id from
				// inventory_products_lrb_types)))");
				ResultSet rsAsm = s2.executeQuery(
						"SELECT asm_id, (SELECT DISPLAY_NAME FROM users where users.ID=asm_id) as ASMName FROM pep.distributor_beat_plan where id in (SELECT distinct beat_plan_id FROM inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id=isap.id join inventory_products_view ipv on ipv.product_id=isap.product_id where isa.created_on between "
								+ Utilities.getSQLDate(CurrentDate) + " and " + Utilities.getSQLDateNext(CurrentDate)
								+ "  and  distributor_id=" + Distributor_id
								+ "  and isap.product_id in (select distinct package_id from inventory_products where lrb_type_id in (select id from inventory_products_lrb_types)))");
				if (rsAsm.next()) {
				...
				ResultSet rsRSM = s2
						.executeQuery("SELECT DISPLAY_NAME FROM users where users.ID=" + rs13.getInt("rsm_id"));
				if (rsRSM.next()) {
...
//				System.out.println("SELECT DISPLAY_NAME FROM users where users.ID="+rs13.getInt("snd_id"));
				ResultSet rsSND = s2
						.executeQuery("SELECT DISPLAY_NAME FROM users where users.ID=" + rs13.getInt("snd_id"));
				if (rsSND.first()) {
					...
					for (int i = 0; i < lrbIds.size(); i++) {
						...
						ResultSet rs40 = s2.executeQuery(
								"SELECT (OV / OT) * 100 FROM(SELECT COUNT(DISTINCT outlet_id) AS OV, (SELECT COUNT(DISTINCT outlet_id)FROM  distributor_beat_plan_log WHERE distributor_id ="
										...
										+ " and " + Utilities.getSQLDateNext(CurrentDate)
										+ ")  and ipv.product_id in (select id from inventory_products where lrb_type_id = "
										+ lrbIds.get(i) + " and is_visible=1))     UP");
						if (rs40.first()) {
...
//						while (rs217.next()) {
					...
					for (int i = 0; i < lrbIds.size(); i++) {
						Total_LMP = 0;
						System.out.println(
								"============>>> select product_id, liquid_in_ml from inventory_products_view where lrb_type_id="
										+ lrbIds.get(i) + "  and is_visible=1");
						ResultSet rs1446 = s2.executeQuery(
								"select product_id, liquid_in_ml from inventory_products_view where lrb_type_id="
										+ lrbIds.get(i) + "  and is_visible=1");
						while (rs1446.next()) {
							Total_LMP += sp.getClosingBalance(Distributor_id, rs1446.getInt("product_id"), EndDate)
									* rs1446.getDouble("liquid_in_ml");

						}
						System.out.println("Total_LMP : " + Total_LMP);

						headercell2 = headerrow22.createCell((int) (short) SerialColumns);
						headercell2.setCellValue(((Total_LMP < 0) ? "E" : Double.toString(Total_LMP)));
						this.SetNormalCellBackColor(workbook, headercell2);
						++SerialColumns;
