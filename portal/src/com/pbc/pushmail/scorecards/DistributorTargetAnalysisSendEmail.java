...
public class DistributorTargetAnalysisSendEmail {
		...
		try {
		   ...
		   s.executeUpdate("CREATE  TABLE dist_targets_report_temp (distributor_id int,distributor_name varchar(100),sales decimal(18,2),target decimal(18,2), percentage varchar(50),package_id int,type_id int,snd_id int, rsm_id int, tdm_id int, target_mtd decimal(18,2))");
			...
			for(int i=1;i<=4;i++){
				...
				SDTitle = "SD"+i;
				String filename_DistributorTargetAnalysis = "DistributorTargetAnalysis_"+SDTitle+"_"+Utilities.getSQLDateWithoutSeprator(new java.util.Date())+".pdf";
				...
				DistributorTargetAnalysisPDF pack = new com.pbc.pushmail.scorecards.DistributorTargetAnalysisPDF();
				pack.createPdf(Utilities.getEmailAttachmentsPath()+ "/" + filename_DistributorTargetAnalysis, EmployeeHierarchy.getSDHead(i).USER_ID,ReportDate);
				...
				/*
				 *
					https://chart.googleapis.com/chart?cht=p3&chs=500x200&chd=t:27,1,1,26,13,7,5,1,2,8,3,2&chco=28495d|0c92e3|d9eaf5|87949c|5546ae|1acf6b|3b9262|b7f522|f5e922|f7bd1f|f78f1f|6a1d05&chl=250%20ML%20STD%7C1000%20ML%20G.%7C500%20ML%20PET%7C1000%20ML%20PET%7C1500%20ML%20PET%7C2250%20ML%20PET%7C1750%20ML%20PET%7C500%20ML%20PET%7C1500%20ML%20PET%7C240%20ML%20SSRB%7C500%20ML%20PET%7C200%20ML%20TETRA
				 */
				...
				//To: SD Head
				//CC: 
				Utilities.sendPBCHTMLEmail(new String[]{EmployeeHierarchy.getSDHead(i).USER_EMAIL},new String[]{"obaid@pbc.com.pk","anas.wahab@pbc.com.pk","imran.hashim@pbc.com.pk","ihsan@pbc.com.pk","omerfk@pbc.com.pk"},null, "DSAR | "+SDTitle+" | "+Utilities.getDisplayDateFormat(ReportDate), getHTMLMessage(EmployeeHierarchy.getSDHead(i).USER_ID, pack,ReportDate, false,GraphString,GraphStringAttainment), new String[]{filename_DistributorTargetAnalysis});
				//Utilities.sendPBCHTMLEmail(new String[]{"obaid@pbc.com.pk","dev@pbc.com.pk"},null,null, "DSAR | "+SDTitle+" | "+Utilities.getDisplayDateFormat(ReportDate), getHTMLMessage(EmployeeHierarchy.getSDHead(i).USER_ID, pack,ReportDate, false,GraphString,GraphStringAttainment), new String[]{filename_DistributorTargetAnalysis});
			...
			for (User user: RSMs){
				...
				if (user.USER_EMAIL != null && user.USER_ID != EmployeeHierarchy.getSDHead(4).USER_ID){
					...
					Utilities.sendPBCHTMLEmail(new String[]{user.USER_EMAIL},new String[]{SND.USER_EMAIL,"obaid@pbc.com.pk","anas.wahab@pbc.com.pk","imran.hashim@pbc.com.pk","ihsan@pbc.com.pk"}, null, "DSAR | "+user.REGION_NAME+" | "+Utilities.getDisplayDateFormat(ReportDate), getHTMLMessage(user.USER_ID, pack,ReportDate, true,"",""), new String[]{filename_DistributorTargetAnalysis});
					//Utilities.sendPBCHTMLEmail(new String[]{"obaid@pbc.com.pk","dev@pbc.com.pk"},null, null, "DSAR | "+user.REGION_NAME+" | "+Utilities.getDisplayDateFormat(ReportDate), getHTMLMessage(user.USER_ID, pack,ReportDate, true,"",""), new String[]{filename_DistributorTargetAnalysis});
			...
			for (User user: TDMs){
				...
				if (user.USER_EMAIL != null){
					...
					if (RSM.USER_EMAIL != null && SND.USER_EMAIL != null){
						...
						Utilities.sendPBCHTMLEmail(new String[]{user.USER_EMAIL},new String[]{SND.USER_EMAIL,RSM.USER_EMAIL,"obaid@pbc.com.pk","anas.wahab@pbc.com.pk","ihsan@pbc.com.pk"}, null, "DSAR | "+user.REGION_NAME+" | "+user.USER_DISPLAY_NAME+" | "+Utilities.getDisplayDateFormat(ReportDate), getHTMLMessage(user.USER_ID, pack,ReportDate, true,"",""), new String[]{filename_DistributorTargetAnalysis});
						//Utilities.sendPBCHTMLEmail(new String[]{"obaid@pbc.com.pk","dev@pbc.com.pk"},null, null, "DSAR | "+user.REGION_NAME+" | "+user.USER_DISPLAY_NAME+" | "+Utilities.getDisplayDateFormat(ReportDate), getHTMLMessage(user.USER_ID, pack,ReportDate, true,"",""), new String[]{filename_DistributorTargetAnalysis});
	...
	public static String getHTMLMessage(long EmployeeID, DistributorTargetAnalysisPDF pack, Date CurrentDate, boolean isRSM,String SNDGraphTarget,String SNDGraphAttainment) throws ClassNotFoundException, InstantiationException, IllegalAccessException, SQLException{
		...
		ResultSet rs141 = s3.executeQuery("select min(dt.start_date), max(dt.end_date) from distributor_targets dt where date("+Utilities.getSQLDate(CurrentDate)+") between dt.start_date and dt.end_date and dt.type_id=3");
	    if(rs141.first()){
		...
		ResultSet rs601 = s.executeQuery("SELECT group_concat(distinct sap_code) from dist_targets_temp dtt where dtt.order_date between "+Utilities.getSQLDate(MinTargetDate)+" and "+Utilities.getSQLDate(MaxTargetDate));
		if (rs601.next()){
		...
		html +="<th data-priority='1'  style='text-align:center; min-width:150px; background-color:#404040; color: white;;border-right:1px solid #fff; min-height: 20px'>&nbsp;</th>";
		html +="<th style='text-align:center; background-color:#404040;border-right:1px solid #fff; color:white;' colspan='3'>CSD</th>";
		html +="<th style='text-align:center; background-color:#404040;border-right:1px solid #fff; color:white' colspan='3'>NCB</th>";
		html +="<th style='text-align:center; background-color:#404040;border-right:1px solid #fff; color:white' colspan='3'>LRB</th>";
		html +="</tr>";		
		
		
		html +="<tr style='font-size:12px;'>";
		html +="<th data-priority='1'  style='text-align:center; background-color:#9E9D9D; c1olor: white;;border-right:1px solid #fff; min-height: 20px'>&nbsp;</th>";
		html +="<th style='text-align:center; background-color:#9E9D9D;border-right:1px solid #fff; c1olor:white; width:75px;'>Target</th>";
		html +="<th style='text-align:center; background-color:#9E9D9D;border-right:1px solid #fff; c1olor:white;width:75px;'>Attainment</th>";
		html +="<th style='text-align:center; background-color:#9E9D9D;border-right:1px solid #fff; c1olor:white;width:75px'>%</th>";
		...
		while(rs11321.next()){
			...
			if (!isRSM){
			html +="<tr>";
			html +="<td style='background-color:#C2C2C2; font-weight: bold; font-size: 13px; border-right:1px solid #fff;'>"+SNDDisplay1+"</td>";	
						...
						if(rs3152.first()){
							...
							if(TargetCoverted1!=0){
								html +="<td style='text-align: center; background-color:#C2C2C2; border-right:1px solid #fff;font-weight: bold; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(TargetCoverted1)+"</td>";
							}else{
								html +="<td style='text-align: center; background-color:#C2C2C2; border-right:1px solid #fff;font-weight: bold; font-size: 13px;'></td>";
							}
							
							if(SalesCoverted1!=0){
								html +="<td style='text-align: center; background-color:#C2C2C2; border-right:1px solid #fff;font-weight: bold; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(SalesCoverted1)+"</td>";
							}else{
								html +="<td style='text-align: center; background-color:#C2C2C2; border-right:1px solid #fff;font-weight: bold; font-size: 13px;'></td>";
							}
							...
							if(CasesCovertedPercentage11!=0){
								html +="<td style='text-align: center; background-color:#C2C2C2; border-right:1px solid #fff;font-weight: bold; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(CasesCovertedPercentage11)+"%</td>";
							}else{
								html +="<td style='text-align: center; background-color:#C2C2C2; border-right:1px solid #fff;font-weight: bold; font-size: 13px;'></td>";
							}
...
////LRB
		...
		while(rs11311.next()){
			...
			html +="<tr>";
			html +="<td style='background-color:#D4D4BB;font-weight: bold; border-right:1px solid #fff;font-size: 13px;'>"+RSMDisplay1+"</td>";	
						...
						if(TargetCoverted1!=0){
							html +="<td style='text-align: center;background-color:#D4D4BB;border-right:1px solid #fff;font-weight: bold; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(TargetCoverted1)+"</td>";
						}else{
							html +="<td style='text-align: center;background-color:#D4D4BB;border-right:1px solid #fff;font-weight: bold; font-size: 13px;'></td>";
						}
						
						if(SalesCoverted1!=0){
							html +="<td style='text-align: center;background-color:#D4D4BB;border-right:1px solid #fff;font-weight: bold; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(SalesCoverted1)+"</td>";
						}else{
							html +="<td style='text-align: center;background-color:#D4D4BB;border-right:1px solid #fff;font-weight: bold; font-size: 13px;'></td>";
						}
						
						if(RSMConvetedCasesPercentage!=0){
							html +="<td style='text-align: center;background-color:#D4D4BB;border-right:1px solid #fff;font-weight: bold; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(RSMConvetedCasesPercentage)+"%</td>";
						}else{
							html +="<td style='text-align: center;background-color:#D4D4BB;border-right:1px solid #fff;font-weight: bold; font-size: 13px;'></td>";
						}
						...
						if(rs31521i.first()){
							...
							if(CasesCovertedPercentage11!=0){
								html +="<td style='text-align: center;background-color:#D4D4BB;border-right:1px solid #fff;font-weight: bold; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(CasesCovertedPercentage11)+"%</td>";
							}else{
								html +="<td style='text-align: center;background-color:#D4D4BB;border-right:1px solid #fff;font-weight: bold; font-size: 13px;'></td>";
							}
						}
...
/////LRB
				...
				ResultSet rs99 = s20.executeQuery("select month,year,start_date,end_date from distributor_targets where distributor_id in (SELECT distributor_id FROM dist_targets_report_temp where rsm_id in("+RSMID11+")) and "+Utilities.getSQLDate(CurrentDate)+" between start_date and end_date limit 1");
				if(rs99.first()){
		...
		while(rs1131.next()){
			...
			html +="<tr>";
			html +="<td style='font-weight: normal; background-color:#FCFCFC;border-right:1px solid #fff;font-size: 13px;'>"+TDMDisplay1+"</td>";	
						...
						if(rs3151i.first()){
							...
							if(CasesCovertedPercentage11!=0){
								html +="<td style='background-color:#FCFCFC; text-align: center;border-right:1px solid #fff;font-weight: normal; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(CasesCovertedPercentage11)+"%</td>";
							}else{
								html +="<td style='background-color:#FCFCFC; text-align: center;border-right:1px solid #fff;font-weight: normal; font-size: 13px;'></td>";
							}
...
/////LRB
	...
	html +="<br><br/>";		
		...
		html +="<tr style='font-size:12px;'>";
		html +="<th data-priority='1'  style='text-align:center; min-width:130px;background-color:#404040; color: white;;border-right:1px solid #fff; min-height: 20px' colspan='2'>&nbsp;</th>";
		...
		while(rs101.next()){
			...
			html +="<th data-priority='1'  style='text-align:center; background-color:#404040; color: white;;border-right:1px solid #fff' colspan="+rs101.getInt("packge_count")+">"+rs101.getString("type_name") +"</th>";
	   ...
	   // long DistributorIDD1=0;
		...
		while(rs1132.next()){
			...
			if (!isRSM){
			html +="<tr>";
			html +="<td style='background-color:#C2C2C2; font-weight: bold; font-size: 13px; border-right:1px solid #fff;'>"+SNDDisplay+"</td>";	
						...
						if(CasesCovertedPercentage1!=0){
							html +="<td style='text-align: center; background-color:#C2C2C2; border-right:1px solid #fff;font-weight: bold; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(CasesCovertedPercentage1)+"%</td>";
						}else{
							html +="<td style='text-align: center; background-color:#C2C2C2; border-right:1px solid #fff;font-weight: bold; font-size: 13px;'></td>";
						}
				...
				while(rs1232.next()){
					...
					if(CasesPercentage1!=0){
						html +="<td style='text-align: center; background-color:#C2C2C2; border-right:1px solid #fff;font-weight: bold; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(CasesPercentage1)+"%</td>";
					}else{
						html +="<td style='text-align: center; background-color:#C2C2C2; border-right:1px solid #fff;font-weight: bold; font-size: 13px;'></td>";
					}
		...
		while(rs1131.next()){
			...
			html +="<tr>";
			html +="<td style='background-color:#D4D4BB;font-weight: bold; border-right:1px solid #fff;font-size: 13px;'>"+RSMDisplay+"</td>";	
						...
						if(CasesCovertedPercentage1!=0){
							html +="<td style='text-align: center;background-color:#D4D4BB;border-right:1px solid #fff;font-weight: bold; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(CasesCovertedPercentage1)+"%</td>";
						}else{
							html +="<td style='text-align: center;background-color:#D4D4BB;border-right:1px solid #fff;font-weight: bold; font-size: 13px;'></td>";
						}
				...
				while(rs1231.next()){
					...
					if(CasesPercentage1!=0){
						html +="<td style='text-align: center;background-color:#D4D4BB;border-right:1px solid #fff;font-weight: bold; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(CasesPercentage1)+"%</td>";
					}else{
						html +="<td style='text-align: center;background-color:#D4D4BB;border-right:1px solid #fff;font-weight: bold; font-size: 13px;'></td>";
					}
				...
				Date DisplayStartDate=new Date();
				Date DisplayEndDate=new Date();
				ResultSet rs99 = s20.executeQuery("select month,year,start_date,end_date from distributor_targets where distributor_id in (SELECT distributor_id FROM dist_targets_report_temp where rsm_id in("+RSMID1+")) and "+Utilities.getSQLDate(CurrentDate)+" between start_date and end_date limit 1");
				if(rs99.first()){
					 MonthNumber = rs99.getInt("month");
					 YearNumber = rs99.getInt("year");
					 DisplayStartDate = rs99.getDate("start_date");
		...
		while(rs113.next()){
			...
			html +="<tr>";
			html +="<td style='font-weight: normal; background-color:#FCFCFC;border-right:1px solid #fff;font-size: 13px;'>"+TDMDisplay+"</td>";	
						...
						if(CasesCovertedPercentage1!=0){
							html +="<td style='background-color:#FCFCFC; text-align: center;border-right:1px solid #fff;font-weight: normal; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(CasesCovertedPercentage1)+"%</td>";
						}else{
							html +="<td style='background-color:#FCFCFC; text-align: center;border-right:1px solid #fff;font-weight: normal; font-size: 13px;'></td>";
						}
				...
				while(rs123.next()){
					...
					if(CasesPercentage1!=0){
						html +="<td style='background-color:#FCFCFC; text-align: center;border-right:1px solid #fff;font-weight: normal; font-size: 13px;'>"+Utilities.getDisplayCurrencyFormatRounded(CasesPercentage1)+"%</td>";
					}else{
						html +="<td style='background-color:#FCFCFC; text-align: center;border-right:1px solid #fff;font-weight: normal; font-size: 13px;'></td>";
					}
