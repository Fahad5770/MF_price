...
public class OutletWorkFlowIssueStep5 {
	...
	InstantiationException, IllegalAccessException {
		...
		try {
			...
			ResultSet rsMain = s.executeQuery("SELECT request_id FROM pep.workflow_requests where current_step_id=5  and status_id=1");
			while(rsMain.next()) {
				...
				int seqId=0;
				ResultSet rsSequenceID = s2.executeQuery("SELECT sequence_id FROM pep.workflow_requests_steps  where request_id="+requestId+" and completed_on is null and step_id=5");
