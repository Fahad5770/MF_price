...
@WebServlet(description = "Stock Stock Adjustment", urlPatterns = { "/inventory/StockTransferExecuteOld" })
public class StockTransferExecuteOld extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public StockTransferExecuteOld() {
		super();
	}
			...
			if (rs1.first()) {
				...
				int PhysicalStockAdjustmentID = 0;
					if(ProductID.length > 0) {
						if (isEditCase) {
							SQLMain = "replace into inventory_distributor_stock_transfer (id, uvid, created_on, created_by, from_distributor_id,to_distributor_id) values("
