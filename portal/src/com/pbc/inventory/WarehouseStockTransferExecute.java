...
@WebServlet(description = "Warehouse Stock TransferExecute ", urlPatterns = { "/inventory/WarehouseStockTransferExecute" })
public class WarehouseStockTransferExecute extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public WarehouseStockTransferExecute() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int WareHouseNumber = Utilities.parseInt(request.getParameter("WarehouseNumberhidden"));
		...
		try {
			...
			if(isEditHiddenFlag==1){ //1=insertion
				s.executeUpdate("insert into inventory_warehouse_transfers(created_on,created_by,vehicle_no,uvid,warehouse_id) values(now(),"+UserID+",'"+VehicleNumber+"',"+UniqueVoucherID+",'"+WareHouseNumber+"')");
				...
				ResultSet rs4 = s.executeQuery("select created_on,is_received from inventory_warehouse_transfers where id="+ReceiptFromProductionEditID);
				if(rs4.first()){
				...
				if(IsSameDay && IsReceivedValid){ //only same day vocher can edit
					s.executeUpdate("update inventory_warehouse_transfers set vehicle_no='"+VehicleNumber+"',warehouse_id='"+WareHouseNumber+"' where id="+ReceiptFromProductionEditID);
					s.executeUpdate("Delete from inventory_warehouse_transfers_products where id="+ReceiptFromProductionEditID); //delete record from sub table agains edit id then insert
					...
					if(IsReceivedValid == false){
						obj.put("error", "Document has already been received, it cannot be edited.");
					}else{
			...
			if(IsSameDay && IsReceivedValid){
				if(ProductID !=null) {
					for(int i=0; i<ProductID.length; i++) {
						...
						s.executeUpdate("insert into inventory_warehouse_transfers_products (id,product_id,raw_cases,units,total_units,liquid_in_ml) values("+MasterTableID+","+ProductID[i]+","+RawCases[i]+","+Units[i]+","+TotalUnits+","+LiquInML+")");
