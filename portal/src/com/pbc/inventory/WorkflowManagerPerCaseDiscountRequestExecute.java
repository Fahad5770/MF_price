...
@WebServlet(description = "Monthly Discount Request Execute", urlPatterns = { "/inventory/WorkflowManagerPerCaseDiscountRequestExecute" })

public class WorkflowManagerPerCaseDiscountRequestExecute extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public WorkflowManagerPerCaseDiscountRequestExecute() {
        super();
    }
		...
		try {
			...
			*/
			...
			if (StepID == 2){ // CE Approval
				String HTMLEmail = WorkflowEmail.getPerCaseDiscountRequestHTMLWithActionButtons(RequestIDVal);
				String HTMLEmailCOO = WorkflowEmail.getPerCaseDiscountRequestHTMLWithActionButtonsCOO(RequestIDVal);
				
				//Getting Distributor Name
				
				//SELECT * FROM inventory_sales_discounts_request isdr join inventory_sales_discounts_request_distributors isdrd on isdr.id=isdrd.product_promotion_id where isdr.request_id=24572
				
				String DistributorName ="";
				ResultSet rs = s.executeQuery("SELECT distributor_id,(select name from common_distributors cd where isdrd.distributor_id=cd.distributor_id) name FROM inventory_primary_percase_request isdr join inventory_primary_percase_request_distributors isdrd on isdr.id=isdrd.product_promotion_id where isdr.request_id="+RequestIDVal);
				if(rs.first()){
					DistributorName = rs.getString("name");
				}
				
				//System.out.println("Hello -"+DistributorName);

				
				
				Utilities.sendPBCHTMLEmail(new String[]{"salman.baig@pbc.com.pk"}, null, new String[]{"anas.wahab@pbc.com.pk"}, "Per Case Discount Request | "+DistributorName+" | ID#"+RequestIDVal, HTMLEmail, null);
				Utilities.sendPBCHTMLEmail(new String[]{"omerfk@pbc.com.pk"}, null, new String[]{"anas.wahab@pbc.com.pk"}, "Per Case Discount Request | "+DistributorName+" | ID#"+RequestIDVal, HTMLEmailCOO, null);
				
				//Utilities.sendPBCHTMLEmail(new String[]{"zulqurnan.aslam@pbc.com.pk"}, null, null, "Per Case Discount Request | "+DistributorName+" | ID#"+RequestIDVal, HTMLEmail, null);
				//Utilities.sendPBCHTMLEmail(new String[]{"dev@pbc.com.pk"}, null, new String[]{"dev@pbc.com.pk"}, "Upfront Discount Request | "+DistributorName+" | ID#"+RequestIDVal, HTMLEmailCOO, null);
