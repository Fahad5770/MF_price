 ...
 */
@WebServlet(description = "User Additional Rights", urlPatterns = { "/inventory/ProductPromotionsAdditionalRightExecute" })
public class ProductPromotionsAdditionalRightExecute extends HttpServlet {
     ...
     */
    public ProductPromotionsAdditionalRightExecute() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			Statement s = ds.createStatement();		
				...
				query = "delete from inventory_sales_promotions_regions where product_promotion_id = "+ProductPromotionsID;
				...
				if(request.getParameterValues("RegionIDhiddenfield") != null)
				{
					...
					for(int ii=0;ii<SelectedRegions.length;ii++)
					{
						...
						query2 = "insert into " +
				                "inventory_sales_promotions_regions(product_promotion_id,region_id)" +
				...
				query = "delete from inventory_sales_promotions_distributors where product_promotion_id = "+ProductPromotionsID;
				s.executeUpdate(query);
				...
				if(request.getParameterValues("DistributorIDhiddenfield") != null)
				{
					...
					for(int ii=0;ii<SelectedDistributor.length;ii++)
					{
						...
						query2 = "insert into " +
				                "inventory_sales_promotions_distributors(product_promotion_id,distributor_id)" +
				...
				query = "delete from inventory_sales_promotions_pjp where product_promotion_id = "+ProductPromotionsID;
				s.executeUpdate(query);
				...
				if(request.getParameterValues("PJPhiddenfield") != null)
				{
					...
					for(int ii=0;ii<SelectedPJPs.length;ii++)
					{
						...
						query2 = "insert into " +
				                "inventory_sales_promotions_pjp(product_promotion_id,pjp_id)" +
				...
				//updating outlet 
				query = "delete from inventory_sales_promotions_outlet where product_promotion_id = "+ProductPromotionsID;
				s.executeUpdate(query);
				...
				if(request.getParameterValues("OutletIDhiddenfield") != null)
				{
					...
					for(int ii=0;ii<SelectedOutlet.length;ii++)
					{
						...
						//System.out.println("Distributor ID "+FeatureIDndDistributorID[0]+" Feature ID "+FeatureIDndDistributorID[1]);
						query2 = "insert into " +
				                "inventory_sales_promotions_outlet(product_promotion_id,outlet_id)" +
				                "values("+ProductPromotionsID+","+FeatureIDndDistributorID[0]+")";
						s.executeUpdate(query2);
						//System.out.println(query2);
				...
				query = "delete from inventory_sales_promotions_employee where product_promotion_id = "+ProductPromotionsID;
				s.executeUpdate(query);
				...
				if(request.getParameterValues("OrderBookerhiddenfield") != null)
				{
					...
					for(int ii=0;ii<SelectedOrderBooker.length;ii++)
					{
						...
						//System.out.println("Distributor ID "+FeatureIDndDistributorID[0]+" Feature ID "+FeatureIDndDistributorID[1]);
						query2 = "insert into " +
				                "inventory_sales_promotions_employee(product_promotion_id,employee_id)" +
				                "values("+ProductPromotionsID+","+FeatureIDndDistributorID[0]+")";
						s.executeUpdate(query2);
						//System.out.println(query2);
