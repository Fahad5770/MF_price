...
@WebServlet(description = "Price List ", urlPatterns = { "/inventory/ProductDiscountsRequestExecute" })
public class ProductDiscountsRequestExecute extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public ProductDiscountsRequestExecute() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		double OtherCost[] = Utilities.parseDouble(request.getParameterValues("OtherCost"));
		...
		try {
			...
			ResultSet rs11 = s.executeQuery("select id from inventory_sales_discounts_request where uvid="+UniqueVoucherID);
			if( rs11.first() ){
				...
				obj.put("error", "Already Exists");
					...
					if(Utilities.parseLong(request.getParameter("isEditCase"))==0)//insertion case master table
					{
						...
						s.executeUpdate("insert into inventory_sales_discounts_request(uvid,valid_from,valid_to,is_active,created_on,created_by,comments) values("+UniqueVoucherID+","+ Utilities.getSQLDate(ValidFromDate) +","+Utilities.getSQLDate(ValidToDate)+","+Active+",now(),"+UserID+",'"+Comments+"')");
					...
					if(SalesPacakgeID != null)
					{
						...
						{					
							...
							s.executeUpdate("insert into inventory_sales_discounts_request_products(id,package_id,type_id,quantity,discount_rate,selling_price,variable_cost,other_cost,promotion_cost,marginal_contribution) values("+MasterTablePromotionID+","+ SalesPacakgeID[i] +",1"+","+Quantity[i]+","+DiscountRate[i]+","+SellingPrice[i]+","+CalculatedVariableCost+","+OtherCost[i]+","+PromotionCost[i]+","+MarginalContribution[i]+")"); //type id = 1 for sales 2 for promotions
							ResultSet rs = s.executeQuery("select LAST_INSERT_ID()");
							if(rs.first()){
							...
							if(SalesBrandID !=null)
							{
								...
								{
									ResultSet rs12 = s.executeQuery("SELECT id FROM inventory_products where package_id="+SalesPacakgeID[i]+" and brand_id="+SalesBrandID[j]+" and category_id=1");
									if(rs12.first()){
										ProductID = rs12.getInt("id");
									}
									s.executeUpdate("insert into inventory_sales_discounts_request_products_brands(id,package_id,brand_id,type_id,product_id,serial_no) values("+MasterTablePromotionID+","+ SalesPacakgeID[i] +","+SalesBrandID[j]+",1,"+ProductID+","+MasterTableSerialID+")"); //type id = 1 for sales 2 for promotions
								}
							}
							RowCount++;
							SerialNo++;
					...
					long ForwardedUserID=0;
					ResultSet rs1 = s.executeQuery("SELECT * FROM pep.workflow_processes_steps where process_id=4 and step_id=2");
					if(rs1.first()){
						ForwardedUserID = rs1.getLong("user_id");
					}
					
					
					Workflow wf = new Workflow();
					long WorkFlowRequestID = wf.createRequest(4, Integer.parseInt(UserID), ForwardedUserID, 3, "Discount Request Raised");
					s.executeUpdate("update inventory_sales_discounts_request set request_id="+WorkFlowRequestID+" where id="+MasterTablePromotionID);
