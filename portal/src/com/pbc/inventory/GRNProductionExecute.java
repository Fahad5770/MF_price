...
@WebServlet(description = "Executes GRN", urlPatterns = { "/inventory/GRNProductionExecute" })
public class GRNProductionExecute extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public GRNProductionExecute() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ResultSet rs = s.executeQuery("select * from inventory_production_receipts where uvid="+Barcode);
			if( rs.first() ){
				
				GRNStockDocument.DISTRIBUTOR_ID =200769;
				GRNStockDocument.DOCUMENT_TYPE_ID = 19;
				GRNStockDocument.DOCUMENT_ID = rs.getLong("id");
				GRNStockDocument.CREATED_ON = rs.getTimestamp("created_on");
				GRNStockDocument.CREATED_BY = rs.getLong("created_by");
				
				
				ResultSet rs2 = s2.executeQuery("select iprp.product_id, iprp.raw_cases, iprp.units, iprp.total_units, iprp.liquid_in_ml from inventory_production_receipts_products iprp, inventory_products_view ipv where iprp.product_id = ipv.product_id and iprp.id = "+rs.getLong("id")+" and ipv.category_id = 1");
				while( rs2.next() ){
			...
			s.executeUpdate("update inventory_production_receipts set is_received=1, received_on=now(), received_by="+UserID+" where uvid="+Barcode);
