...
@WebServlet(description = "GL Unpost Invoice Execute", urlPatterns = { "/cash/GLUnpostInvoiceExecute" })
public class GLUnpostInvoiceExecute extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public GLUnpostInvoiceExecute() {
        super();
    }
		...
		if( isValidationClear ){
			...
			try {
				...
				if( rs.first() ){
					...
					s.executeUpdate("delete from gl_invoice_posting where id="+GLInvoicePostingID);
					
					
					s.executeUpdate("update gl_order_posting set is_invoiced = 0 where order_no = "+UnpostedOrderNo);
