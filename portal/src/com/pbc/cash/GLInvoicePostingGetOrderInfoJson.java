...
@WebServlet(description = "Get Order Info Json", urlPatterns = { "/sap/GLInvoicePostingGetOrderInfoJson" })

public class GLInvoicePostingGetOrderInfoJson extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GLInvoicePostingGetOrderInfoJson() {
        super();
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			*/
			...
			ResultSet rs10 = s.executeQuery("SELECT * FROM gl_order_posting where order_no = "+SaleOrderNo);
			if( rs10.first() ){
				isOrderPosted = true;
			}
			...
			if(isDuplicate == true){
				...
				JsonSuccess = false;
				if(isLifted){
					JsonErrorMessage = "Already Lifted";
				}else{
				...
				try {
					...
					if (tab[0].getNumRows() > 0){
						...
						ResultSet rs = s3.executeQuery("SELECT credit_limit FROM gl_customer_credit_limit where customer_id="+tab[0].getString("KNKLI")+" and curdate() between valid_from and valid_to and is_active=1");
						if(rs.first()){
						...
						long AdvanceAccountID = 0;
						ResultSet rs7 = s3.executeQuery("SELECT id FROM gl_accounts where customer_id="+tab[0].getString("KNKLI")+" and type_id=6");
						if( rs7.first() ){
							AdvanceAccountID = rs7.getLong("id");
						}
						
						
						double AdvanceAccountBalance = 0;
						ResultSet rs6 = s3.executeQuery("SELECT sum(debit) total_debit, sum(credit) total_credit FROM gl_transactions_accounts where account_id="+AdvanceAccountID);
						if( rs6.first() ){
							AdvanceAccountBalance = rs6.getDouble(1) - rs6.getDouble(2);
							AdvanceAccountBalance = AdvanceAccountBalance * (-1);
						}
						...
						double AdvanceAgainstOrder = 0;
						long OrderCustomerID = 0;
						ResultSet rs5 = s3.executeQuery("SELECT order_amount, customer_id FROM gl_order_posting where order_no = "+SaleOrderNo);
						if(rs5.first()){
							AdvanceAgainstOrder = rs5.getDouble(1);
							OrderCustomerID = rs5.getLong(2);
						}
						...
						if (OrderCustomerID != Utilities.parseLong(tab[0].getString("KNKLI"))){
							JsonSuccess = false;
							JsonErrorMessage = "Customer ID did not match with Order";
						}
						...
						if (Math.round(AdvanceAccountBalance) < Math.round(AdvanceAgainstOrder)){
							AdvanceAgainstOrder = 0;
							JsonObj.put("AdvanceAgainstOrderInsufficientBalance", "true");
							JsonSuccess = false;
							JsonErrorMessage = "Insufficient balance in Advance account";
							
						}else{
							JsonObj.put("AdvanceAgainstOrderInsufficientBalance", "false");
						}
						
						
						JsonObj.put("AdvanceAgainstOrder", AdvanceAgainstOrder);
						...
						if (isOrderPosted == false){
							JsonSuccess = false;
							JsonErrorMessage = "Advance is not received against this order.";
						}
