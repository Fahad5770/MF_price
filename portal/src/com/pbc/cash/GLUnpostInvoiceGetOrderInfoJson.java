...
@WebServlet(description = "GL Unpost Invoice Get Order Info Json", urlPatterns = { "/sap/GLUnpostInvoiceGetOrderInfoJson" })

public class GLUnpostInvoiceGetOrderInfoJson extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GLUnpostInvoiceGetOrderInfoJson() {
        super();
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			ResultSet rs = s.executeQuery("SELECT *, (SELECT name FROM common_distributors where distributor_id=gip.customer_id) customer_name FROM gl_invoice_posting gip where gip.order_no = "+SaleOrderNo);
			if( rs.first() ){
				
				JsonSuccess = true;
				
				JsonObj.put("OrderDate", Utilities.getDisplayDateFormat(rs.getDate("order_date")));
				JsonObj.put("InvoiceNo", rs.getString("invoice_no"));
				JsonObj.put("InvoiceDate", Utilities.getDisplayDateFormat(rs.getDate("invoice_date")));
				JsonObj.put("InvoiceAmount", rs.getString("invoice_amount"));
