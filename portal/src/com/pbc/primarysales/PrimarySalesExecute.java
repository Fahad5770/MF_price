...
@WebServlet(description = "Executes Desk Sale", urlPatterns = { "/primarysales/PrimarySalesExecute" })
public class PrimarySalesExecute extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public PrimarySalesExecute() {
        super();
    }
			...
			if( DateUtils.isSameDay(new java.util.Date(), VoucherDate) ){	// false only in editcase on date conflict
				...
				if( rs1.first() ){
					...
					//s.getConnection().setAutoCommit(false);
						...
						SQLMain = "insert into primary_sales_orders (uvid, created_on, created_by,   distributor_id, invoice_amount,plant_id) values("+UniqueVoucherID+", now(), "+UserID+","+DistributorID+",0,"+PlantID+")";
					...
					s.executeUpdate("delete from primary_sales_orders_products where id="+DeskSaleID);
					...
					double InvoiceNetAmount = 0;				
						...
						s.executeUpdate("update primary_sales_orders set invoice_amount = "+InvoiceAmount+" where id = "+DeskSaleID);
					
						
						//inserting into delivery note
						
						
						ResultSet rs = s.executeQuery("SELECT * FROM primary_sales_orders where id="+DeskSaleID);
						while(rs.next()){
							int InventoryDeliveryNoteIDMaster=0;
							
							s2.executeUpdate("INSERT INTO inventory_delivery_note (created_on,created_by,distributor_id,remarks,vehicle_no,sap_order_no,barcode,is_delivered,delivered_on,delivered_by,is_received,warehouse_id,payment_method,is_partial,invoice_no,vehicle_type,uvid,invoice_amount,freight_amount,outsourced_primary_sales_id) "+
											" VALUES( from_days(to_days(curdate())-1),"+rs.getLong("created_by")+","+rs.getLong("distributor_id")+",'','',"+rs.getLong("id")+","+rs.getLong("uvid")+",1,"+Utilities.getSQLDateTime(rs.getTimestamp("created_on"))+",1,0,1,1,0,"+rs.getLong("id")+",6,"+rs.getLong("uvid")+","+rs.getDouble("invoice_amount")+",0,"+rs.getLong("id")+
											")");
							
							ResultSet rs11 = s2.executeQuery("select LAST_INSERT_ID()");
							if(rs11.first()){
								InventoryDeliveryNoteIDMaster = rs11.getInt(1);
							}
							
							//Detail table entry
							
							ResultSet rs2 = s3.executeQuery("SELECT * FROM primary_sales_orders_products where id="+rs.getLong("id"));
							while(rs2.next()){
								s4.executeUpdate("INSERT INTO inventory_delivery_note_products(delivery_id,product_id,raw_cases,units,total_units,liquid_in_ml,batch_code) VALUES("+InventoryDeliveryNoteIDMaster+","+rs2.getLong("product_id")+","+rs2.getLong("raw_cases")+","+rs2.getLong("units")+","+rs2.getLong("total_units")+","+rs2.getLong("liquid_in_ml")+",'')");
							}
