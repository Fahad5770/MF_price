		...
		<td style="width: 100%" valign="top">
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					...
					<tbody>
						...
						String SQL="select mo.id,mo.mobile_order_no,date(mo.created_on) CreatedDate,time(created_on) CreatedTime,(select concat(co.id,' - ', co.name ) from common_outlets co where co.id=mo.outlet_id ) Outlet ,(select concat(cd.distributor_id,' - ', cd.name ) from common_distributors cd where cd.distributor_id=mo.distributor_id ) customer_name,(select concat(u.ID,' - ', u.Display_Name ) from users u where u.id=mo.created_by ) BookedBy,(select most.label from mobile_order_status_types most where most.id=mo.status_type_id) Status,mo.lat,mo.lng,mo.accuracy,mo.unedited_order_id from mobile_order mo where created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+OrderBookerIDsWhere+"";
						...
						while(rs.next()){
							...
							ResultSet rs2 = s2.executeQuery("select id from mobile_order_assets where id="+UnedittedID);
							if(rs2.first()){
								IsBarCode=true;
							}
						...
						<tr style="font-size:10px;">
							...
							<td style="text-align:center;"><a href="#popupDialogReportCenterR276ProductRawCases" data-rel="popup" data-position-to="window" data-transition="pop" onclick="ReportCenterR276ProductRawCases(<%=UnedittedID%>)" ><%if(IsBarCode){ %>View</a> <%} %></td>																						
