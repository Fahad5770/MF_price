...
<head>
	...
	<script src="js/WorkflowManagerPerCaseDiscountRequest.js?1242121111=1242121111"></script>
  ...
  long RequestID = Utilities.parseLong(request.getParameter("requestID"));
   ...
   %>
    ...
    <jsp:include page="Header2.jsp" >
    	<jsp:param value="Product Per Case Discount Request" name="title"/>
    </jsp:include>
     <!-- /header -->
 <script>
        ...
        <fieldset class="ui-grid-a">
		        ...
		        <form name="WorkflowContainerForm" class="WorkflowForm" id="WorkflowContainerForm" method="POST" action="#">
					<ul data-role="listview" data-inset="true" style="margin-left: 22px;margin-top: 6px;margin-right: 5px;">
						...
						<li data-role="list-divider">Per Case Discount Product</li>
						<li>
						...
						<table border="0" style="width: 100%; margin-top: 0px; padding: 0px;" class="workflow_form_font">
							...
							<%
							ResultSet rs6 = s1.executeQuery("SELECT ipprp.id,ipprp.package_id,(select label from inventory_packages ip where ip.id=ipprp.package_id) package_labele_id,ipprp.type_id,ipprp.quantity,ipprp.percase_discount_rate,ipprp.amount,ipprp.serial_no FROM inventory_primary_percase_request_products ipprp join inventory_primary_percase_request ippr on ippr.id=ipprp.id where ippr.request_id="+RequestID+" and ipprp.type_id=1");
							...
							while(rs6.next()){
								...
								PerCaseDiscountRate=rs6.getDouble("percase_discount_rate");
								...
								ResultSet rs2 = s2.executeQuery("SELECT ipprp.id,ipprp.package_id,(select label from inventory_packages ip where ip.id=ipprp.package_id) package_label,ipprplt.lrb_type_id,(select label from inventory_products_lrb_types iplt where iplt.id=ipprplt.lrb_type_id) lrb_type_label,ipprp.type_id,ipprplt.serial_no FROM inventory_primary_percase_request_products ipprp join inventory_primary_percase_request_products_lrb_types ipprplt on ipprp.id=ipprplt.id and ipprp.serial_no=ipprplt.serial_no and ipprp.package_id=ipprplt.package_id and ipprp.type_id=ipprplt.type_id join inventory_primary_percase_request ippr on ippr.id=ipprp.id where ippr.request_id="+RequestID+" and ipprp.type_id=1 and ipprp.package_id="+rs6.getLong("package_id")+" and ipprp.serial_no="+MasterSerialID+" group by ipprp.type_id,ipprplt.lrb_type_id");
								...
								while(rs2.next()){
										BrandLabel += rs2.getString("lrb_type_label")+"<br/> ";
								...
								<tr>
									...
									<th  style="background-color:#efedee; padding:5px;font-size:12px;">LRB Type</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px;"></th>
								</tr>
								...
								<tr>	
									...
									<th  style="background-color:#efedee; padding:5px;font-size:12px; ">Per Case Distributor Rate</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px; ">Amount</th>
						...
						<table border="0" style="width: 100%; margin-top: 0px; padding: 0px;" class="workflow_form_font">
							...
							<tbody>
							<%
							ResultSet rs = s.executeQuery("select * from inventory_primary_percase_request where request_id="+RequestID);
							while(rs.next()){
							%>
							<tr>
						...
						<%
						ResultSet rs223 = s.executeQuery("SELECT cd.distributor_id,cd.name FROM inventory_primary_percase_request_distributors isprd join inventory_primary_percase_request ispr on isprd.product_promotion_id=ispr.id join common_distributors cd on cd.distributor_id=isprd.distributor_id where ispr.id="+PromoID);
						if(rs223.first()){
						...
						<table border="0" style="width: 100%; margin-top: 0px; padding: 0px;" class="workflow_form_font">
							...
							<tbody>
							<%
							ResultSet rs22 = s.executeQuery("SELECT cd.distributor_id,cd.name FROM inventory_primary_percase_request_distributors isprd join inventory_primary_percase_request ispr on isprd.product_promotion_id=ispr.id join common_distributors cd on cd.distributor_id=isprd.distributor_id where ispr.id="+PromoID);
							while(rs22.next()){
							%>
						...
						<table border="0" style="width: 100%; margin-top: 0px; padding: 0px;" class="workflow_form_font">
							...
							<tobdy>
						<%
						ResultSet rs41 = s.executeQuery("SELECT isdrd.distributor_id,(select name from common_distributors cd where cd.distributor_id=isdrd.distributor_id) name FROM inventory_primary_percase_request isdr join inventory_primary_percase_request_distributors isdrd on isdr.id=isdrd.product_promotion_id where request_id="+RequestID);
						while(rs41.next()){
						%>
