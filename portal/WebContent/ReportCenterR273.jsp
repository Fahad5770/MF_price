...
function EmptyCreditLimitProcessing(RequestID,CustomerID,ID){
	...
	$.ajax({
		...
		url: "cash/EmptyCreditLimitApproveExecute",
		data: {
...
function ReportCenterR273ProductRawCases(ParamDocumentID){
	
	$('#ReportCenterR273ProductRawCases').html( "<img src='images/snake-loader.gif' >" );
	
	$.ajax({
	    url: "ReportCenterR273ProductRawCases.jsp",
	    data: {
	    ...
	    success: function( html ) {
	    	//alert(html);
	    	$('#ReportCenterR273ProductRawCases').html( html ).trigger('create');
	    },
	    error: function( xhr, status ) {
	    	alert("Server could not be reached.");
	    }
			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					...
					<tbody>
					    ...
					    <tr style="font-size:11px;">							
							...
							<th data-priority="1"  style="text-align:center;  width:10%">Product</th>							
							...
							<th data-priority="1"  style="text-align:center;  width:10%">Credit Type</th>
						...
						String SQL="SELECT request_id,id,ececlr.distributor_id,(select name from common_distributors cd where ececlr.distributor_id=cd.distributor_id) distributor_name,credit_type,(select label from ec_empty_credit_types ecect where ecect.id=credit_type) credit_type_name,reason,start_date,end_date FROM pep.ec_empty_credit_limit_request ececlr where created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and is_active=1 and is_processed=0";
						//System.out.println(SQL);
						ResultSet rs = s.executeQuery(SQL);
						while(rs.next()){
						
							CLRequestID = rs.getLong("request_id");
							CLCustomerID = rs.getLong("distributor_id");
							ID = rs.getLong("id");
						...
						<tr>
							...
							<td><%=CLRequestID %></td>
							<td>
							<%=CLCustomerID %>-<%=rs.getString("distributor_name") %>
							</td>
							
							<td><a href="#popupDialogReportCenterR273ProductRawCases" data-rel="popup" data-position-to="window" data-transition="pop" onclick="ReportCenterR273ProductRawCases(<%=rs.getString("id")%>)" >View</a></td>																						
							<td><%=Utilities.getDisplayDateFormat(rs.getDate("start_date")) %>-<%=Utilities.getDisplayDateFormat(rs.getDate("end_date")) %></td>
							<td><%=rs.getString("reason") %></td>
							<td><%=rs.getString("credit_type_name") %></td>
						
							<td>
								<fieldset data-role="controlgroup" data-type="horizontal" data-mini="true" >								    
								    <input type="radio" name="PostingOptions" id="PostingOptionsID_<%=ID%>" value="1" onClick="EmptyCreditLimitProcessing('<%=CLRequestID %>','<%=CLCustomerID%>',<%=ID%>)">
