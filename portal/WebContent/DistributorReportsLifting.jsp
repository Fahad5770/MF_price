...
function redirect(DistributorID, SapOrderNo){
	...
	//alert(SapOrderNo);
	var url = 'DistributorReportsLiftingSummary.jsp?SapOrderNo='+SapOrderNo;
	/*if(OrderID == ''){
	...
	function OrderBookerGetOrderActivityReport(OrderID){
		...
		var url = 'DistributorReportsLiftingSummary.jsp?OrderID='+OrderID;
		if(OrderID == ''){
			url = 'DistributorReportsLiftingSummary.jsp';
		}
		$("#OrderActivityReportTD").html("<img src='images/snake-loader.gif'>");
			...
			<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="c" data-count-theme="c" style="margin-top:-13px;">
				...
				while(rs22.next()){
					...
					if(!DateUtils.isSameDay(LastDate, rs22.getDate("created_on")) ){
						...
						ResultSet rs3 = s2.executeQuery("select sum(invoice_amount) sub_total from inventory_delivery_note idn where idn.created_on between "+Utilities.getSQLDate(rs22.getDate("created_on"))+" and "+Utilities.getSQLDateNext(rs22.getDate("created_on"))+" and idn.distributor_id in ("+DistributorIDs+")");
	            		if(rs3.first()){
