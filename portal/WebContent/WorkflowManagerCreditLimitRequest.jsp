...
<head>
	...
	<script src="js/WorkflowManagerCreditLimitRequest.js?124211=124211"></script>
						...
						<table border="0" style="width: 100%; margin-top: 0px; padding: 0px;" class="workflow_form_font">
							...
							<thead>
								...
								<tr>
									...
									<th  style="background-color:#efedee; padding:5px;font-size:12px">Credit Limit</th>
							...
							ResultSet rs6 = s1.executeQuery("SELECT customer_id,(select name from common_distributors cd where cd.distributor_id=customer_id) customer_name,credit_limit,valid_to,comments,valid_from FROM gl_customer_credit_limit_request where request_id="+RequestID);
							while(rs6.next()){
								CustomerName = rs6.getLong("customer_id")+" - "+rs6.getString("customer_name");	
								CreditLimit = rs6.getDouble("credit_limit");
								ExpiryDateTo = rs6.getDate("valid_to");
								ExpiryDateFrom = rs6.getDate("valid_from");
								Comments = rs6.getString("comments");
							}
								
							%>
								<tr style="b1ackground-color:#efedee">
									<td style="width:25%; padding:5px;" valign="top"><%=CustomerName %></td>
									<td style="width:25%; padding:5px;"><%=Utilities.getDisplayCurrencyFormatRounded(CreditLimit) %></td>
