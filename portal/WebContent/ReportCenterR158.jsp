					...
					<tbody>
					    ...
					    <tr style="font-size:11px;">							
							...
							while(CurrentDate.compareTo(EndDate) <= 0){
								...
								<tr>
								<td><%=Utilities.getDisplayDateFormat(CurrentDate) %></td>
								<%
								...
								while(rs.next()){	
									...
									ResultSet rs1 = s2.executeQuery("SELECT glcr.receipt_type,glcr.created_on, glcr.created_on_date,glcr.created_by,glcr.warehouse_id,glcri.instrument_id, (select glci.label from gl_cash_instruments glci where glci.id=glcri.instrument_id) instrument_name, amount, sum(amount) total_amount"
											+" FROM gl_cash_receipts glcr join gl_cash_receipts_instruments glcri"+
												" on glcr.id=glcri.id where 1=1 "+WhereGlEmployee+" and glcri.instrument_id="+rs.getLong("id")+WhereWarehouse+" and created_on between "+Utilities.getSQLDateLifting(CurrentDate)+" and "+Utilities.getSQLDateNextLifting(CurrentDate)+" group by glcri.instrument_id ");
