		...
		<td style="width: 100%" valign="top">
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">					 
				...
				<tbody>
					...
					double FixedSamplingTotal=0;
					double PercaseSamplingTotal=0;
					...
					"	            sma.adjustment, "+
					"	            IFNULL(sma.net_payable, 0) - IFNULL(sma.adjustment*(-1), 0) - IFNULL((SELECT "+ 
					"	                    fixed_company_share "+
					"	                FROM "+
					"	                    sampling s "+
					"	                WHERE "+
					...
					"	    WHERE "+
					"	        sma.status_id = 1 "+
					"	            AND sma.status_on BETWEEN "+Utilities.getSQLDate(StartDate)+" AND "+Utilities.getSQLDateNext(EndDate)+" "+
					"	            AND net_payable != 0) tab) tab2 "+
					"	WHERE "+
					"	    net_amount != 0 order by distributor_id");
					
					
					while(rs.next()){
						
						 FixedSamplingTotal+=rs.getDouble("fixed_sampling");
						 PercaseSamplingTotal+=rs.getDouble("percase_sampling");
						 AdjustmentTotal+=rs.getDouble("adjustment");
						 NetAmountTotal+=rs.getDouble("net_amount");
						...
						<tr style="font-size:11px;">
							...
							<td style="text-align: left; "><%=rs.getString("outlet_id")+" - "+rs.getString("outlet_name")%></td>
							<td style="text-align: left; "><%=rs.getString("distributor_id")+" - "+rs.getString("distributor_name")%></td>
					...
					<tr style="font-size:11px;">
							...
							<td style="text-align: right"><%=Utilities.getDisplayCurrencyFormat(FixedSamplingTotal)%></td>
							<td style="text-align: right"><%=Utilities.getDisplayCurrencyFormat(PercaseSamplingTotal)%></td>
							<td style="text-align: right"><%=Utilities.getDisplayCurrencyFormat(AdjustmentTotal)%></td>
							<td style="text-align: right"><%=Utilities.getDisplayCurrencyFormat(NetAmountTotal)%></td>
						</tr>
