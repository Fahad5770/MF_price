	...
	<tr>
		...
		<td style="width: 100%" valign="top">
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					...
					double GrandTotal = 0;
					ResultSet rs = s.executeQuery("select cd.distributor_id, cd.name from common_distributors cd where 1=1 "+WhereHOD+WhereRSM+WhereSM+WhereTDM+WhereASM+WhereDistributors+" and distributor_id in (select distributor_id from inventory_sales_adjusted where created_on between "+Utilities.getSQLDate(d[0])+" and "+Utilities.getSQLDateNext(Utilities.getEndDateByDate(d[d.length-1]) )+" ) order by distributor_id");
					...
					%>
						<tr>
							...
							for (int i = 0; i < d.length; i++){
								...
								ResultSet rs2 = s2.executeQuery("select sum(isap.total_units * ipv.liquid_in_ml)/6000 from inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id = isap.id join inventory_products_view ipv on isap.product_id = ipv.product_id where distributor_id = "+DistributorID+" and created_on between "+Utilities.getSQLDate(iStartDate)+" and "+Utilities.getSQLDateNext(iEndDate)+"");
