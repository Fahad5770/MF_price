			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					...
					<tbody>
					    ...
					    <tr style="font-size:11px;">							
							...
							<th data-priority="1"  style="text-align:center;  width:30%">Deposited In</th>							
							...
							<th data-priority="1"  style="text-align:center;  width:10%">Reference Number</th>
						...
						<tr>
							...
							<input type="hidden" name="UniqueVoucherID" id="UniqueVoucherID_<%=ID %>" value="<%=i+""+Utilities.getUniqueVoucherIDShort(SessionUserID)%>"/>
							</td>
							...
							<td>
								<fieldset data-role="controlgroup" data-type="horizontal" data-mini="true" >								    
								    <input type="radio" name="PostingOptions" id="PostingOptionsID_<%=ID%>" value="1" onClick="PaymentPosting(<%=ID%>,<%=IstrumentID%>,'<%=rs.getString("instrument_no") %>',<%=WarehouseID%>,<%=AccountNumber%>,<%=rs.getDouble("amount")%>)">
								    <label for="PostingOptionsID_<%=ID%>">Cleared</label>
								    <input type="radio" name="PostingOptions" id="PostingOptionsIDD_<%=ID%>" value="2" <% if(rs.getInt("instrument_id")==1){ %>disabled<%} %> onClick="PaymentPosting(<%=ID%>,<%=IstrumentID%>,'<%=rs.getString("instrument_no") %>',<%=WarehouseID%>,<%=AccountNumber%>,<%=rs.getDouble("amount")%>)">
								    <label for="PostingOptionsIDD_<%=ID%>">Bounced</label>								    
								</fieldset>
							</td>
