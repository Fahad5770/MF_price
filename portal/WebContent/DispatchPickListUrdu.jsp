...
<%@page import="com.pbc.dictionary.Dictionary"%>
<style>
		...
		<div style="width: 8.27in; border: solid 1px; margin-top: 5px; page-break-after: always">
              ...
              <table border="0" style="width: 100%">
              	<tr>
                    ...
                    <td align="right" style="padding-right: 20px"><b>Pick List</b></td>
                </tr>
              </table>
			  				...
			  				<table cellspacing="0" cellpadding="0" align="left" border=0 style="width:100%; margin-bottom:10px; font-size: 12px"  data-role="table" data-mode="reflow" class="ui-responsive table-stroke">
									 <thead>
									    <tr>
											...
											<th data-priority="1"  style="text-align:right">Invoice</th>
											<th data-priority="1"  style="text-align:right">Extra</th>
											<th data-priority="1"  style="text-align:right">Total</th>
								...
								<%
									...
									long TotalBottles = 0;
									ResultSet rs3 = s.executeQuery("select ipv.package_id, ipv.package_label, ipv.unit_per_sku, sum( case when tl.type = 's' then tl.bottles else 0 end) invoice_qty, sum( case when tl.type = 'e' then tl.bottles else 0 end) extra_qty, sum(tl.bottles) total_qty  from ("+
											"SELECT isip.product_id, sum(isip.total_units) bottles, 's' type FROM inventory_sales_invoices_products isip where isip.id in (select sales_id from inventory_sales_dispatch_invoices where id = "+DispatchID+") group by isip.product_id union all select isdep.product_id, sum(isdep.total_units), 'e' from inventory_sales_dispatch_extra_products isdep where isdep.dispatch_id = "+DispatchID+" group by isdep.product_id"+
											") tl join inventory_products_view ipv on tl.product_id = ipv.product_id group by ipv.package_id order by ipv.package_sort_order");
									while(rs3.next()){
										
										TotalRawCases += Utilities.getRawCasesAndUnits(rs3.getLong("total_qty"), rs3.getInt("unit_per_sku"))[0];
										TotalBottles += Utilities.getRawCasesAndUnits(rs3.getLong("total_qty"), rs3.getInt("unit_per_sku"))[1];
										
										%>
										<tr style="background:#ececec">
					   	            		<td align="left"><%=rs3.getString("package_label")%></td>
					   	            		<td style="text-align:right; "><b><%=Utilities.convertToRawCases(rs3.getLong("invoice_qty"), rs3.getInt("unit_per_sku"))%></b></td>
					   	            		<td style="text-align:right; "><b><%if (rs3.getLong("extra_qty") != 0){%><%=Utilities.convertToRawCases(rs3.getLong("extra_qty"), rs3.getInt("unit_per_sku"))%><%} %></b></td>
					   	            		<td style="text-align:right; "><b><%=Utilities.convertToRawCases(rs3.getLong("total_qty"), rs3.getInt("unit_per_sku"))%></b></td>
					   	            	</tr>
					   	            	
										<%
										
										ResultSet rs4 = s2.executeQuery("select ipv.brand_id, ipv.brand_label, ipv.unit_per_sku, sum( case when tl.type = 's' then tl.bottles else 0 end) invoice_qty, sum( case when tl.type = 'e' then tl.bottles else 0 end) extra_qty, sum(tl.bottles) total_qty  from ("+
												"SELECT isip.product_id, sum(isip.total_units) bottles, 's' type FROM inventory_sales_invoices_products isip where isip.id in (select sales_id from inventory_sales_dispatch_invoices where id="+DispatchID+") group by isip.product_id union all select isdep.product_id, sum(isdep.total_units), 'e' from inventory_sales_dispatch_extra_products isdep where isdep.dispatch_id = "+DispatchID+" group by isdep.product_id"+
												") tl join inventory_products_view ipv on tl.product_id = ipv.product_id and ipv.package_id = "+rs3.getInt("package_id")+" group by ipv.brand_id order by ipv.package_sort_order");
										
										while(rs4.next()){
											%>
											
											<tr>
						    					<td style="padding-left:20px; border-bottom:1px solid #000;"><%=rs4.getString("brand_label")%></td>
						    					<td style="text-align:right; border-bottom:1px solid #000;"><%=Utilities.convertToRawCases( rs4.getLong("invoice_qty"), rs4.getInt("unit_per_sku"))%></td>
						    					<td style="text-align:right; border-bottom:1px solid #000;"><%if (rs4.getLong("extra_qty") != 0){%><%=Utilities.convertToRawCases( rs4.getLong("extra_qty"), rs4.getInt("unit_per_sku"))%><%} %></td>
						    					<td style="text-align:right; border-bottom:1px solid #000;"><%=Utilities.convertToRawCases( rs4.getLong("total_qty"), rs4.getInt("unit_per_sku"))%></td>
						    		    	</tr>
									...
									<tr style="background:#ececec">
				   	            		...
				   	            		<td style="text-align:right; padding-right:10px"><b></b></td>
				   	            		<td style="text-align:right; padding-right:10px"><b></b></td>
				   	            		<td style="text-align:right;"><b><%=Utilities.formatCasesAndUnits(TotalRawCases, TotalBottles)%></b></td>
				   	            	</tr>
			  	...
			  	<table align="center" style="width: 95%; font-size:12px; margin-bottom:10px" border="0" cellspacing="0">
			  		...
			  		<thead>
			  			<tr>
			  				...
			  				<th align="left" nowrap="nowrap" style="width: 0%; padding-left:5px">Amount</th>
		  ...
		  while (rs.next()){
			  ...
			  %>
		  			...
		  			<tr style="min-height: 50px">
		  				...
		  				<td valign="bottom" style="padding-left:5px; border-bottom: 1px solid #ccc; direction:rtl; font-size: 11pt; font-family: Tahoma;"><%=""+OutletID+" - "+ dic.translateToUrdu(OutletName) %></td>
		  				<td valign="bottom" style="padding-left:5px; border-bottom: 1px solid #ccc; direction:rtl; font-size: 11pt; font-family: Tahoma;"><%=dic.translateToUrdu(OutletAddress) %></td>
		  				<td valign="bottom" style="padding-left:5px; border-bottom: 1px solid #ccc; direction:rtl; font-size: 11pt; font-family: Tahoma;"><%=dic.translateToUrdu(Owner) %></td>
