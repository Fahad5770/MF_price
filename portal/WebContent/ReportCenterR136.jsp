...
if(session.getAttribute(UniqueSessionID+"_SR1SelectedComplaintID") != null){
	ComplaintID = session.getAttribute(UniqueSessionID+"_SR1SelectedComplaintID").toString();
}
...
<script type="text/javascript">
	...
	function OpenComplaintForm(ComplaintID, IsVerified){
		
		//alert("OrderBookerGetOrderActivityReport Function");
		var url = 'ReportCenterR136ComplaintOverviewForm.jsp?ComplaintID='+ComplaintID+'&IsVerified='+IsVerified+'&UniqueSessionID=<%=UniqueSessionID%>';
	...
	function PoplulateComplaints(IsVerified){
		/*
		...
		$.mobile.showPageLoadingMsg();
		$.get("CRMComplaintVerificationList.jsp?IsVerified="+IsVerified+'&UniqueSessionID=<%=UniqueSessionID%>', function(data) {
			  $.mobile.hidePageLoadingMsg();
	...
	<% if( !ComplaintID.equals("") && !ComplaintID.equals("0") ){ %>
	
	OpenComplaintForm(<%=ComplaintID%>);
