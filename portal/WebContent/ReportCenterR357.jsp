...
function ExportToExcel(D1,D2,DistributorIDs,BrandIDs,OrderBookersIDs,SKUIDs,ASMIDs,TSOIDs,RegionIDs,CityIDs, ChannelIDs){
	//alert(D1);
	$.mobile.showPageLoadingMsg();
	$.ajax({
		
		url: "reports/R357ExcelMain",
		data: {
			StartDate: D1,
			EndDate: D2,
...
<ul data-role="listview" data-inset="true"
	...
	data-icon="false">
	<li data-role="list-divider" data-theme="a">Order Distance Report</li>
	<li>
							...
							<tr>
								<td style="text-align: center;width:75%;">
   									...
   									<div id="aExcelFileReady">
   										<a onclick="ExportToExcel(<%=Utilities.getSQLDate(StartDate) %>, <%=Utilities.getSQLDate(EndDate) %>, '<%=DistributorIDs %>',
   										'<%=BrandIDs %>','<%=OrderBookersIDs %>','<%=SKUIDs %>','<%=ASMIDs %>','<%=TSOIDs %>','<%=RegionIDs %>',
   										'<%=CityIDs %>' , '<%=ChannelIDs %>'
   										)"
   										
   										 href="#"    data-theme="c" data-inline="true" style="margin-top: 0px; margin-left: 0px; margin-bottom: 0px">Export To Excel</a>
   								</div>
