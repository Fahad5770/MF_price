...
<script type="text/javascript">
	...
	if(PjpID == "x125"){
		var url = 'ReportCenterR378NewOutletSummary.jsp?PJPID=x125&UniqueSessionID=<%=UniqueSessionID%>';
	}	
	
	var url = 'ReportCenterR378NewOutletSummary.jsp?CensusID='+PjpID+'&UniqueSessionID=<%=UniqueSessionID%>';
	...
	<tr>
		...
		<td style="width: 30%" valign="top">
			...
			<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="c" data-count-theme="c" style="margin-top:-13px;">
				...
				System.out.println("SELECT * FROM rental_discount where created_on BETWEEN "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and  approved_1=0 and approved_2=0 and is_declined = 0 order by id desc");
				ResultSet rs22 = s.executeQuery("SELECT * FROM rental_discount where created_on BETWEEN "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and  approved_1=0 and approved_2=0 and is_declined = 0 order by id desc");
				...
				while(rs22.next()){
					...
					OutletName = rs22.getString("outlet_name");
					
					%>
					<li style="height: 35px" >
						<a data-ajax="false" id="Census_<%=rs22.getString("id")%>" href="javascript: LoadCensusDetailReport(<%=rs22.getString("outlet_id")%>)" style="font-size:11px; font-weight:normal;"><%=OutletName%></a>
					</li>
					<%
					TotalOutlets++;
					}				
