...
function DispatchReturnsAddProduct(){	
	...
	if($('#EmptyCreditReceiptType15').val() == "" || $('#EmptyCreditReceiptType15').val() == "0"){
		document.getElementById('EmptyCreditReceiptType15').focus();
		return false;
	}else{
		var EmptyCreditReceiptType =  $('#EmptyCreditReceiptType15').val();
		var EmptyCreditReceiptTypeText =  $('#EmptyCreditReceiptType15 option:selected').text();
	}
	
	if($("#EmptyCreditReceiptBrandIDTempStorage").val()!="24" && $("#EmptyCreditReceiptBrandIDTempStorage").val()!="25" && $("#EmptyCreditReceiptBrandIDTempStorage").val()!="26" && $("#EmptyCreditReceiptBrandIDTempStorage").val()!="27" && $("#EmptyCreditReceiptBrandIDTempStorage").val()!="28" && $("#EmptyCreditReceiptBrandIDTempStorage").val()!="29"){
		if($('#EmptyCreditReceiptType15').val()=="5"){
			alert("You can't select other brands for this option");
			return false;
		}
	}
		    ...
		    success: function( json ) {
		    	...
		    	if (json.exists == "true"){
		    		...
		    		if (isAlreadyEntered != true){
		    			...
		    			//Other Brand Check
		    			var EmptyCreditReceiptTypeModified="";
		    			
		    			if(json.IsOtherBrand==1){ //if other brand then select type other brand
		    				EmptyCreditReceiptTypeModified = 5; //5 is for other brand		    				
		    				EmptyCreditReceiptTypeText="Other Brand"
		    				//$("#EmptyCreditReceiptType15").selectmenu("disable");
		    				//$('#EmptyCreditReceiptType15').selectmenu('refresh');		    			
		    			}else{
		    				EmptyCreditReceiptTypeModified = EmptyCreditReceiptType;		    				
			    			//$("#EmptyCreditReceiptType15").selectmenu("enable");
		    			}
		    			...
		    			var AddedProductTypeArray = new Array();
		    			
		    			
		    			AddedProductArray = document.getElementsByName("DispatchReturnsMainFormProductName");
		    			AddedProductTypeArray = document.getElementsByName("EmptyCreditReceiptType");
		    			//alert(AddedProductTypeArray[0].value);
		    			
		    			
		    			
		    			for(var i=0;i<AddedProductArray.length;i++)
		    			{
		    				if(AddedProductArray[i].value==$('#DispatchReturnsProductCode').val() && AddedProductTypeArray[i].value==EmptyCreditReceiptTypeModified)
		    				{
		    			...
		    			if(!IsAlreadyProductAdded)
	    				{
							...
							"<tr id='DeskSale"+RowMaxID+"'>"+
								...
								"<td>"+EmptyCreditReceiptTypeText+"<input type='hidden' name='EmptyCreditReceiptType' value='"+EmptyCreditReceiptTypeModified+"'></td>"+
...
function ProductionReceiptSubmit()
{
	...
	if($("#DispatchReturnsVehicleNumber").val() !=""){
		...
		$.mobile.loading( 'show');
		$.ajax({
		    url: "empty/EmptyCreditReceiptExecute",
		    ...
		    success: function( json ) {
		    	...
		    	if(json.success=="true")	    		
	    		{
		    		...
		    		alert("Saved");
		    		location = "EmptyCreditReceipt.jsp?PrintID="+json.MasterTableID;
...
function LoadProductCode(ProductCode,BrandID){
	
	$("#DispatchReturnsProductCode").val(ProductCode);
	document.getElementById('DispatchReturnsRawCases').focus();
	
	$("#EmptyCreditReceiptBrandIDTempStorage").val(BrandID);
	
	
	if(BrandID==24 || BrandID==25 || BrandID==26 || BrandID==27 || BrandID==28 || BrandID==29){
		$("#EmptyCreditReceiptType15").val(5);
		$("#EmptyCreditReceiptType15").selectmenu("disable");
		$('#EmptyCreditReceiptType15').selectmenu('refresh');
	}else{
		$("#EmptyCreditReceiptType15").selectmenu("enable");
...
function LoadEditCase(ReceiptProductionID){
...
function doPrint(CashReceiptID){
	...
	$('#EmptyCreditReceiptID').val( CashReceiptID );
	$('#EmptyCreditPrintExecute').submit();
	
}

function doPrintManual(){
	if($("#ReceiptIDForPrint").val()==""){
		document.getElementById('ReceiptIDForPrint').focus();
	}else{
		$('#EmptyCreditReceiptID').val( $("#ReceiptIDForPrint").val() );
		$('#EmptyCreditPrintExecute').submit();
	}
