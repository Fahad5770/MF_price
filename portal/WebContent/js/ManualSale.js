...
function DeskSaleSubmit(){
	...
	var dateReg = /^(0?[1-9]|[12][0-9]|3[01])[\/\-](0?[1-9]|1[012])[\/\-]\d{4}$/;  //for DD/MM/YYY
	
	if(!$("#dated").val().match(dateReg)){
	...
	$.ajax({
	    ...
	    success: function( json ) {
	    	if (json.success == "true"){
	    		location = "ManualSale.jsp?DeskSaleIDPrint="+json.DeskSaleID;
	    	}else{
	    		$('#DeskSaleSave').removeClass('ui-disabled');
				alert(json.error);
	    	}
...
function showSearchContent(){
	...
	$.get('ManualSaleSearch.jsp?FromDate='+$('#DeskSaleFromDate').val()+'&ToDate='+$('#DeskSaleToDate').val(), function(data) {
...
function getDeskSaleInfoJson(EditID){
	...
	if (EditID != ""){
		$.ajax({
		    url: "inventory/GetManualSaleInfoJson",
		    data: {
		    	EditID: EditID
		    },
		    type: "POST",
