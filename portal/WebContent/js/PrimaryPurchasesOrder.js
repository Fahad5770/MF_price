		...
		$.ajax({
		    ...
		    success: function( json ) {
		    	if (json.exists == "true"){
		    		...
		    		setTimeout("$('#DeskSaleRawCases').focus(); $('#DeskSaleRawCases').removeAttr('readonly');", 100);
		    	}else{
...
function DeskSaleAddProduct(){
	...
	if( $('#DeskSalePlantID').val() == '' ){
		alert('Please select Plant before adding products');
		document.getElementById('"DeskSalePlantID"').focus();
		return false;
	}else{
		var value = $('#DeskSalePlantID').val();
	...
	$("#DeskSaleDistributorIDHidden").val(0);
	$("#DeskSalePlantIDHidden").val($("#DeskSalePlantID").val());
	...
	if (val != "" && val.length > 0){
		...
		$.ajax({ 
		    ...
		    data: {
		    	ProductCode: val,
		    	DistributorID: $("#DeskSaleDistributorID").val(),
		    ...
		    success: function( json ) {
		    	...
		    	if (json.exists == "true"){
		    		...
		    		/*if(json.StockStatus == '1'){
		    			alert('No Stock Available');
		    			document.getElementById('DeskSaleRawCases').focus();
		    			return false;
		    		}else if(json.StockStatus == '2'){
		    			alert('Quantity should not exceed '+ json.AvailableStockInRawCases);
		    			document.getElementById('DeskSaleRawCases').focus();
		    			return false;
		    		}
		    		*/
		    		...
		    		if (isAlreadyEntered != true){
		    			...
		    			var PSRate=$('#DeskSaleRate').val();
		    			var PSUnitRate = $('#DeskSaleUnitRate').val();
		    			var PSAmount= $('#DeskSaleAmount').val();
		    			var IsPromo=0;
						...
						"<tr id='DeskSale"+RowMaxID+"'>"+
							...
							"<td>"+ValDeskSaleRawCases+"<input type='hidden' name='DeskSaleMainFormRawCases' value='"+ValDeskSaleRawCases+"'><input type='hidden' name='PSIspromo' value='"+IsPromo+"'/></td>"+
							"<td>"+ValDeskSaleUnits+"<input type='hidden' name='DeskSaleMainFormUnits' value='"+ValDeskSaleUnits+"'></td>"+
							"<td>"+formatInCommas(PSRate)+"<input type='hidden' name='DeskSaleMainFormRateHidden' value='"+PSRate+"'><input type='hidden' name='DeskSaleMainFormUnitRateHidden' value='"+PSUnitRate+"'></td>"+
							"<td>"+formatInCommas(PSAmount)+"<input type='hidden' id='DeskSaleMainFormAmount' name='DeskSaleMainFormAmount' value='"+PSAmount+"'></td>"+
...
function updateInvoiceSummary(){
	...
	$('#DeskSaleMainFormTotalInvoiceAmountField').val(parseFloat(TotalAmount).toFixed(2));
...
function DeskSaleSubmit(){
	...
	$('#DeskSaleSave').addClass("ui-disabled");
	$.ajax({
	    url: "primarysales/PrimaryPurchasesExecute",
	    ...
	    success: function( json ) {
	    	if (json.success == "true"){
	    		location = "PrimaryPurchasesOrder.jsp";
	    	}else{
	    		$('#DeskSaleSave').removeClass('ui-disabled');
				alert(json.error);
	    	}
...
function getProductInfoJson(ProductID){
	...
	if( $('#DeskSaleDistributorID').val() == '' ){
		//alert('Please select Outlet before adding products');
		document.getElementById("DeskSaleDistributorID").focus();
		return false;
	}else{
		var value = $('#DeskSaleDistributorID').val();
		if(isInteger(value) == false){
			document.getElementById('DeskSaleDistributorID').focus();
			return false;
		}
...
function ProductSearchCallBack(SAPCode, ProductName){
	...
	setTimeout("$('#DeskSaleRawCases').focus(); $('#DeskSaleRawCases').removeAttr('readonly'); getProductInfoJson("+SAPCode+")", 100);
}
...
function getDistributorName12(){
	//alert();
	if(isInteger($('#DeskSaleDistributorID').val()) == false ){
		$('#DeskSaleDistributorID').val('');
		return false;
	}
	...
	$.ajax({
		...
		success:function(json){
			...
			if(json.exists == "true"){
				$('#DeskSaleDistributorName').val(json.DistributorName);
				$('#DeskSaleRegionIDHidden').val(json.RegionID);
				//getOutletMasterInfo();
			}else{
				$.mobile.hidePageLoadingMsg();
				$('#DeskSaleDistributorName').val('Distributor not found');
...
function getPlantName12(){
	//alert();
	if(isInteger($('#DeskSalePlantID').val()) == false ){
		$('#DeskSalePlantID').val('');
		return false;
	}
	...
	$.ajax({
		...
		url: "common/GetDistributorInfoJson",
		data: {
			DistributorID: $('#DeskSalePlantID').val(),
			FeatureID:273
		},
		type:"POST",
		dataType:"json",
		success:function(json){
			...
			if(json.exists == "true"){
				$('#DeskSalePlantName').val(json.DistributorName);
				
			}else{
				$.mobile.hidePageLoadingMsg();
				$('#DeskSalePlantName').val('Plant not found');
...
function DeskSaleAddProductPromo(){
	...
	if( ( $('#DeskSaleRawCasesPromo').val() == "" || $('#DeskSaleRawCasesPromo').val() == "0" ) && ( $('#DeskSaleUnitsPromo').val() == "" || $('#DeskSaleUnitsPromo').val() == "0" ) ){
		document.getElementById('DeskSaleRawCasesPromo').focus();
		return false;
	}else{
		var value_raw_cases = $('#DeskSaleRawCasesPromo').val();
		if(value_raw_cases!= "" && isInteger(value_raw_cases) == false){
			document.getElementById('DeskSaleRawCasesPromo').focus();
			return false;
		}
		
		var value_units = $('#DeskSaleUnitsPromo').val();
		if(value_units != "" && isInteger(value_units) == false){
			document.getElementById('DeskSaleUnitsPromo').focus();
			return false;
		}
	...
	var value = $('#DeskSaleDiscountPromo').val();
	if(isInteger(value) == false){
		document.getElementById('DeskSaleDiscountPromo').focus();
		return false;
	}
	
	
	var ValProductCode = $('#DeskSaleProductCodePromo').val();
	if( $('#DeskSaleBatchCodePromo').val() == "" && !( ValProductCode == '5012' || ValProductCode == '5014' ) ){
		document.getElementById('DeskSaleBatchCodePromo').focus();
		return false;
	}
	...
	var val = $('#DeskSaleProductCodePromo').val();
	...
	if (val != "" && val.length > 0){
		...
		$.ajax({ 
		    ...
		    data: {
		    	ProductCode: val,
		    	DistributorID: $("#DeskSalePlantID").val(),
		    	RawCases: $('#DeskSaleRawCasesPromo').val(),
		    	Units: $('#DeskSaleUnitsPromo').val()
		    },
		    type: "POST",
		    dataType : "json",
		    success: function( json ) {
		    	...
		    	if (json.exists == "true"){
		    		...
		    		*/
		    		var ValDeskSaleRawCases = $('#DeskSaleRawCasesPromo').val();
	    			if( ValDeskSaleRawCases == '' ){
	    				ValDeskSaleRawCases = '0';
	    			}
		    		...
		    		if (isAlreadyEntered != true){
		    			...
		    			var ValDeskSaleUnits = $('#DeskSaleUnitsPromo').val();
		    			if( ValDeskSaleUnits == '' ){
		    				ValDeskSaleUnits = '0';
		    			}
		    			...
		    			if( $('#DeskSaleDiscountPromo').val() == '' ){
		    				$('#DeskSaleDiscountPromo').val('0');
		    			}
		    			
		    			var RowMaxID = parseInt($('#RowMaxID').val()) + 1;
		    			
				    	var content = ""+
						"<tr id='PromoDeskSale"+RowMaxID+"'>"+
							"<td>"+$('#DeskSaleProductCodePromo').val()+"<input type='hidden' name='DeskSaleMainFormUnitPerSKUPromo' value='"+json.UnitPerSKU+"'></td>"+	
							"<td>"+json.PackageLabel+"<input type='hidden' name='DeskSaleMainFormPackageLabelPromo' value='"+json.PackageLabel+"' ><input type='hidden' name='PackageIDPromo' value='"+json.PackageID+"' ><input type='hidden' name='DeskSaleMainFormLiquidInMLPromo' value='"+json.LiquidInML+"'></td>"+						
							"<td>"+json.BrandLabel+"<input type='hidden' name='DeskSaleMainFormBrandLabelPromo' value='"+json.BrandLabel+"' ><input type='hidden' name='BrandIDPromo' value='"+json.BrandID+"' ><input type='hidden' id='ProductCodePromo' name='ProductCodePromo' value='"+val+"'><input type='hidden' name='ProductIDPromo' value='"+json.ProductID+"'></td>"+
							"<td>"+ValDeskSaleRawCases+"<input type='hidden' name='DeskSaleMainFormRawCasesPromo' value='"+ValDeskSaleRawCases+"'></td>"+
							"<td>"+ValDeskSaleUnits+"<input type='hidden' name='DeskSaleMainFormUnitsPromo' value='"+ValDeskSaleUnits+"'></td>"+
							
							"<td>"+$('#DeskSaleDiscountPromo').val()+"<input type='hidden' name='DeskSaleMainFormDiscountPromo' value='"+$('#DeskSaleDiscountPromo').val()+"'></td>"+
							"<td>"+formatInCommas($('#DeskSaleNetAmountPromo').val())+"<input type='hidden' id='DeskSaleMainFormNetAmountPromo' name='DeskSaleMainFormNetAmountPromo' value='"+$('#DeskSaleNetAmountPromo').val()+"'></td>"+
							"<td><a data-role='button' data-mini='true' data-icon='delete' data-iconpos='notext' data-inline='true' onClick=\"DeskSaleDeleteRow('PromoDeskSale"+RowMaxID+"');\">Delete</a></td>"+
						"</tr>";
				    	
						$("#DeskSaleTableBodyPromo").append(content).trigger('create');
						
						RowCount++;
						
						$('#NoProductRowPromo').css('display', 'none');
						
						$('#DeskSaleProductCodePromo').val('');
						$('#DeskSaleRawCasesPromo').val('');
						$('#DeskSaleUnitsPromo').val('');
						$('#DeskSaleBatchCodePromo').val('');
						
						$('#DeskSaleDiscountPromo').val('');
						$('#DeskSaleNetAmountPromo').val('');
						...
						$('#DeskSaleProductCodePromo').focus();
...
function IsPromoCheck(){
	if ($('input.checkbox_check').is(':checked')) {
		$("#DeskSaleRate").attr('readonly', true);
		$("#DeskSaleAmount").attr('readonly', true);
	}else{
		$("#DeskSaleRate").attr('readonly', false);
		$("#DeskSaleAmount").attr('readonly', false);
	}
}
...
$.mobile.showPageLoadingMsg();
	...
	$.ajax({
		...
		url: "primarysales/GetPriceListJson",
		data: {
			DistributorID: $("#DeskSaleDistributorID").val(),
			ProductCode:$("#DeskSaleProductCode").val()
		},
		type:"POST",
		dataType:"json",
		success:function(json){
