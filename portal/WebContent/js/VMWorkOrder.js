...
function AddSalesVolRows(){
	...
	$('#VMInHouseRow input').each(function(){ 
		var NewInsertedListID = $("#VMInHouseAct option:selected").val();    		
		//alert($(this).val());
	...
	if(flag){
		...
		var RowCount = $("#VMHouseActRowCount").val(); //getting row count value
		
		var content = ""+
		"<tr id='SalesVolDynamicTableRows_"+RowCount+"'>"+
			"<td>"+$("#VMInHouseAct option:selected").text()+
			"<input type='hidden' name='SelectVMInHouseAct' value='"+$("#VMInHouseAct").val()+"' /></td>"+			
			
			
			
			"<td><a data-role='button' data-mini='true' data-icon='delete' data-iconpos='notext' data-inline='true' onClick=\"InHouseDeleteRow('SalesVolDynamicTableRows_"+RowCount+"')\">Delete</a></td>"+
		"</tr>";
		
		$("#NoProductRow").html("");
		$("#VMInHouseRow").append(content).trigger('create');
		$("#VMHouseActRowCount").val(parseInt(RowCount)+1); //adding row count value and saving it
		}else{
			alert("Already Added");
		}
	}else{
...
function InHouseDeleteRow(RowID){
	$("#"+RowID).remove();
	RowCount--;
	if( RowCount < 1){
		$('#NoProductRow').css('display', 'table-row');
...
function AddOutsourcingRows(){
	...
	if($("#SelectOutSourceType").val()==""){
		flag=false;
	}
	
	$('#VMOutSourceRow input').each(function(){ 
		var NewInsertedListID = $("#SelectOutSourceType option:selected").val();    		
		//alert($(this).val());
		if($(this).val().trim() == NewInsertedListID.trim())
		{
			flag1 = false;
	...
	if(flag){
		...
		var RowCount = $("#VMOutsourceRowCount").val(); //getting row count value
		
		var content = ""+
		"<tr id='SalesVolDynamicTableRows_"+RowCount+"ix'>"+
			"<td>"+$("#SelectOutSourceType option:selected").text()+
			"<input type='hidden' name='SelectVMOutsourceAct' value='"+$("#SelectOutSourceType").val()+"' /></td>"+			
			
			
			
			"<td><a data-role='button' data-mini='true' data-icon='delete' data-iconpos='notext' data-inline='true' onClick=\"OutSourceDeleteRow('SalesVolDynamicTableRows_"+RowCount+"ix')\">Delete</a></td>"+
		"</tr>";
		
		$("#NoProductRow1").html("");
		$("#VMOutSourceRow").append(content).trigger('create');
		$("#VMOutsourceRowCount").val(parseInt(RowCount)+1); //adding row count value and saving it
		}else{
			alert("Already Added");
		}
	}else{
...
function OutSourceDeleteRow(RowID){
	$("#"+RowID).remove();
	RowCount--;
	if( RowCount < 1){
		$('#NoProductRow1').css('display', 'table-row');
...
function VMWorkOrderSave()
{
	...
	$.ajax({    		
			url: "vm/VMWorkOrderExecute",    			
			    data: $("#VMWorkOrderMainForm" ).serialize(),
			    type: "POST",
			    dataType : "json",
			success:function(json){
				if(json.success == "true"){    				
					location = "VMWorkOrder.jsp";
...
function CalculateTotalCost(){
	...
	var OutsourceCost = 0;
	 ...
	 InHouseCost = $("#InHouseCost").val();
	 OutsourceCost = $("#OutSourceCost").val();
	...
	if(OutsourceCost==""){
	 ...
	 var total = parseInt(InHouseCost)+parseInt(OutsourceCost);
	
	$("#TotalCost").val(parseInt(total));
}

function getEmployeeNameRequestedTo(){
	//alert();
	if( $('#VMRequestTo').val() == "" ){
		return false;
	}
	if( isInteger($('#VMRequestTo').val()) == false ){
		setTimeout("$('#VMRequestTo').focus();", 100);
		return false;
	}
	...
	$.ajax({
		url: "employee/GetEmployeeBySAPCodeJSON",
		data: {
			SAPCode: $('#VMRequestTo').val()
		},
		type:"POST",
		dataType: "json",
		success: function(json){ 
			if( json.exists == 'true' ){
				$('#VMRequestToName').val(json.EmployeeName);
			}
		},
		error: function(xhr, status){
			alert("Server could not be reached.");
...
function getEmployeeNameRequestedBy(){
	//alert();
	if( $('#VMRequestBy').val() == "" ){
		return false;
	}
	if( isInteger($('#VMRequestBy').val()) == false ){
		setTimeout("$('#VMRequestBy').focus();", 100);
		return false;
	}
	...
	$.ajax({
		url: "employee/GetEmployeeBySAPCodeJSON",
		data: {
			SAPCode: $('#VMRequestBy').val()
		},
		type:"POST",
		dataType: "json",
		success: function(json){ 
			if( json.exists == 'true' ){
				$('#VMRequestByName').val(json.EmployeeName);
			}
		},
		error: function(xhr, status){
			alert("Server could not be reached.");
...
function getEmployeeNameInspectedBy(){
	//alert();
	if( $('#VMInspectedBy').val() == "" ){
		return false;
	}
	if( isInteger($('#VMInspectedBy').val()) == false ){
		setTimeout("$('#VMInspectedBy').focus();", 100);
		return false;
	}
	...
	$.ajax({
		url: "employee/GetEmployeeBySAPCodeJSON",
		data: {
			SAPCode: $('#VMInspectedBy').val()
		},
		type:"POST",
		dataType: "json",
		success: function(json){ 
			if( json.exists == 'true' ){
				$('#VMInspectedByName').val(json.EmployeeName);
			}
		},
		error: function(xhr, status){
			alert("Server could not be reached.");
...
function getEmployeeNameCustodian(){
	//alert();
	if( $('#VMCustodian').val() == "" ){
		return false;
	}
	if( isInteger($('#VMCustodian').val()) == false ){
		setTimeout("$('#VMCustodian').focus();", 100);
		return false;
	}
	...
	$.ajax({
		url: "employee/GetEmployeeBySAPCodeJSON",
		data: {
			SAPCode: $('#VMCustodian').val()
		},
		type:"POST",
		dataType: "json",
		success: function(json){ 
			if( json.exists == 'true' ){
				$('#VMCustodianName').val(json.EmployeeName);
			}
		},
		error: function(xhr, status){
			alert("Server could not be reached.");
...
function getEmployeeNameAssignedBy(){
	//alert();
	if( $('#VMJobAssignedBy').val() == "" ){
		return false;
	}
	if( isInteger($('#VMJobAssignedBy').val()) == false ){
		setTimeout("$('#VMJobAssignedBy').focus();", 100);
		return false;
	}
	...
	$.ajax({
		url: "employee/GetEmployeeBySAPCodeJSON",
		data: {
			SAPCode: $('#VMJobAssignedBy').val()
		},
		type:"POST",
		dataType: "json",
		success: function(json){ 
			if( json.exists == 'true' ){
				$('#VMJobAssignedByName').val(json.EmployeeName);
			}
		},
		error: function(xhr, status){
			alert("Server could not be reached.");
...
function getEmployeeNameAssignedTo(){
	//alert();
	if( $('#VMJobAssignedTo').val() == "" ){
		return false;
	}
	if( isInteger($('#VMJobAssignedTo').val()) == false ){
		setTimeout("$('#VMJobAssignedTo').focus();", 100);
		return false;
	}
	...
	$.ajax({
		url: "employee/GetEmployeeBySAPCodeJSON",
		data: {
			SAPCode: $('#VMJobAssignedTo').val()
		},
		type:"POST",
		dataType: "json",
		success: function(json){ 
			if( json.exists == 'true' ){
				$('#VMJobAssignedToName').val(json.EmployeeName);
			}
		},
		error: function(xhr, status){
			alert("Server could not be reached.");
...
function getEmployeeNameConfirmedBy(){
	//alert();
	if( $('#VMJobConfirmedBy').val() == "" ){
		return false;
	}
	if( isInteger($('#VMJobConfirmedBy').val()) == false ){
		setTimeout("$('#VMJobConfirmedBy').focus();", 100);
		return false;
	}
	...
	$.ajax({
		url: "employee/GetEmployeeBySAPCodeJSON",
		data: {
			SAPCode: $('#VMJobConfirmedBy').val()
		},
		type:"POST",
		dataType: "json",
		success: function(json){ 
			if( json.exists == 'true' ){
				$('#VMJobConfirmedByName').val(json.EmployeeName);
			}
		},
		error: function(xhr, status){
			alert("Server could not be reached.");
