...
function ProductionReceiptSubmit()
{
	...
	if($("#DispatchReturnsVehicleNumber").val() !=""){
		...
		if( $('#SerialNo').val()!= "" && isInteger( $('#SerialNo').val() ) == false ){
			alert('Serial No must be an integer');
			document.getElementById("SerialNo").focus();
			return false;
		}
		...
		$.mobile.loading( 'show');
		$.ajax({
		    url: "inventory/ReceiptFromProductionExecute",
		    ...
		    success: function( json ) {
		    	...
		    	if(json.success=="true")	    		
	    		{
		    		...
		    		location = "ReceiptFromProduction.jsp?ID="+json.MasterTableID;
...
function showSearchContent(){
	...
	$.get('ReceiptFromProductionSearch.jsp?FromDate='+$('#ReceiptFromProductionFromDate').val()+'&ToDate='+$('#ReceiptFromProductionToDate').val(), function(data) {
