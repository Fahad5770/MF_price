...
$( document ).delegate("#DeskSale", "pageinit", function() {
	...
	DeskSaleExtraLoadReturnSummary();
	DeskSaleExtraLoadReconciliationSummary();
	
	
});

function DeskSaleExtraLoadReturnSummary(){
	$.mobile.showPageLoadingMsg();
	$.get('DeskSaleExtraLoadSummary.jsp?dispatchid='+DispatchID, function(data) {
		
		$.mobile.hidePageLoadingMsg();
		
		  $("#DeskSaleExtraLoadReturnSummary").html(data);
		  $("#DeskSaleExtraLoadReturnSummary").trigger('create');
		 
	});
}

function DeskSaleExtraLoadReconciliationSummary(){
	$.mobile.showPageLoadingMsg();
	$.get('DispatchAdjustmentReconciliationSummary.jsp?dispatchid='+DispatchID, function(data) {
		
		$.mobile.hidePageLoadingMsg();
		
		$("#DeskSaleExtraLoadReconciliationStatus").html(data);
		$("#DeskSaleExtraLoadReconciliationStatus").trigger('create');
		...
		$.ajax({
		    ...
		    success: function( json ) {
		    	if (json.exists == "true"){
		    		...
		    		window.location='DeskSaleExtraLoad.jsp';
...
function DeskSaleAddProduct(){
	...
	var length = document.getElementsByName("DeskSaleExtraLoadSummaryProducts").length;
	var ExtraLoadProductFlag = 0;
	for(var i = 0; i < length; i++){
		if( document.getElementsByName("DeskSaleExtraLoadSummaryProducts")[i].value == $('#DeskSaleProductCode').val() ){
			ExtraLoadProductFlag++;
			
		}
	}
	
	if(ExtraLoadProductFlag == 0){
		alert('Product not listed');
		document.getElementById('DeskSaleProductCode').focus();
		return false;
	}
...
function updateDeskSalePromotions(){
	/*
	$.get('DeskSaleExtraLoadPromotions.jsp?'+$('#DeskSaleMainForm').serialize(), function(data) {
	...
	$.ajax({
	    url: "DeskSaleExtraLoadPromotions.jsp",
...
function DeskSaleSubmit(){
	...
	$('#DeskSaleSave').addClass("ui-disabled");
	$.ajax({
	    url: "inventory/DeskSaleExtraLoadExecute",
	    ...
	    success: function( json ) {
	    	if (json.success == "true"){
	    		location = "DeskSaleExtraLoad.jsp?DispatchID="+json.DispatchID;
	    	}else{
	    		$('#DeskSaleSave').removeClass('ui-disabled');
				alert(json.error);
	    	}
...
function showSearchContent(){
	...
	$.get('DeskSaleExtraLoadSearch.jsp?FromDate='+$('#DeskSaleFromDate').val()+'&ToDate='+$('#DeskSaleToDate').val()+'&DispatchID='+DispatchID, function(data) {
					...
					for( var i = 0; i < json.rows.length; i++ ){
						...
						if( json.rows[i].IsPromotion == '0' ){
							...
							"<tr id='DeskSale"+RowMaxID+"'>"+
								...
								"<td>"+formatInCommas(json.rows[i].NetAmount)+"<input type='hidden' id='DeskSaleMainFormNetAmount' name='DeskSaleMainFormNetAmount' value='"+json.rows[i].NetAmount+"'></td>"+
								"<td><a data-role='button' data-mini='true' data-icon='delete' data-iconpos='notext' data-inline='true' onClick=\"DeskSaleDeleteRow('DeskSale"+RowMaxID+"');\">Delete</a></td>"+
							"</tr>";
	...
	if (r==true)
	  {	  
		$.ajax({
		    ...
		    success: function( json ) {
		    	if (json.success == "true"){
		    		location = "DeskSaleExtraLoad.jsp";
		    	}else{
					alert(json.error);
		    		//alert("Server could not be reached.");
		    	}
...
{
	//alert($("#DispatchIDForLastUse").val());	
