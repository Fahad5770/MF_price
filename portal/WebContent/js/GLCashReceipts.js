...
$( document ).delegate("#GLCashReceipts", "pageinit", function() {
	if(msg == 1){
		
		setTimeout(function(){
			$('#MsgPopupDialog').popup("open");
		}, 500);
...
function FormSubmit(){
	...
	$('#ReceiptTypeSubmitForm').val( $('#Type').val() );
	$('#NarrationSubmitForm').val( $('#Narration').val() );
	
	$.ajax({
	    url: "cash/GLCashReceiptsExecute",
	    ...
	    success: function( json ) {
	    	...
	    	if (json.success == "true"){
	    		//alert("true");
	    		window.location = 'GLCashReceipts.jsp?msg=1&PrintID='+json.PrintID;
	    	}else{
	    		alert(json.error);
	    	}
	    },
...
function AddRow(){
	...
	if( $('#AddRowAmount').val() == "" ){
		...
		if( isInteger($('#AddRowAmount').val()) == false ){
