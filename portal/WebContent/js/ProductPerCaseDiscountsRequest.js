...
$( document ).delegate("#BrandExchange", "pageinit", function() {
	...
	$("#BrandExchangePackage").change( function( event ) {
		getLRBTypeList( $("#BrandExchangePackage").val() );
...
function getLRBTypeList(PackageID){
	$.mobile.showPageLoadingMsg();
	
	if (PackageID != ""){
		$.ajax({
		    url: "inventory/ProductPerCaseDiscountGetLrbTypeJson",
		    data: {
		    	PackageID: PackageID		    	
		    },
		    type: "POST",
		    ...
		    success: function( json ) {
		    	...
		    	if (json.exists == "true"){
		    		...
		    		//{
		    			//BrandSelectLis+="<option value='"+json.rows[i].id+"'>"+json.rows[i].label+"</option>";
		    		//}
		    		var LRBTypeSelectLis="";
					var x=0;
		    		for(var i=0;i<json.rows.length;i++)
		    		{
		    			
		    			LRBTypeSelectLis+='<input type="checkbox" name="PromotionPBrandList" id="PromotionPBrandList_'+x+'" value="'+json.rows[i].id+'" style="width:80%">'+
						'<label for="PromotionPBrandList_'+x+'" >'+json.rows[i].label+' </label>';
		    			x++;
		    		}
		    		LRBTypeSelectLis +='<input type="checkbox" name="PromotionPBrandList12f" id="PromotionPBrandList_12f" value="" onClick="CheckAllCheckboxes()">'+
		    		...
		    		$('#SpanProductPromotionsIssuePromotions').html(LRBTypeSelectLis);
...
function AddSalesToTable()
{
	...
	if(!IsValid)
	{
		...
		"<tr id='ProductPromotionsDynamicTableRows_"+RowCount+"' style='width:100%'>"+
			...
			"<td><input type='text' placeholder='Quantity' id='EstimatedSalesVolumeRawCases"+RowCount+"' name='Quantity' data-mini='true' onKeyup='CalculateNetPrice("+RowCount+");'/></td>"+
			"<td><input type='text' placeholder='Percase Discount' id='SalesSKUPrice"+RowCount+"' name='PercaseDiscountRate' data-mini='true' onKeyup='CalculateNetPrice("+RowCount+");' /></td>"+
			"<td><input type='text' placeholder='Amount' id='FreeSKUPriceBottles"+RowCount+"' name='SellingPrice' data-mini='true'  readonly/></td>"+
...
function ProductPromotionsSubmit()
{
	...
	if($("#DistributorID2").val().trim()!=""){
		...
		$.ajax({    		
				url: "inventory/ProductPerCaseDiscountsRequestExecute",    			
				...
				success:function(json){
					if(json.success == "true"){    				
						...
						if(json.errorM=="1"){
							$("#DiscReqErrorSumamry").html(
									
									"Requst ID : "+json.RequestID +"<br/>"+
									"Validity: "+json.ValidFrom+" - "+json.ValidTo +"<br/>"+
									"Distributor : "+json.Distributor +"<br/>"+
									"Package : "+json.PackageLabel +"<br/>"+
									"LRB : "+json.LrbType +"<br/>"+
									"Discount Rate : "+json.DiscountRate +"<br/>"+
									"Quantity : "+json.Quantity +"<br/>"
...
function AddDistributor()
{
	...
	if(!DistributorAlreadyAdded && $("#DistributorID2").val().trim()!="")
	{
    	...
    	$("#DiscReqAddDist").addClass('ui-disabled');
    	$("#DistributorID2").addClass('ui-disabled');
...
{    	
	...
	$("#WarehouseeSave").removeClass('ui-disabled');
	$("#DiscReqAddDist").removeClass('ui-disabled');
	$("#DistributorID2").removeClass('ui-disabled');
}
...
function ProductPromotionsAdditionalRightSubmit(ProductPromotionID)
{
	...
	$.ajax({
	    url: "inventory/ProductPerCaseDiscountsRequestAdditionalRightExecute",
	    ...
	    success: function( json ) {
	    	if (json.success == "true"){		    		
	    		window.location="ProductPerCaseDiscountsRequest.jsp";
	    	}else{
				alert(json.error);
	    		//alert("Server could not be reached.");
	    	}
...
function CalculateNetPrice(RowID){
	$("#FreeSKUPriceBottles"+RowID).val($("#EstimatedSalesVolumeRawCases"+RowID).val()*$("#SalesSKUPrice"+RowID).val());
}
