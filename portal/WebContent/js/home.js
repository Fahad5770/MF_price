...
function showWorkflowStatus(TypeID){
	...
	if( !isNaN(days) ){
		$.get('HomeWorkflowStatus.jsp?days='+days+'&TypeID='+TypeID+'&'+Math.random()+'='+Math.random(), function(data) {
...
function setWorkflowEvents(pageid){
	...
	$(pageid +" .WorkflowSubmitConfirm").on("click", function( event, ui ) {
		...
		$.ajax({
		    url: "outlet/WorkflowManagerNewOutletRequestExecute",
		    data: $(pageid + " .WorkflowForm" ).serialize(),
		    type: "POST",
		    dataType : "json",
		    success: function( json ) {
...
function addPerCaseRow(pageid){
	...
	var content = '<tr>'+
    '<td>'+
    	'<select data-mini="true" id="PerCasePackage'+num+'" name="PerCasePackage'+num+'" onchange="GetBrandList(this.value, '+num+')">'+
    	'<option value="0">Select...</option>';
	for (x = 0; x < rowsnum; x++){
		
		content += '<option value="'+INVENTORY_PACKAGE_JSON.rows[x].id+'">'+INVENTORY_PACKAGE_JSON.rows[x].label+'</option>';
		
	}
	
	content += '</select>'+
    '</td>'+
    '<td><span id="BrandSpan'+num+'">'+
	'<select data-mini="true" id="PerCaseBrand'+num+'" name="PerCaseBrand'+num+'">'+
	'';
		for (x = 0; x < rowsbrand; x++){
			
			content += '<option value="'+INVENTORY_BRAND_JSON.rows[x].id+'">'+INVENTORY_BRAND_JSON.rows[x].label+'</option>';
	...
	////////////////////////////////////////////////////////////////
	'</td>'+
    '<td id="DivPerCaseCompanyShare'+num+'"><input type="text" id="PerCaseCompanyShare'+num+'" name="PerCaseCompanyShare'+num+'" data-mini="true" placeholder="Rs."></td>'+
    '<td><input type="text" id="PerCaseAgencyShare'+num+'" name="PerCaseAgencyShare'+num+'" data-mini="true" placeholder="Rs."></td>'+
    '<td><input type="text" id="PerCaseDeductionTerm'+num+'" name="PerCaseDeductionTerm'+num+'" data-mini="true" placeholder="Rs. / Month"></td>'+
    '<td><input type="text" id="PerCaseHandToHand'+num+'" name="PerCaseHandToHand'+num+'" data-mini="true" placeholder="Hand to Hand">'+
    '</td>'+
    '<td>'+
    	'<input type="text" id="PerCaseValidFrom'+num+'" name="PerCaseValidFrom'+num+'" data-mini="true" placeholder="DD/MM/YYYY"></td>'+
	'<td><input type="text" id="PerCaseValidTo'+num+'" name="PerCaseValidTo'+num+'" data-mini="true" placeholder="DD/MM/YYYY">'+
    '</td>'+
	...
	$(pageid + " #PerCaseCompanyShare"+num+"").rules("add", {
		number: true,
		maxlength: 12
	});				
	$(pageid + " #PerCaseAgencyShare"+num+"").rules("add", {
		number: true,
		maxlength: 12
	});				
	$(pageid + " #PerCaseDeductionTerm"+num+"").rules("add", {
		number: true,
		maxlength: 12
	});				
	$(pageid + " #PerCaseValidFrom"+num+"").rules("add", {
		pkDate: true
	});				
	$(pageid + " #PerCaseValidTo"+num+"").rules("add", {
		pkDate: true
	});				
	...
	$.ajax({
	    ...
	    success: function( json ) {
	    	...
	    	if (json.exists == "true"){
	    		...
	    		//for(var x=0;json.rowslrb.length;x++){
	    			
	    			//console.log("ABC"+json.rowslrb[x].lrb_type_id);
	    			
	    			//lrbtypeString +="<option name='' id='' value='a_"+json.rowslrb[x].lrb_type_id+"'>"+json.rowslrb[x].lrb_type_label+"</option>";
	    		...
	    		if(json.is_LrbType=="false"){
	    			$('#BrandSpan'+num).html((json.SelectOptions).replace("<select id='select_id' name='select_id' data-mini='true' ><option value=''>Select Brand</option>", "<select id='PerCaseBrand"+num+"' name='PerCaseBrand"+num+"' data-mini='true' >"));
	    			
	    		}else{
	    			$('#BrandSpan'+num).html((json.SelectOptions2).replace("<select id='select_id' name='select_id' data-mini='true' ><option value=''>Select Brand</option>", "<select id='PerCaseBrand"+num+"' name='PerCaseBrand"+num+"' data-mini='true' >"));
	    		}
...
function GetBrandListAndLrbType(PackageIDVal, num){
