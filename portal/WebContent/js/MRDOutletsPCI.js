$( document ).delegate("#MRDOutletsPCIFormPage", "pageinit", function() {
...
function FormSubmit(){
	...
	$.ajax({
		...
		url: "mrd/MRDPCIOutletsExecute",
		data: $('#MRDOutletsPCIForm').serialize(),
		type:"POST",
		dataType:"json",
		success:function(json){
			$.mobile.hidePageLoadingMsg();
			if(json.success == "true"){
				window.location='MRDOutletsPCI.jsp';
			}else{
				alert(json.error);
			}			
		},
...
function DeleteSubmit(){
	...
	$.ajax({
		...
		url: "mrd/MRDOutletsDeletePCIOutlet", 
		data: {
			OutletID: $('#OutletID').val()
		},
		type:"POST",
