		    	...
		    	if (json.exists == "true"){
		    		...
		    		if (isAlreadyEntered != true){
		    			...
		    			if(!IsAlreadyProductAdded && IsAlreadyProductDispatchReturnSummaryAdded && !IsAlreadyTotalUnitsDispatchReturnSummaryAdded)
	    				{
							...
							/*
							$.post('DispatchReturnsSummary.jsp?'+data1+'&DispatchID='+$("#DispatchIDForInsertion").val()+"&VehicleName="+$("#VehicleNameForTitle").val(), function(data) {   			
								  $("#DispatchReturnsSummary").html(data);
								  $("#DispatchReturnsSummary").trigger('create');
							});
							*/
							//$.mobile.loading( 'show');
							$.ajax({
							    url: "DispatchReturnsSummary.jsp",
							    data: data1+'&DispatchID='+$("#DispatchIDForInsertion").val()+"&VehicleName="+$("#VehicleNameForTitle").val(),
							    type: "POST",
...
function DispatchReturnsSubmit()
{
	...
	$.mobile.loading( 'show');
	$.ajax({
	    url: "distributor/DispatchReturnsExecute",
