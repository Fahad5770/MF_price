...
$(document)
		.delegate(
				...
				function() {
					...
					$('#DeskSaleProductCode').on(
							'dblclick',
							function(e, data) {

								$("#LookupProductSearch").on(
										"popupbeforeposition",
										function(event, ui) {
											lookupProductInit();
										});
					...
					setTimeout(
							'$("#DeskSalePaymentMethod").change(function(event){	DeskSaleUpdateChangeStatus(); });',
							1000);

					$('#DeskSaleProductCode').on("dblclick", function(e, data) {
						$('#autocomplete_product').html('');
						$("#popupSearch").popup("open", {
							positionTo : "#DeskSaleProductCode"
						});
																...
																$
																		.each(
																				...
																				function(
																						i,
																						val) {
																					html += "<li><a href='#' data-ajax='false' onclick=\"setProduct("
																							+ val.SapCode
																							+ ")\">"
																							+ val.PackageName
																							+ " "
					...
					$("#FromDateSpan")
							.click(
									function(event) {
										document
												.getElementById("DeskSaleFromDate").disabled = false;
										document
												.getElementById("DeskSaleToDate").disabled = false;
				...
				.ajax({
					...
					success : function(json) {
						if (json.exists == "true") {
							...
							setTimeout(
									"$('#DeskSaleRawCases').focus(); $('#DeskSaleRawCases').removeAttr('readonly');",
									100);
						} else {
			...
			success : function(json) {
				if (json.exists == "true") {
					...
					for (var i = 0; i < json.rows.length; i++) {
						str += "<option value='" + json.rows[i].id + "'>"
								+ json.rows[i].label + "</option>";
					}
...
function DeskSaleAddProduct() {
	...
	if (($('#DeskSaleRawCases').val() == "" || $('#DeskSaleRawCases').val() == "0")
			&& ($('#DeskSaleUnits').val() == "" || $('#DeskSaleUnits').val() == "0")) {
	...
	if ($('#DeskSaleRate').val() == "") {
		document.getElementById('DeskSaleRate').focus();
		return false;
	} else {
		var value = $('#DeskSaleRate').val();
		if (isInteger(value) == false) {
			document.getElementById('DeskSaleRate').focus();
			return false;
		}
	...
	if ($('#DeskSaleBatchCode').val() == ""
			&& !(ValProductCode == '5012' || ValProductCode == '5014')) {
	...
	if (val != "" && val.length > 0) {
		...
		$
				.ajax({
					url : "common/GetProductInfoDeskSales",
					data : {
						ProductCode : val,
						DistributorID : DistributorID,
						RawCases : $('#DeskSaleRawCases').val(),
						Units : $('#DeskSaleUnits').val()
					// UrlString :$('#DeskSaleMainForm').serialize()
					},
					type : "POST",
					dataType : "json",
					success : function(json) {
						...
						if (json.exists == "true") {
							...
							/*
							 * if(json.StockStatus == '1'){ alert('No Stock
							 * Available');
							 * document.getElementById('DeskSaleRawCases').focus();
							 * return false; }else if(json.StockStatus == '2'){
							 * alert('Quantity should not exceed '+
							 * json.AvailableStockInRawCases);
							 * document.getElementById('DeskSaleRawCases').focus();
							 * return false; }
							 */
							var ValDeskSaleRawCases = $('#DeskSaleRawCases')
									.val();
							...
							if (isAlreadyEntered != true) {
								...
								var ValDeskSaleUnits = $('#DeskSaleUnits')
										.val();
								if (ValDeskSaleUnits == '') {
									ValDeskSaleUnits = '0';
								}
								
								var DeskSaleTax = $('#DeskSaleTax')
										.val();
								if (DeskSaleTax == '') {
									DeskSaleTax = '0';
								}
								...
								if (parseInt(ValDeskSaleUnits) >= parseInt(json.UnitPerSKU)) {
									...
									document.getElementById('DeskSaleUnits')
											.focus();
									return false;
								}
								...
								var content = "" + "<tr id='DeskSale"
										+ RowMaxID
										+ "'>"
										+ "<td>"
										+ $('#DeskSaleProductCode').val()
										+ "<input type='hidden' name='DeskSaleMainFormUnitPerSKU' value='"
										+ json.UnitPerSKU
										+ "'></td>"
										+ "<td>"
										+ json.PackageLabel
										+ "<input type='hidden' name='DeskSaleMainFormPackageLabel' value='"
										+ json.PackageLabel
										+ "' ><input type='hidden' name='PackageID' value='"
										+ json.PackageID
										+ "' ><input type='hidden' name='DeskSaleMainFormLiquidInML' value='"
										+ json.LiquidInML
										+ "'></td>"
										+ "<td>"
										+ json.BrandLabel
										+ "<input type='hidden' name='DeskSaleMainFormBrandLabel' value='"
										+ json.BrandLabel
										...
										+ "<td>"
										+ ValDeskSaleRawCases
										+ "<input type='hidden' name='DeskSaleMainFormRawCases' value='"
										+ ValDeskSaleRawCases
										+ "'></td>"
										+ "<td>"
										+ ValDeskSaleUnits
										+ "<input type='hidden' name='DeskSaleMainFormUnits' value='"
										+ ValDeskSaleUnits
										+ "'></td>"
										+ "<td>"
										+ formatInCommas($('#DeskSaleRate')
												.val())
										+ "<input type='hidden' name='DeskSaleMainFormRateHidden' value='"
										+ $('#DeskSaleRate').val()
										+ "'><input type='hidden' name='DeskSaleMainFormUnitRateHidden' value='"
										+ $('#DeskSaleUnitRate').val()
										+ "'></td>"
										+ "<td>"
										+ formatInCommas($('#DeskSaleAmount')
												.val())
										+ "<input type='hidden' id='DeskSaleMainFormAmount' name='DeskSaleMainFormAmount' value='"
										+ $('#DeskSaleAmount').val()
										+ "'></td>"
										+ "<td>"
										...
										+ "'></td>"
										+ "<td>"
										+ formatInCommas($('#DeskSaleNetAmount')
												.val())
										+ "<input type='hidden' id='DeskSaleMainFormNetAmount' name='DeskSaleMainFormNetAmount' value='"
										+ $('#DeskSaleNetAmount').val()
										+ "'></td>"
										+ "<td><a data-role='button' data-mini='true' data-icon='delete' data-iconpos='notext' data-inline='true' onClick=\"DeskSaleDeleteRow('DeskSale"
										+ RowMaxID + "');\">Delete</a></td>"
										+ "</tr>";

								$("#DeskSaleTableBody").append(content)
										.trigger('create');
								...
								$('#DeskSaleOutletID').attr('readonly',
										'readonly');
								...
								document.getElementById('DeskSaleProductCode')
										.focus();
								enableRow();
								$.mobile.hidePageLoadingMsg();
							}
...
function disableRow() {
	$('#DeskSaleProductCode').attr('disabled', 'disabled');
	$('#DeskSaleRawCases').attr('disabled', 'disabled');
...
function enableRow() {
	$('#DeskSaleProductCode').removeAttr('disabled', 'disabled');
	$('#DeskSaleRawCases').removeAttr('disabled', 'disabled');
	$('#DeskSaleUnits').removeAttr('disabled', 'disabled');
...
function updateDeskSalePromotions() {
	...
	/*
	 * $.get('DeskSalePromotions.jsp?'+$('#DeskSaleMainForm').serialize(),
	 * function(data) {
	 * 
	 * $("#DeskSalePromotionsDiv").html(data);
	 * $("#DeskSalePromotionsDiv").trigger('create');
	 * 
	 * setTimeout(function(){ updateInvoiceSummary();
	 * $('#DeskSaleSave').removeClass('ui-disabled'); $.mobile.loading( 'hide'); },
	 * 500);
	 * 
	 * 
	 * 
...
function updateInvoiceSummary() {
	...
	for (var i = 0; i < ArraySize; i++) {
		TotalAmount += parseFloat($("input[name=DeskSaleMainFormAmount]")[i].value);
	}
	
	var TotalTaxAmount = 0;
	for (var i = 0; i < ArraySize; i++) {
		TotalTaxAmount += parseFloat($("input[name=DeskSaleMainFormTax]")[i].value);
	}
	
	
	var total_discont = 0;
	for (var i = 0; i < ArraySize; i++) {
		console.log($("input[name=DeskSaleMainFormDiscount]")[i].value);
	...
	$('#DeskSaleMainFormTotalInvoiceAmountField').val(
			formatInCommas(parseFloat(TotalAmount).toFixed(2)));
	
	$('#DeskSaleMainFormWithHoldingTax').val(
			parseFloat(TotalTaxAmount).toFixed(2));
			
	$('#DeskSaleMainFormWithHoldingTaxField').val(
			formatInCommas(parseFloat(TotalTaxAmount).toFixed(2)));

	var TotalInvoiceAmount = parseFloat(TotalAmount) + parseFloat(TotalTaxAmount);
	$('#DeskSaleMainFormTotalAmount').val(
			parseFloat(TotalInvoiceAmount).toFixed(2));
	$('#DeskSaleMainFormTotalAmountField').val(
			formatInCommas(parseFloat(TotalInvoiceAmount).toFixed(2)));
	...
	if ($('#DeskSaleDiscount').val() == '') {
		...
		if (isInteger(value) == false) {
			...
			Discount = parseFloat($('#DeskSaleDiscount')
					.val());
			$('#DeskSaleDiscount').val(
					$('#DeskSaleDiscount').val());
		}
...
function DeskSaleSubmit() {

	var PromotionBrandLength = $('select[name=DeskSalePromotionBrand]').length;
	for (var i = 0; i < PromotionBrandLength; i++) {
		if ($('select[name=DeskSalePromotionBrand]')[i].value == "") {
			alert("Please select Brand from Promotion List");
			return false;
		}
	}
	...
	$('#DeskSaleDistributorNameHidden')
			.val($('#DeskSaleDistributorName').val());

	$('#DeskSaleSave').addClass("ui-disabled");
	$.ajax({
		url : "inventory/DeskSaleExecute",
		...
		success : function(json) {
			if (json.success == "true") {
				location = "DeskSale.jsp?DeskSaleIDPrint=" + json.DeskSaleID;
			} else {
				$('#DeskSaleSave').removeClass('ui-disabled');
				alert(json.error);
			}
...
function showSearchContent() {
	...
	$.get('DeskSaleSearch.jsp?FromDate=' + $('#DeskSaleFromDate').val()
			+ '&ToDate=' + $('#DeskSaleToDate').val(), function(data) {
...
function getDeskSaleInfoJson(EditID) {

	if (EditID != "") {
		$
				.ajax({
					url : "inventory/GetDeskSaleInfoJson",
					data : {
						EditID : EditID
					},
					type : "POST",
					...
					success : function(json) {
						...
						if (json.exists == "true") {
							console.log("1 Tax " + json.WHTaxAmount);
							...
							$('#DeskSaleMainFormTotalInvoiceAmountField').val(
									formatInCommas(parseFloat(
											json.InvoiceAmount).toFixed(2)));
							$('#DeskSaleMainFormSalesTaxField').val(
									formatInCommas(parseFloat(
											json.SalesTaxAmount).toFixed(2)));
							$('#DeskSaleMainFormWithHoldingTaxField').val(
									formatInCommas(parseFloat(json.WHTaxAmount)
											.toFixed(2)));
											
											
											
							$('#DeskSaleMainFormTotalAmountField').val(
									formatInCommas(parseFloat(json.TotalAmount)
											.toFixed(2)));
											
							$('#DeskSaleMainFormTotalDiscountField').val(
									formatInCommas(parseFloat(json.Discount)
											.toFixed(2)));
											
							$('#DeskSaleMainFormFinalNetAmountRoundedField')
									.val(formatInCommas(json.NetAmount));

							$('#DeskSaleMainFormTotalInvoiceAmount').val(
									parseFloat(json.InvoiceAmount).toFixed(2));
							$('#DeskSaleMainFormSalesTax').val(
									parseFloat(json.SalesTaxAmount).toFixed(2));
							$('#DeskSaleMainFormWithHoldingTax').val(
									parseFloat(json.WHTaxAmount).toFixed(2));
							$('#DeskSaleMainFormTotalAmount').val(
									parseFloat(json.TotalAmount).toFixed(2));
							$('#DeskSaleMainFormTotalDiscount').val(
									parseFloat(json.Discount).toFixed(2));
							$('#DeskSaleMainFormAdjustment').val(
									parseFloat(json.FractionAdjustment)
											.toFixed(2));
							$('#DeskSaleMainFormFinalNetAmountRounded').val(
									json.NetAmount);
							...
							for (var i = 0; i < json.rows.length; i++) {
								...
								if (json.rows[i].IsPromotion == '0') {
									...
									var RowMaxID = parseInt($('#RowMaxID')
											.val()) + 1;
									...
									var content = "" + "<tr id='DeskSale"
											...
											+ json.rows[i].SAPCode
											+ "<input type='hidden' name='DeskSaleMainFormUnitPerSKU' value='"
											+ json.rows[i].UnitPerSKU
											+ "'></td>"
											+ "<td>"
											+ json.rows[i].PackageName
											+ "<input type='hidden' name='DeskSaleMainFormPackageLabel' value='"
											+ json.rows[i].PackageName
											+ "' ><input type='hidden' name='PackageID' value='"
											+ json.rows[i].PackageID
											+ "' ><input type='hidden' name='DeskSaleMainFormLiquidInML' value='"
											+ json.rows[i].LiquidInML
											+ "'></td>"
											+ "<td>"
											+ json.rows[i].BrandName
											+ "<input type='hidden' name='DeskSaleMainFormBrandLabel' value='"
											+ json.rows[i].BrandName
											+ "' ><input type='hidden' name='BrandID' value='"
											+ json.rows[i].BrandID
											...
											+ json.rows[i].SAPCode
											+ "'><input type='hidden' name='ProductID' value='"
											+ json.rows[i].ProductID
											+ "'></td>"
											+ "<td>"
											+ json.rows[i].RawCases
											+ "<input type='hidden' name='DeskSaleMainFormRawCases' value='"
											+ json.rows[i].RawCases
											+ "'></td>"
											+ "<td>"
											...
											+ "'></td>"
											+ "<td>"
											+ formatInCommas(json.rows[i].RateRawCases)
											+ "<input type='hidden' name='DeskSaleMainFormRateHidden' value='"
											+ json.rows[i].RateRawCases
											+ "'><input type='hidden' name='DeskSaleMainFormUnitRateHidden' value='"
											+ json.rows[i].RateUnits
											+ "'></td>"
											+ "<td>"
											+ formatInCommas(json.rows[i].TotalAmount)
											+ "<input type='hidden' id='DeskSaleMainFormAmount' name='DeskSaleMainFormAmount' value='"
											+ json.rows[i].TotalAmount
											+ "'></td>"
											+ "<td>"
									...
									DeskSalePromotionsTable += "<tr>";
									DeskSalePromotionsTable += "<td style='width:10%'>"
											+ json.rows[i].SAPCode + "</td>";
									DeskSalePromotionsTable += "<td style='width:10%'>"
											+ json.rows[i].PackageName
											+ "</td>";
									DeskSalePromotionsTable += "<td style='width:10%'>"
											+ json.rows[i].BrandName + "</td>";
									DeskSalePromotionsTable += "<td style='width:10%; text-align:right'>"
											+ json.rows[i].RawCases + "</td>";
									DeskSalePromotionsTable += "<td style='width:10%; text-align:right'>"
											+ json.rows[i].Units + "</td>";
									DeskSalePromotionsTable += "<td style='width:10%; text-align:right'>"
											+ formatInCommas(json.rows[i].RateRawCases)
											+ "</td>";
									...
									DeskSalePromotionsTable += "<td style='width:10%; text-align:right'>"
											+ formatInCommas(json.rows[i].TotalAmount);
									DeskSalePromotionsTable += "<input type='hidden' name='PromotionsProductCode' value='"
											+ json.rows[i].SAPCode
											+ "' > "
											+

											"<input type='hidden' name='PromotionID' value='"
											+ json.rows[i].PromotionID
											+ "' > "
											+

											"<input type='hidden' name='PromotionsPackageID' value='"
											+ json.rows[i].PackageID
											+ "' > "
											+ "<input type='hidden' name='PromotionsBrandID' value='"
											+ json.rows[i].BrandID
											+ "' > "
											+

											"<input type='hidden' name='PromotionsProductID' value='"
											+ json.rows[i].ProductID
											+ "' > "
											+ "<input type='hidden' name='PromotionsUnitPerSKU' value='"
											+ json.rows[i].UnitPerSKU
											+ "' > "
											+ "<input type='hidden' name='PromotionsLiquidInML' value='"
											+ json.rows[i].LiquidInML
											+ "' > "
											+

											"<input type='hidden' name='PromotionsRawCases' value='"
											+ json.rows[i].RawCases
											+ "' > "
											+ "<input type='hidden' name='PromotionsUnits' value='"
											+ json.rows[i].Units
											+ "' > "
											+ "<input type='hidden' name='PromotionsRateRawCase' value='"
											+ json.rows[i].RateRawCases
											+ "' > "
											+ "<input type='hidden' name='PromotionsRateUnit' value='"
											+ json.rows[i].RateUnits
											+ "' > "
											+ "<input type='hidden' name='PromotionsAmount' value='"
											+ json.rows[i].TotalAmount
											+ "' > "
											+ "</td>";
									DeskSalePromotionsTable += "</tr>";
							...
							if (is_promotion) {
								$("#DeskSalePromotionsDiv").append(
										DeskSalePromotionsTable).trigger(
										'create');
							}
				...
				success : function(json) {
					...
					if (json.error == '1') {
						...
						if (json.exists == "true") {
							...
							$('#DeskSaleDistributorID').val(
									json.DistrubutorName);

							$('#DeskSaleMainFormDistributorID').val(
									json.DistrubutorID);
							...
							for (var i = 0; i < json.rows.length; i++) {
								...
								TableContent += '<td>'
										+ json.rows[i].PackageName + '</td>';

								var QuantityUnits = '';
								if (json.rows[i].QuantityUnits > 0) {
									QuantityUnits = '/'
											+ json.rows[i].QuantityUnits;
								}
								...
								TableContent += '<td style="text-align:right" id="package_ordrd_'
										+ json.rows[i].PackageID
										+ '">'
										+ json.rows[i].QuantityRawCases
										+ QuantityUnits + '</td>';
								TableContent += '<td style="text-align:right" id="package_dlvry_'
										+ json.rows[i].PackageID
										+ '">'
										+ ShellVal + '</td>';
								...
								if (json.rows[i].CategoryID == 3) {
									if (!isEditCase) {
										...
										var content = "" + "<tr id='DeskSale"
												...
												+ "'>"
												+ "<td>"
												+ json.rows[i].ProductSAPCode
												+ "<input type='hidden' name='DeskSaleMainFormUnitPerSKU' value='1'></td>"
												+ "<td>"
												+ json.rows[i].PackageName
												+ "<input type='hidden' name='PackageID' value='"
												+ json.rows[i].PackageID
												+ "' ><input type='hidden' name='DeskSaleMainFormLiquidInML' value='0'></td>"
												+ "<td>"
												+ json.rows[i].BrandName
												+ "<input type='hidden' id='ProductCode' name='ProductCode' value='"
												+ json.rows[i].ProductSAPCode
												...
												+ "<td><a data-role='button' data-mini='true' data-icon='delete' data-iconpos='notext' data-inline='true' onClick=\"DeskSaleDeleteRow('DeskSale"
												+ RowMaxID
												+ "'); UpdatePackageDeliveryQuantities("
												+ json.rows[i].PackageID + ", "
												+ json.rows[i].QuantityRawCases
												+ ", 0); UpdateShellCounter('"
												+ json.rows[i].QuantityRawCases
												+ "', '')\">Delete</a></td>"
												+ "</tr>";
							...
							// $('#ProductInfoSpan').html(json.PackageLabel+" -
							// "+json.BrandLabel);

							if (isEditCase) {

								var PackageID = document
										.getElementsByName("PackageID");
								var FormRawCases = document
										.getElementsByName("DeskSaleMainFormRawCases");
								var FormUnits = document
										.getElementsByName("DeskSaleMainFormUnits");
								...
								for (x = 0; x < PackageID.length; x++) {
									...
									if (OldQty != null) {
										// alert('package_dlvry_'+PackageID[x].value+'
										// = '+OldQty);
										...
										var TotalRawCases = parseInt(DBRawCases)
												+ parseInt(OldRawCases);
										var TotalUnits = parseInt(DBUnits)
												+ parseInt(OldUnits);
							...
							$.mobile.hidePageLoadingMsg();
							document.getElementById("DeskSaleSAPOrderNo")
									.focus();
						}
...
function ProductSearchCallBack(SAPCode, ProductName) {
	...
	setTimeout(
			"$('#DeskSaleRawCases').focus(); $('#DeskSaleRawCases').removeAttr('readonly'); getProductInfoJson("
					+ SAPCode + ")", 100);
}
...
function getOutletShelfRent(){
	...
	$.ajax({
		...
		success : function(json) {
			...
			if (json.exists == "true") {
				$('#ShelfRent').val(json.ShelfRent);
				$("input[name=ShelfRent1]").val(json.ShelfRent);
			}
		},
		error : function(xhr, status) {
			$.mobile.hidePageLoadingMsg();
...
function getOutletName() {
	...
	$.ajax({
		...
		url : "common/GetOutletForDeskSaleJSON",
		data : {
			SAPCode : $('#DeskSaleOutletID').val(),
			FeatureID : 52
		},
		...
		success : function(json) {
			...
			if (json.exists == "true") {
				...
				$.mobile.hidePageLoadingMsg();
				$('#DeskSaleOutletName').val(json.msg);
				$('#OutletAddress').val('');
				$('#DeskSaleDistributorName').val('');
			}
		},
		error : function(xhr, status) {
			$.mobile.hidePageLoadingMsg();
...
function getOutletMasterInfo() {
	$.ajax({
		...
		success : function(json) {
			...
			if (json.exists == "true") {
				$("#DeskSaleDistributorName").val(
						json.distributor_id + " - " + json.distributor_name);
				// $('#DeskSaleDistributorIDHidden').val(json.distributor_id);
				$("#OutletAddress").val(
						json.address + " (" + json.owner_name + ", "
								+ json.owner_tele + ")");
...
function DeskSaleCalculate() {
	...
	if ($('#DeskSaleDiscount').val() == '') {
		...
		Discount = $('#DeskSaleDiscount').val() * RawCases;
			...
			.ajax({
				...
				success : function(json) {
					...
					if (json.exists == "true") {
						...
						var RateRawCase = parseFloat(json.RawCasePrice)
								.toFixed(2);
						...
						var AmountRawCase = parseFloat(RawCases) * RateRawCase;
						var AmountUnit = parseFloat(Units) * RateUnit;

						var TotalAmount = parseFloat(
								parseFloat(AmountRawCase)
										+ parseFloat(AmountUnit)).toFixed(2);
						
						SalesTaxPercent = parseFloat(parseFloat(json.SalesTaxAmount));

						WHDTaxPercent = parseFloat(parseFloat(json.WHTaxAmount));

						$('#RateSpan' + counter).html(RateRawCase);
						$('#AmountSpan' + counter).html(
								formatInCommas(TotalAmount));
						...
						console.log("Tax : " + WDHTaxAmount +"-" + SalesTaxPercent);
...
function validateProductCode() {
	...
	if ($('#DeskSaleProductCode').val() == '') {
		$('#DeskSaleRawCases').attr('readonly', 'readonly');
		$('#DeskSaleUnits').attr('readonly', 'readonly');
		$('#DeskSaleRate').attr('readonly', 'readonly');
		$('#DeskSaleDiscount').attr('readonly', 'readonly');
	} else {
		$('#DeskSaleRawCases').removeAttr('readonly');
		$('#DeskSaleUnits').removeAttr('readonly');
		// $('#DeskSaleRate').removeAttr('readonly');
		// $('#DeskSaleDiscount').removeAttr('readonly');
	}

}

function setRowEmpty() {
	$('#DeskSaleRawCases').val('');
	$('#DeskSaleUnits').val('');
	$('#DeskSaleRate').val('');
	$('#DeskSaleAmount').val('');
