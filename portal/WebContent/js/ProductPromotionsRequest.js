		    ...
		    success: function( json ) {
		    	...
		    	if (json.exists == "true"){
		    		...
		    		for(var i=0;i<json.rows.length;i++)
		    		{
		    			...
		    			BrandSelectLis+='<input type="checkbox" name="PromotionBrandList" id="PromotionBrandList_'+x+'" value="'+json.rows[i].id+'" style="width:80%" onclick="SetBrandHidden('+json.rows[i].id+')">'+
...
function AddSalesToTable()
{
	...
	if( $('#ProductPromotionsRawCases').val() == "" && $('#ProductPromotionsUnits').val() == ""){
		document.getElementById('ProductPromotionsRawCases').focus();
		IsValid = true;
	}
	...
	if(!IsValid)
	{
		...
		PPackageID = $('#BrandExchangePackage').val();
		
		BBrandID = $("#ProductPromtionBrandIDD").val();
		
		
		$.ajax({
		    url: "common/GetVariableCostJSON",
		    data: {
		    ...
		    success: function( json ) {
		    	...
		    	if (json.exists == "true"){						    		
					$('#VariableCost').val(json.VariableCost);
		...
		var content = ""+
		"<tr id='ProductPromotionsDynamicTableRows_"+RowCount+"'>"+
			...
			"<td>"+$("#ProductPromotionsRawCases").val()+"<input type='hidden' name='ProductPromotionsMainFormRawCases' value='"+$("#ProductPromotionsRawCases").val()+"'></td>"+
			"<td>"+$("#ProductPromotionsUnits").val()+"<input type='hidden' name='ProductPromotionsMainFormUnits' value='"+$("#ProductPromotionsUnits").val()+"'></td>"+
			"<td><a data-role='button' data-mini='true' data-icon='delete' data-iconpos='notext' data-inline='true' onClick=\"ProductPromotionsDeleteRowForSales('ProductPromotionsDynamicTableRows_"+RowCount+"')\">Delete</a></td>"+
		"</tr>";
		...
		$("#SalesAddID").addClass('ui-disabled');
...
function ProductPromotionsDeleteRowForSales(RowID){
	$("#"+RowID).remove();
	RowCount--;
	if( RowCount < 1){
		$('#NoProductRow').css('display', 'table-row');
		$('#BrandExchangeSave').addClass('ui-disabled');
		$("#SalesAddID").removeClass('ui-disabled');
	}
}
...
function AddSalesToTablePromotions()
{
	...
	if(!IsValid)
	{
		...
		"<tr id='ProductPromotionsPDynamicTableRows_"+RowCount1+"'>"+
			...
			"<td>"+BrandListName+
			BrandListHiddenFields+
			"<td>"+$("#ProductPromotionsPRawCases").val()+"<input type='hidden' name='ProductPromotionsPMainFormRawCases' value='"+$("#ProductPromotionsPRawCases").val()+"'></td>"+
			"<td>"+$("#ProductPromotionsPUnits").val()+"<input type='hidden' name='ProductPromotionsPMainFormUnits' value='"+$("#ProductPromotionsPUnits").val()+"'></td>"+
...
function ProductPromotionsSubmit()
{
	...
	$.ajax({    		
			url: "inventory/ProductPromotionsRequestExecute",    			
