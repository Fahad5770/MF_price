		    ...
		    success: function( json ) {
		    	...
		    	if (json.exists == "true"){
		    		...
		    		if (isAlreadyEntered != true){
		    			...
		    			{
		    				if(AddedProductArray[i].value==$('#DispatchReturnsProductCode').val() && AddedProductTypeArray[i].value==EmptyCreditReceiptType)
		    				{
		    			...
		    			if(!IsAlreadyProductAdded)
	    				{
							...
							"<tr id='DeskSale"+RowMaxID+"'>"+
								...
								"<td>"+EmptyCreditReceiptTypeText+"<input type='hidden' name='EmptyCreditReceiptType' value='"+EmptyCreditReceiptType+"'></td>"+
...
function ProductionReceiptSubmit()
{
	...
	if($("#DispatchReturnsVehicleNumber").val() !="" && $("#EmptyAccAdjustmentType15").val() !=0){
		...
		$("#EmptyAccountAdjustmentTypeHidden").val($("#EmptyAccAdjustmentType").val());
		$("#EmptyAccountAdjustmentReasonHidden").val($("#EmptyAccAdjustmentReason").val());
		
		$.mobile.loading( 'show');
		$.ajax({
		    url: "empty/EmptyCreditAccountAdjustmentExecute",
		    ...
		    success: function( json ) {
		    	...
		    	if(json.success=="true")	    		
	    		{
		    		...
		    		location = "EmptyCreditAccountAdjustment.jsp?ID="+json.MasterTableID;
