...
var RowCount = 0;
$( document ).delegate("#DispatchEmptyReturnsMainInner", "pageinit", function() {
	
var data1 = $("#DispatchReturnMainForm" ).serialize();
$.mobile.loading( 'show');	
$("#DispatchReturnsSummary").html("<img src='images/snake-loader.gif'>");
$.get('DispatchEmptyReturnsSummary.jsp?'+data1+'&DispatchID='+$("#DispatchIDForInsertion").val()+"&VehicleName="+$("#VehicleNameForTitle").val(), function(data) {   			
		    ...
		    success: function( json ) {
		    	...
		    	if (json.exists == "true"){
		    		...
		    		if (isAlreadyEntered != true){
		    			...
		    			{
		    				//alert("Already Stored units "+AddedTotalUnitsInDispatchReturnsSummaryArray[i].value+" New Entered Units "+AddedProductInDispatchReturnsSummaryArray[i].value+"|"+TotalUnits);
...
function DispatchReturnsSubmit()
{
	$.mobile.loading( 'show');
	$.ajax({
	    url: "distributor/DispatchEmptyReturnsExecute",
	    ...
	    success: function( json ) {
    		...
    		{
	    		location = "DispatchReturnsMain.jsp";
