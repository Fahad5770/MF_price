...
function FormSubmit(){
	...
	$.ajax({
	    url: "cash/GLUnpostCashReceiptsExecute",
	    ...
	    success: function( json ) {
	    	...
	    	if (json.success == "true"){
	    		//alert("true");
	    		window.location = 'GLUnpostCashReceipts.jsp';
	    	}else{
	    		alert(json.error);
	    	}
	    },
...
function getEditInfoJson(){
	...
	$.ajax({
		...
		url: "cash/GLUnpostCashReceiptGetReceiptInfoJson",
		data: {
		...
		success:function(json){
			...
			if(json.success == "true"){
				$('#DistributorID').val(json.CustomerID);
				$('#DistributorName').val(json.CustomerName);
				
				$('#Type').val(json.ReceiptTypeLabel);
				...
				for(var i = 0 ; i < json.rows.length; i++){
					...
					var content = ""+
					"<tr>"+
						"<td>"+json.rows[i].InstrumentLabel+"</td>"+
						"<td style='text-align: right; padding-right: 20px'>"+numberWithCommas(parseFloat(json.rows[i].Amount).toFixed(2))+
							"<input type='hidden' name='InstrumentAmount' id='InstrumentAmount' value='"+json.rows[i].Amount+"'></td>"+
						"<td>"+json.rows[i].InstrumentNo+"</td>"+
						"<td>"+DateVal+"</td>"+
						"<td>&nbsp;</td>"+
					"</tr>";
				...
				var NoItemRowTR = "<tr id='NoProductRow'><td colspan='5' style='margin: 1px; padding: 0px;'><div style='width: 100%; background-color: #FFFFFF; padding: 5px;'>No instrument added.</div></td></tr>";
