...
function DeliveryNoteAddProduct(){
	...
	var ValProductCode = $('#DeliveryNoteProductCode').val();
	if( $('#DeliveryNoteBatchCode').val() == "" && !( ValProductCode == '5012' || ValProductCode == '5014' ) ){
		document.getElementById('DeliveryNoteBatchCode').focus();
		return false;
	}
...
function DeliveryNoteSubmit(){
	
	/*if( $('#DeliveryNoteDistributorID').val() == "" ){
		document.getElementById('DeliveryNoteDistributorID').focus();
		return false;
	}else{
		if(isInteger($('#DeliveryNoteDistributorID').val()) == false){
			$("#DeliveryNoteDistributorName").html("ID must be numeric value");
			document.getElementById('DeliveryNoteDistributorID').focus();
			return false;
		}
	}*/
	
	if( $('#DeliveryNoteSAPOrderNo').val() == "" ){
		document.getElementById('DeliveryNoteSAPOrderNo').focus();
		return false;
	}else{
		var value = $('#DeliveryNoteSAPOrderNo').val();
		if(isInteger(value) == false){
			document.getElementById('DeliveryNoteSAPOrderNo').focus();
			return false;
		}
	...
	if( $('#DeliveryNotePaymentMethod').val() == "" ){
		document.getElementById('DeliveryNotePaymentMethod').focus();
		return false;
	}
	
	if( $('#DeliveryNoteVehicleNo').val() == "" ){
		document.getElementById('DeliveryNoteVehicleNo').focus();
		return false;
	}
	
	if( $('#DeliveryNoteVehicleType').val() == "" ){
		document.getElementById('DeliveryNoteVehicleType').focus();
		return false;
	}
	
	if( $('#DeliveryNoteVehicleType').val() == "2" ){	//	if haulage
		
		if( $('#DeliveryNoteFreightAmount').val() == "" ){
			document.getElementById('DeliveryNoteFreightAmount').focus();
			return false;
		}else{
			if(isFloat( $('#DeliveryNoteFreightAmount').val() ) == false ){
				document.getElementById('DeliveryNoteFreightAmount').focus();
				return false;
			}
		}
		
		if( $('#DeliveryNoteFreightContractorID').val() == "" ){
			document.getElementById('DeliveryNoteFreightContractorID').focus();
			return false;
		}
	...
	$('#DeliveryNoteMainFormInvoiceNo').val($('#DeliveryNoteInvoiceNo').val());
	$('#DeliveryNoteMainFormInvoiceAmount').val($('#DeliveryNoteInvoiceAmount').val());
	
	
	$('#DeliveryNoteMainFormPaymentMethod').val($('#DeliveryNotePaymentMethod').val());
	$('#DeliveryNoteMainFormSAPOrderNo').val($('#DeliveryNoteSAPOrderNo').val());
	$('#DeliveryNoteMainFormIsPartial').val($('#DeliveryNoteIsPartial').val());
	$('#DeliveryNoteMainFormVehicleNo').val($('#DeliveryNoteVehicleNo').val());
	$('#DeliveryNoteMainFormVehicleType').val($('#DeliveryNoteVehicleType').val());
	$('#DeliveryNoteMainFormRemarks').val($('#DeliveryNoteRemarks').val());
	
	$('#DeliveryNoteMainFormFreightAmount').val($('#DeliveryNoteFreightAmount').val());
	$('#DeliveryNoteMainFormFreightContractorID').val($('#DeliveryNoteFreightContractorID').val());
	
	
	$('#DeliveryNoteIsPalatizeHidden').val($('#DeliveryNoteIsPalatize').val());
	
	
	$('#DeliveryNoteSave').addClass("ui-disabled");
	$.ajax({
	    url: "inventory/DeilveryNoteExecute",
	    ...
	    success: function( json ) {
	    	if (json.success == "true"){
	    		location = "DeliveryNote.jsp?DeliveryIDPrint="+json.DeliveryID;
	    	}else{
				alert(json.error);
	    		//alert("Server could not be reached.");
	    	}
...
function getProductInfoJson(ProductID){
	...
	if( ProductID == '5012' || ProductID == '5014' ){
		...
		$('#DeliveryNoteUnits').attr('disabled', false);
		$('#DeliveryNoteBatchCode').attr('disabled', false);
	}
