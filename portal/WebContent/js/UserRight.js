    ...
    function UserAdditionalRightSubmit()
    {
    	...
    	{
    		$.ajax({
    		    url: "employee/UserAdditionalRightExecute",
    		    ...
    		    success: function( json ) {
    		    	if (json.success == "true"){
    		    		...
    		    		else
    	    			{
    		    			...
    		    			//$('input[name=FeatureCheckbox]').attr("checked", false).checkboxradio("refresh");
    ...
    function SetTitle(title,FeatureID)
    {
    	...
    	if($("#SapCode").val()!="")
		{
    		$.get('UserRightDataScope.jsp?title='+title+'&featureid='+FeatureID+'&UserID='+$("#SapCode").val(), function(data) {   			
    ...
    {
    	var DistributorIDVal = $('#DistributorID2').val();
    	var isDistributorFound = false;
    	if(DistributorIDVal != ""){
    		var len = $('input[name=AdminScopeDistributors]').length;
    		if(len > 0){
	    		
	    		for(var i = 0; i < len; i++){
	    			if( $('input[name=AdminScopeDistributors]')[i].value == DistributorIDVal ){
	    				isDistributorFound = true;
	    				break;
	    			}
	    		}
	    		
	    		if(isDistributorFound == false){
	    			alert("Distributor not in the scope.");
	    			return false;
	    		}
    		}
