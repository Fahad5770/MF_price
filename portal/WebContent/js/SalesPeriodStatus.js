    ...
    $( document ).on( "pageinit", "#UserRight", function() {
    	...
    	$.get('SalesPeriodStatusDataScope.jsp', function(data) {   			
    		
    		$("#SalesPeriodDataScope").html(data);
    		  $("#SalesPeriodDataScope").trigger('create');
    		});
    ...
    { //alert($("#isEdit").val());
    	...
    	if($('#password').val().length > 0){
    		...
    		{		
        		$.ajax({
        		    url: "employee/UserRightAdminExecute",
    	...
    	$.ajax({
    		...
    		success:function(json){
    			if(json.exists == "true"){
    				...
    				$('#DistributorName2').val('Not Found');
    			}
    		},
    		error:function(xhr, status){
    			alert("Server could not be reached.");
    ...
    function SavePeriodStatus(){
    	//alert($("#DistributorIDhiddenfield").val());
    	if($("#isDistAdded").val()=="0"){
    	...
    	$.ajax({
    		...
    		url: "inventory/SalesPeriodStatusExecute",
    		data: $("#SalesPeriodForm" ).serialize(),
    		type:"POST",
    		dataType:"json",
    		success:function(json){
    			if(json.success == "true"){
    				alert("Period has been opened successfully");
    				window.location.href="SalesPeriodStatus.jsp"
    			}else{
    				alert(json.error);
    			}
    		},
    ...
    function OpenAction(PID)
    { 
    	$("#PeriodID").val(PID);
    	
    	
    	$( "#popupPadded" ).on( "window", function( event, ui ) {    		
		} );
    ...
    function AddDistributor()
    {
    	...
    	var FunctionName = "onClick=RemoveList('DistributorIDDynamicallyAdded_"+$("#DistributorID2").val()+"')";
    	if(!DistributorAlreadyAdded && $("#DistributorID2").val().trim()!="" && $("#DistributorName2").val().trim()!="Not Found")
    	{
    ...
    function ClosePeriodStatus(){

    	$.ajax({
    		
    		url: "inventory/SalesPeriodCloseExecute",
    		data: $("#SalesPeriodChangeForm" ).serialize(),
    		type:"POST",
    		dataType:"json",
    		success:function(json){
    			if(json.success == "true"){
    				alert("Period has been closed successfully");
    				window.location.href="SalesPeriodStatusChange.jsp"
    			}else{
    				alert(json.error);
    			}
    		},
    ...
    function CheckAllMark(){
    	...
    	if($("#togglebit").val()=="0"){
    		$('.AllClassC').prop('checked', true).checkboxradio('refresh');
    		$("#togglebit").val("1");
    	}else{
    		$('.AllClassC').prop('checked', false).checkboxradio('refresh');
    		$("#togglebit").val("0");
    	}
