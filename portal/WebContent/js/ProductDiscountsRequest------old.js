		    ...
		    success: function( json ) {
		    	...
		    	if (json.exists == "true"){
		    		...
		    		for(var i=0;i<json.rows.length;i++)
		    		{
		    			...
		    			selectedpacksids = "'PromotionBrandList_"+json.rows[i].id+"',"+PackageID+","+json.rows[i].id;
		    			
		    			BrandSelectLis+='<input type="checkbox" name="PromotionBrandList" id="PromotionBrandList_'+json.rows[i].id+'" value="'+json.rows[i].id+'" style="width:80%"  onClick="CustomeValidationOfPacks('+selectedpacksids+')">'+
						'<label for="PromotionBrandList_'+json.rows[i].id+'" >'+json.rows[i].label+' </label>';
		    			x++;
		    			selectedpacksids = "";
		    		}
		    		if(PackageID!=2 && PackageID!=6){//for 1500ML and 500ML PET
...
function CustomeValidationOfPacks(textboxid,packid,brandid){
	
	if(packid==2){
		if(brandid==12){ //to check 1500ML PET and Brand is AQUAFINA
			if($("#PromotionBrandList_12").prop('checked') == true){
				$('#SpanProductPromotionsIssue input[type="checkbox"]').each(function () { 
					//$("#"+$(this).attr('id')).attr('checked', false);
					$("#"+$(this).attr('id')).attr('disabled',true);
					//
				});
				$("#PromotionBrandList_12").attr("disabled", false);
			}else if($("#PromotionBrandList_12").prop('checked') == false){
				$('#SpanProductPromotionsIssue input[type="checkbox"]').each(function () { 
					$("#"+$(this).attr('id')).attr('disabled',false);
				});
			}
		}else{
			//if(brandid ==4 || brandid ==10 || brandid ==22 || brandid ==5 || brandid ==2 || brandid ==1 || brandid ==3){
			if($("#PromotionBrandList_4").prop('checked') == true || $("#PromotionBrandList_10").prop('checked') == true || $("#PromotionBrandList_22").prop('checked') == true || $("#PromotionBrandList_5").prop('checked') == true || $("#PromotionBrandList_2").prop('checked') == true || $("#PromotionBrandList_1").prop('checked') == true || $("#PromotionBrandList_3").prop('checked') == true){
				$("#PromotionBrandList_12").attr("disabled", true);
			}else{
				$("#PromotionBrandList_12").attr("disabled", false);
			}
	...
	}else if(packid==6){ //for 500M PET
		if(brandid==14 || brandid==15){ //to check 500ML PET and Brand is B.B(RED) and G.R(Gold)
			if($("#PromotionBrandList_14").prop('checked') == true || $("#PromotionBrandList_15").prop('checked') == true){
				$('#SpanProductPromotionsIssue input[type="checkbox"]').each(function () { 
					//$("#"+$(this).attr('id')).attr('checked', false);
					$("#"+$(this).attr('id')).attr('disabled',true);
					//
				});
				$("#PromotionBrandList_14").attr("disabled", false);
				$("#PromotionBrandList_15").attr("disabled", false);
			}else if($("#PromotionBrandList_14").prop('checked') == false || $("#PromotionBrandList_15").prop('checked') == false){
				$('#SpanProductPromotionsIssue input[type="checkbox"]').each(function () { 
					$("#"+$(this).attr('id')).attr('disabled',false);
				});
			}
		}else{
			if($("#PromotionBrandList_4").prop('checked') == true || $("#PromotionBrandList_10").prop('checked') == true || $("#PromotionBrandList_12").prop('checked') == true || $("#PromotionBrandList_5").prop('checked') == true || $("#PromotionBrandList_2").prop('checked') == true || $("#PromotionBrandList_1").prop('checked') == true || $("#PromotionBrandList_7").prop('checked') == true){
				$("#PromotionBrandList_14").attr("disabled", true);
				$("#PromotionBrandList_15").attr("disabled", true);
			}else{
				$("#PromotionBrandList_14").attr("disabled", false);
				$("#PromotionBrandList_15").attr("disabled", false);
			}
...
function AddSalesToTable()
{
	...
	if(!IsValid)
	{
		...
		"<tr id='ProductPromotionsDynamicTableRows_"+RowCount+"' style='width:100%'>"+
			...
			"<td><input type='text' placeholder='Discount Rate' id='SalesSKUPrice"+RowCount+"' name='DiscountRate' data-mini='true' onKeyup='CalculateMarginalContribution("+RowCount+");' /></td>"+
...
function ProductPromotionsSubmit()
{
	...
	var vflag = true;
	var vflag1 = true;
	//alert($('input[name="DistributorIDhiddenfield"]').val());
	if(typeof  $('input[name="DistributorIDhiddenfield"]').val()=='undefined'){  //if no distributor is selected
		vflag = false;
	}
	
	//validating form
	
	//ProductPromotionsTableBody
	var idfocus="";
	$('#ProductPromotionsTableBody input[type="text"]').each(function () {
		if ($(this).attr('id').indexOf("VariableCost") >=0 || $(this).attr('id').indexOf("MarginalContribution") >= 0){
