...
function CloseAccountPopup(AccountID, AccountLabel){
	$('#AccountID').val(AccountID);
	$('#AccountIDSelect').val(AccountLabel);
...
function FormSubmit(){
	...
	$('#AccountIDSubmitForm').val( $('#AccountID').val() );
	$('#NarrationSubmitForm').val( $('#Narration').val() );
	
	$.ajax({
	    url: "cash/GLCashPaymentsExecute",
	    ...
	    success: function( json ) {
	    	...
	    	if (json.success == "true"){
	    		//alert("true");
	    		window.location = 'GLCashPayments.jsp';
	    	}else{
	    		alert(json.error);
	    	}
	    },
			...
			if(json.success == "true"){
				...
				"<tr id='GLCashReceiptTR"+RowMaxID+"'>"+
					...
					"<td style='text-align: right; padding-right: 20px'>"+numberWithCommas(parseFloat($('#AddRowAmount').val()).toFixed(2))+
						...
						"<input type='hidden' name='InstrumentCategoryID' id='InstrumentCategoryID' value='"+json.InstrumentCategoryID+"'>"+
...
function AutoAddRowCheque(InstrumentLabel, Amount, InstrumentID, InstrumenCategoryID, ReferenceNo, InstrumentDate, SerialNo){
	...
	"<tr id='GLCashReceiptTR"+RowMaxID+"'>"+
		...
		"<td style='text-align: right; padding-right: 20px'>"+numberWithCommas(parseFloat(Amount).toFixed(2))+
			...
			"<input type='hidden' name='InstrumentCategoryID' id='InstrumentCategoryID' value='"+InstrumenCategoryID+"'>"+
