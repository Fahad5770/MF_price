...
function setWorkflowEvents(pageid){
	...
	$(pageid + " .WorkflowDeclineConfirmButton").on( "click", function( event, ui ) {
		...
		var urll="";
		urll="workflow/DeclineRequestExecute";
		/*if(StepID==4){
			urll="cash/WorkflowManagerCreditLimitRequestExecute";
		}else {
			urll="workflow/DeclineRequestExecute";
		}*/
		...
		$.ajax({
		   // url: "workflow/",
		    url: urll,
		    data: $(pageid + " .WorkflowForm" ).serialize() + "&declined=1",
		    type: "POST",
		    dataType : "json",
		    success: function( json ) {
		    	if (json.success == "true"){
		    	//	$.mobile.changePage("WorkflowManagerRegisterOutlet.jsp?requestID="+json.RequestID+"&processID=1&SecondPage=1");
		    		$.mobile.changePage("home.jsp");
		    	}else{
		    		$( pageid + " #AJAXErrorMessage" ).html(json.error);
		    		setTimeout(function() {
			    		$( pageid + " #AJAXErrorPopup" ).popup( "open", { transition: "slidedown",positionTo: "window" } );
	...
	$(pageid +" .WorkflowSubmitConfirm").on("click", function( event, ui ) {
		...
		$.ajax({
		    url: "outlet/WorkflowManagerFixDiscountRequestExecute",
