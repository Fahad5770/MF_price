...
$( document ).delegate("#GLOneTimeCreditPage", "pageinit", function() {
...
function FormSubmit(){
	...
	if( $('#ApproverID').val() == "" ){
		...
		if( isInteger( $('#ApproverID').val() ) == false ){
			alert("Approver ID must be an Integer.");
			$('#ApproverID').focus();
			return false;
		}else{
			if( $('#isApproverValid').val() == 'false' ){
				alert("Please enter valid Approver ID.");
				$('#ApproverID').focus();
				return false;
			}
		}
	...
	if( $('#Amount').val() == "" ){
		...
		if( isFloat( $('#Amount').val() ) == false ){
			alert("Amount must be a Number or in Decimal.");
			$('#Amount').focus();
			return false;
		}
	}
	...
	$.ajax({
	    url: "cash/GLOneTimeCreditExecute",
	    
	    data: $('#GLOneTimeCreditForm').serialize(), 
	    type: "POST",
	    dataType : "json",
	    success: function( json ) {
	    	...
	    	if (json.success == "true"){
	    		//alert("true");
	    		window.location = 'GLOneTimeCredit.jsp';
	    	}else{
	    		alert(json.error);
	    	}
	    },
...
function getUserName(){
	...
	$.ajax({
		...
		data: {
			UserID: $('#ApproverID').val()
		},
		type:"POST",
		dataType:"json",
		success:function(json){
			if(json.exists == "true"){
				$('#ApproverName').val(json.DistributorName);
				$('#isApproverValid').val('true');
			}else{
				$('#ApproverName').val('');
				$('#isApproverValid').val('true');
			}
...
function openApproverSelectionPopup(){
...
function EmployeeSearchCallBackOneTimeCredit(SAPCode, EmployeeName){
	$('#ApproverID').val(SAPCode);
	$('#ApproverName').val(EmployeeName);
	$('#isApproverValid').val('true');
}
