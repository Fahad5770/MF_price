...
if (DistributorID != 0){
	...
	WhereCustomerID = " and distributor_id in ("+DistributorIDs+") ";
			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
				<thead>
					<tr>
						...
						<td colspan="4" style="text-align: center">Discount / Case</td>
						<td colspan="3" style="text-align: center">Other Cost / Case</td>
					...
					<tr style="font-size:11px;">
						...
						<th style="width: 7.2%;text-align: center;">Gross Revenue</th>
						<th style="width: 7.2%;text-align: center;">Sales Promotion</th>
						<th style="width: 7.2%;text-align: center;">Upfront Discount</th>
						<th style="width: 7.2%;text-align: center;">Retailer - Variable</th>
						<th style="width: 7.2%;text-align: center;">Retailer - Fixed</th>
						<th style="width: 7.2%;text-align: center;">Freight</th>
						<th style="width: 7.2%;text-align: center;">Unloading</th>
						<th style="width: 7.2%;text-align: center;">Haulage</th>
						<th style="width: 7.2%;text-align: center;">Net Revenue</th>
						<th style="width: 7.2%;text-align: center;">Primary</th>
						<th style="width: 7.2%;text-align: center;">Secondary</th>
						<th style="width: 7.2%;text-align: center;">Retailer Variable</th>
						<th style="width: 7.2%;text-align: center;">Retailer Fixed</th>
					</tr>
				</thead>
				...
				<tbody>
					...
					String SQL = "SELECT distributor_id customer_id, name customer_name FROM common_distributors where distributor_id in (select distinct distributor_id from inventory_sales_adjusted isa where isa.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+") "+WhereHOD+WhereRSM+WhereTDM+WhereCustomerID+"";
					//System.out.println(SQL);
					ResultSet rs = s.executeQuery(SQL);
					while(rs.next()){
						
						long CustomerID = rs.getLong("customer_id");
						double UpFrontDiscount = 0;//rs.getDouble("upfront_discount") * (-1);
						double Freight = 0;//rs.getDouble("freight") * (-1);
						double Unloading = 0;//rs.getDouble("unloading") * (-1);
						
						
						TotalGrossRevenue += 0;//rs.getDouble("gross_revenue");
						TotalSalesPromotion += 0;//rs.getDouble("sales_promotion");
						TotalUpFrontDiscount += 0;//UpFrontDiscount;
					...
					<tr>
						...
						<%
						////////////////////////////////////////// per case discount //////////////////////////////////////////
						int PackageID = 0;//rs.getInt("package_id");
						int TypeID = 0;//rs.getInt("product_type_id");
						
						
						double TotalGrossDiscount = 0;
						double GrossValue = 0;
						...
						while(CurrentDate.before(Utilities.getDateByDays(EndDate,1))){
							//out.println(Utilities.getSQLDate(CurrentDate));
...
// 						}
						...
						double NetRevenue = 0;
						NetRevenue = 0;//rs.getDouble("gross_revenue") - (rs.getDouble("sales_promotion")+(rs.getDouble("upfront_discount")*-1)+(rs.getDouble("freight")*-1)+(rs.getDouble("unloading")*-1)+HaulageDiscount);
						...
						<td style="text-align: right"><% if( GrossValue != 0 ) out.print(Utilities.getDisplayCurrencyFormatOneDecimal(GrossValue)); %></td>
						<td style="text-align: right"><% if( FixedDiscount != 0 ) out.print(Utilities.getDisplayCurrencyFormatOneDecimal(FixedDiscount)); %></td>
					</tr>
					<%
					}
					...
					<tr>
						...
						<th style="text-align: right"><%=Utilities.getDisplayCurrencyFormatOneDecimal(TotalSecondaryDiscount)%></th>
						<th style="text-align: right"><%=Utilities.getDisplayCurrencyFormatOneDecimal(TotalFixedDiscount)%></th>
					</tr>
