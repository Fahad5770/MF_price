			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					...
					<tbody>
						...
						<tbody>
							...
							int UnitPerSKU=0;
							//System.out.println("SELECT ecer.id,ecer.distributor_id,(select name from common_distributors cd where cd.distributor_id= ecer.distributor_id) distributor_name,ecer.vehicle_no,ecerp.type_id,(select label from ec_empty_receipt_types ecert where ecert.id=ecerp.type_id) type_name,ecerp.product_id,ip.package_id,(select label from inventory_packages ipp where ipp.id=ip.package_id) package_name, ip.brand_id,(select label from inventory_brands ib where ib.id=ip.brand_id) brand_name,ecerp.raw_cases,ecerp.units,ecerp.total_units,ip.unit_per_sku,ecer.created_on FROM ec_empty_receipt ecer join ec_empty_receipt_products ecerp on ecer.id=ecerp.id join inventory_products ip on ecerp.product_id=ip.id where 1=1 and ecer.warehouse_id in("+WarehouseIDs+") and ecer.created_on between "+Utilities.getSQLDateTime(StartDateTimeConverted)+" and "+Utilities.getSQLDateTime(EndDateTimeConverted)+WhereCustomerID+WherePackage+WhereBrand+WhereWarehouse+WhereEmptyReceiptType);
							...
							ResultSet rs = s.executeQuery("SELECT ect.id,ect.distributor_id,(select name from common_distributors cd where cd.distributor_id= ect.distributor_id) distributor_name,(select vehicle_no from inventory_delivery_note idn where idn.delivery_id=ect.delivery_note_id) vehicle_no ,ect.type_id,(select label from ec_empty_receipt_types ecert where ecert.id=ect.type_id) type_name,ect.product_id,ip.package_id,(select label from inventory_packages ipp where ipp.id=ip.package_id) package_name, ip.brand_id,(select label from inventory_brands ib where ib.id=ip.brand_id) brand_name,ect.raw_cases_issued,ect.units_issued,ect.total_units_issued,ip.unit_per_sku,ect.created_on,ect.delivery_note_id,ect.empty_adjustment_id, (select sap_order_no from inventory_delivery_note idn where idn.delivery_id=ect.delivery_note_id) order_no FROM ec_transactions ect join inventory_products ip on ect.product_id=ip.id where 1=1 and (ect.delivery_note_id is not null OR ect.empty_adjustment_id is not null and ect.total_units_issued !=0) and ect.warehouse_id in("+WarehouseIDs+") and ect.created_on between "+Utilities.getSQLDateTime(StartDateTimeConverted)+" and "+Utilities.getSQLDateTime(EndDateTimeConverted)+WhereCustomerID+WherePackage+WhereBrand+WhereWarehouse+WhereEmptyReceiptType);
							while(rs.next()){
							String TypeString ="";
								long[] ConvertedCases = Utilities.getRawCasesAndUnits(rs.getLong("total_units_issued"),rs.getInt("unit_per_sku"));
								TotalTotalUnits += rs.getLong("total_units_issued");
								UnitPerSKU=rs.getInt("unit_per_sku");
								
								if(rs.getLong("delivery_note_id")!=0){
									TypeString = "Delivery";
								}else{
						...
						<tr style="font-size:11px;">							
							<td><%=rs.getLong("id") %></td>
							<td><%=TypeString %></td>
							<td><%if (rs.getString("order_no")!=null){out.print(rs.getString("order_no"));} %></td>
