...
ResultSet rs41 = s6.executeQuery("SELECT group_concat(distinct package_id) FROM employee_targets_t2 et join employee_targets_packages_t2 etp on et.id = etp.id where month=date_format("+Utilities.getSQLDate(StartDate)+",'%m') and year=date_format("+Utilities.getSQLDate(StartDate)+",'%Y')");
			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					 <thead>
					   <tr style="font-size:11px;">
							...
							//System.out.println("SELECT count(distinct package_id) package_count,lrb_type_id,(select label from inventory_products_lrb_types iplt where iplt.id=lrb_type_id) type_name FROM inventory_products_view where  package_id in ("+ProductsLifted+") and category_id = 1 and lrb_type_id is not null group by lrb_type_id");
					    ...
					    <tr style="font-size:11px;">
							...
							while(rs12.next()){
								System.out.println("SELECT distinct package_id, package_label FROM inventory_products_view where  package_id in ("+ProductsLifted+") "+WherePackage+" and category_id = 1 and lrb_type_id="+rs12.getLong("lrb_type_id")+" order by package_sort_order");
					...
					//	System.out.println("select pjp_id ,employee_id, (select DISPLAY_NAME from users where id=employee_id) as name from employee_targets where month=date_format("+Utilities.getSQLDate(StartDate)+",'%m') and year=date_format("+Utilities.getSQLDate(StartDate)+",'%Y')");
						ResultSet rs1 = s8.executeQuery("select id,pjp_id ,employee_id, (select DISPLAY_NAME from users where id=employee_id) as name from employee_targets_t2 where month=date_format("+Utilities.getSQLDate(StartDate)+",'%m') and year=date_format("+Utilities.getSQLDate(StartDate)+",'%Y')"); //distributor query
						...
						<td><%=pjpId%></td>
							...
							while(rs31.next()){								
								...
								//ResultSet rs4 = s2.executeQuery("SELECT  package_id, brand_id FROM employee_targets_packages_brands where id in (select pjp_id from employee_targets where month=date_format("+Utilities.getSQLDate(StartDate)+",'%m') and year=date_format("+Utilities.getSQLDate(StartDate)+",'%Y')) and package_id in ("+ProductsLifted+") and package_id in (select distinct package_id from  inventory_products_view where lrb_type_id in ("+lrb_type_id+"))");
								...
								ResultSet rs4 = s2.executeQuery("SELECT  distinct package_id FROM inventory_products_view where  package_id in ("+ProductsLifted+") and category_id = 1 and lrb_type_id="+lrb_type_id+" order by package_sort_order");
								while(rs4.next()){
							...
							double qty=0;
								...
								//System.out.println("SELECT sum(etpb.quantity) q2, (select liquid_in_ml from inventory_products_view v where v.brand_id=etpb.brand_id and lrb_type_id="+lrb_type_id+" and package_id="+PackageID+" ) unit FROM employee_targets et  join employee_targets_packages etp on et.id=etp.id join employee_targets_packages_brands etpb on et.id=etpb.id and etp.package_id=etpb.package_id  where et.month=date_format("+Utilities.getSQLDate(StartDate)+",'%m') and et.year=date_format("+Utilities.getSQLDate(StartDate)+",'%Y') and et.pjp_id="+pjpId+" and  etp.package_id="+PackageID+" and  etpb.brand_id in (SELECT brand_id FROM inventory_products_view where lrb_type_id="+lrb_type_id+" and package_id="+PackageID+" and category_id=1)");
								//	ResultSet rs3 = s5.executeQuery("SELECT sum(etpb.quantity) q2, (select liquid_in_ml from inventory_products_view v where v.brand_id=etpb.brand_id and lrb_type_id="+lrb_type_id+" and package_id="+PackageID+" ) unit FROM employee_targets et  join employee_targets_packages etp on et.id=etp.id join employee_targets_packages_brands etpb on et.id=etpb.id and etp.package_id=etpb.package_id  where et.month=date_format("+Utilities.getSQLDate(StartDate)+",'%m') and et.year=date_format("+Utilities.getSQLDate(StartDate)+",'%Y') and et.pjp_id="+pjpId+" and  etp.package_id="+PackageID+" and  etpb.brand_id in (SELECT brand_id FROM inventory_products_view where lrb_type_id="+lrb_type_id+" and package_id="+PackageID+" and category_id=1)");
									ResultSet rs3 = s5.executeQuery("select sum(quantity) q2, (select liquid_in_ml from inventory_packages where id="+PackageID+" ) unit from employee_targets_packages_brands_t2 where package_id="+PackageID+" and id="+id);
									while(rs3.next()){
									 qty = rs3.getDouble("q2");
									 qty2 +=	 (qty!=0) ? (qty)*rs3.getDouble("unit") : 0;
...
<%-- 								<td style="text-align: right;"><%= Utilities.convertToRawCases(qty,unit_per_sku) %></td>  --%>
							...
							%>
								<%-- <td style="text-align: right;"><%= (qty!=0) ? Utilities.getDisplayCurrencyFormat((qty)*rs3.getDouble("unit")) : 0 %></td>	 --%>
								<td style="text-align: right;"><%=Utilities.getDisplayCurrencyFormat(qty2) %></td>					
							<%
