...
String WherePJP = "";
if (PJPIDs.length() > 0) {
	WherePJP = " and isa.outlet_id in (SELECT distinct outlet_id FROM distributor_beat_plan_schedule where id in("
	+ PJPIDs + "))";
}
...
if (OrderBookerIDs.length() > 0) {
	OrderBookerIDsWhere = " and order_no in (select mobile_order_no from mobile_order_unedited mou where mou.created_by in ("
	+ OrderBookerIDs + "))";
}
...
<ul data-role="listview" data-inset="true"
	...
	data-icon="false">
	<li data-role="list-divider" data-theme="a">Outlet Sales Summary
		(Detailed)</li>
	<li>
					...
					<table border=0
						...
						<tbody>
							...
							while (rs.next()) {
								...
								ResultSet rs212 = s5.executeQuery(
								"SELECT assigned_to,(SELECT DISPLAY_NAME FROM users where id=assigned_to) orderbooker_name  FROM pep.distributor_beat_plan_users where id in (SELECT id FROM pep.distributor_beat_plan_schedule where outlet_id="
										+ OutletID + ")");
								if (rs212.first()) {
								...
								ResultSet rs34 = s3.executeQuery(
								"SELECT distinct date(created_on) created_on FROM pep.inventory_sales_adjusted where created_on between "
										+ Utilities.getSQLDate(StartDate) + " and " + Utilities.getSQLDateNext(EndDate) + " and outlet_id="
										+ OutletID + "");
								while (rs34.next()) {
							...
							<tr style="font-size: 11px;">
								...
								<td style="text-align: center;">
									...
									if (ids.get(pid) != 15 && ids.get(pid) != 16 && ids.get(pid) != 59 && ids.get(pid) != 21 && ids.get(pid) != 22) { // except for raw case
										/* System.out.println(
												"SELECT sum(total_units) as c, ipv.unit_per_sku FROM pep.inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id=isap.id join inventory_products_view ipv on ipv.product_id=isap.product_id where isa.created_on between "
														+ Utilities.getSQLDate(CurrentDate) + " and " + Utilities.getSQLDateNext(CurrentDate)
														+ " and isa.outlet_id=" + OutletID + " and isap.product_id=" + ids.get(pid)
														+ "  order by isa.id desc"); */
										ResultSet rs91 = s4.executeQuery(
										"SELECT sum(total_units) as c, ipv.unit_per_sku FROM pep.inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id=isap.id join inventory_products_view ipv on ipv.product_id=isap.product_id where isa.adjusted_on between "
												+ Utilities.getSQLDate(CurrentDate) + " and " + Utilities.getSQLDateNext(CurrentDate)
												+ " and isa.outlet_id=" + OutletID + " and isap.product_id=" + ids.get(pid)
												+ "  order by isa.id desc");
										if (rs91.first()) {
											...
											//	TotalSum[pid] += tempData;
												...
												TotalSum[pid] +=	(rs91.getInt("c") / rs91.getInt("unit_per_sku"));
											...
											}else if(ids.get(pid) == 1 || ids.get(pid) == 4 || ids.get(pid) == 5 || ids.get(pid) == 8){
										...
										/* ResultSet rs91 = s4.executeQuery(
										"SELECT sum(total_units), ipv.unit_per_sku FROM pep.inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id=isap.id join inventory_products_view ipv on ipv.product_id=isap.product_id where isa.created_on between "
												+ Utilities.getSQLDate(CurrentDate) + " and " + Utilities.getSQLDateNext(CurrentDate)
												+ " and isa.outlet_id=" + OutletID + " and isap.product_id=" + ids.get(pid)
												+ " order by isa.id desc"); */
							...
							<tr style="font-size: 11px;">
								...
								%>
								<th data-priority="1" style="text-align: center;"><%=TotalSum[pid] %></th>
								<%
								}
								%>
