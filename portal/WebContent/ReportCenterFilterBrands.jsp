...
<fieldset data-role="controlgroup" data-corners="true" style="margin-left: 0px; margin-top: -10px; padding-top: 0px;">
			<input type="button" name="Apply" id="Apply" data-theme="a" value="Brands" style="text-align: left" data-mini="true">
           ...
           if (session.getAttribute(UniqueVoID+"_SR1SelectedBrands") != null){
        	   SelectedBrandArray = (long[])session.getAttribute(UniqueVoID+"_SR1SelectedBrands");           	
           }
           
          //  ResultSet rs = s.executeQuery("select * from inventory_brands where id in (select brand_id from inventory_products where category_id = 1)");
          System.out.println("select * from inventory_products_lrb_types where id!=3");
             ResultSet rs = s.executeQuery("select * from inventory_products_lrb_types where id!=3");
            while(rs.next()){
            	if (session.getAttribute(UniqueVoID+"_SR1SelectedBrands") != null){
            	...
            	%>    		
   					...
   					<label for="BrandID_<%=rs.getString("id") %>" ><span style="font-size:9pt; font-weight:normal;"><%=rs.getString("label") %></span></label> 
