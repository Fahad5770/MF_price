...
<table style="width: 100%; margin-top:-8px" cellpadding="0" cellspacing="0">
	...
	<tr>
		...
		//ResultSet rs = s.executeQuery("select * from employee_view where sap_code in (select distinct dbpv.assigned_to from distributor_beat_plan_view dbpv  where dbpv.distributor_id in ("+DistributorIds+") "+WhereHOD+WhereRSM+")");
		  ResultSet rs = s.executeQuery("select mo.created_by, u.DISPLAY_NAME, count(*) no_of_orders, sum(mo.net_amount) total_amount from mobile_order mo join users u on u.id = mo.created_by where mo.mobile_timestamp between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateTimeNext(EndDate)+" and mo.distributor_id in ("+DistributorIds+")"+WhereHOD+WhereRSM+WhereSM+WhereTDM+WhereASM+" and status_type_id in (1,2) group by mo.created_by");
		...
		while(rs.next()){
			...
			long TotalOrders=0;
			long TotalNoOrders=0;
			
			
			ResultSet rs2 = s2.executeQuery("SELECT count(*) t_order FROM pep.mobile_order where created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateTimeNext(EndDate)+" and created_by="+CreatedBy);
			if(rs2.first()){
				TotalOrders = rs2.getLong("t_order");
			}
			
			
			ResultSet rs21 = s2.executeQuery("SELECT count(*) t_order FROM pep.mobile_order_zero where created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateTimeNext(EndDate)+" and created_by="+CreatedBy);
			if(rs21.first()){
				TotalNoOrders = rs21.getLong("t_order");
			}
			
			MarketVisits = TotalOrders+TotalNoOrders;
