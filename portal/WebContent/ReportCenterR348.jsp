					...
					<table border=0
						...
						<tbody>
							...
							<tr>
								...
								System.out.println(
										"select sum(quantity*ip.liquid_in_ml) quantity quantity from employee_targets_packages etp join employee_targets et on etp.id= et.id join inventory_packages ip on etp.package_id = ip.id where month=date_format("
										+ Utilities.getSQLDate(StartDate) + ",'%m') and year=date_format(" + Utilities.getSQLDate(StartDate)
										+ ",'%Y') and et.pjp_id = " + PJPID);
								ResultSet rs311 = s4.executeQuery(
										"select sum(quantity*ip.liquid_in_ml) quantity from employee_targets_packages etp join employee_targets et on etp.id= et.id join inventory_packages ip on etp.package_id = ip.id where month=date_format("
										+ Utilities.getSQLDate(StartDate) + ",'%m') and year=date_format(" + Utilities.getSQLDate(StartDate)
										+ ",'%Y') and et.pjp_id = " + PJPID);
								while (rs311.next()) {
								...
								System.out.println(
										"select sum(isap.total_units * ip.liquid_in_ml) AS SD_CONVERTED_CASES from inventory_sales_adjusted_products isap    join inventory_packages ip on isap.cache_package_id = ip.id where    isap.cache_created_on_date between "
										+ Utilities.getSQLDate(StartDateAttaintment) + " and " + Utilities.getSQLDate(EndDateAttaintment)
										+ " and isap.cache_pjp_id=" + PJPID);
								...
								System.out.println(
									...
									while (rs4.next()) {
										...
										System.out.println(
										"SELECT et.id,et.employee_id,etp.package_id,etp.quantity q1, sum(etp.quantity) q2 FROM employee_targets et  join employee_targets_packages_brands etp on et.id=etp.id   where et.month=date_format("
							...
							/* 	ResultSet rs33 = s8.executeQuery(
								...
								ResultSet rs34 = s9.executeQuery(
									...
									totalskuView = rs35.getLong(1);
										...
										ResultSet rs36 = s11.executeQuery(
										"SELECT sum(isap.total_units) FROM pep.inventory_sales_adjusted_products isap   where isap.cache_lrb_type_id="
												...
												+ " and " + Utilities.getSQLDate(EndDateAttaintment) + "");
										while (rs36.next()) {
