...
Date AdjustedDate = Utilities.parseDate(request.getParameter("DispatchAdjustedPrintingAdjustedDate"));
...
<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="a" data-count-theme="c">
	...
	<%
		...
		ResultSet rs = s.executeQuery("SELECT distinct vehicle_id, (SELECT vehicle_no FROM distribtuor_vehicles where id=vehicle_id) vehicle_name, driver_id, (SELECT name FROM distributor_employees where id=driver_id) driver_name, id,dispatch_type FROM inventory_sales_dispatch where distributor_id="+DistributorID+" and is_adjusted=1 and adjusted_on between "+Utilities.getSQLDate(AdjustedDate)+" and "+Utilities.getSQLDateNext(AdjustedDate));
		while(rs.next()){
			%>
			<%
