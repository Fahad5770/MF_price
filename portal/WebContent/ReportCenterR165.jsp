...
<script type="text/javascript">
	...
	if(PjpID == "x125"){
		var url = 'ReportCenterR165PJPOutletSummary.jsp?PJPID=x125&UniqueSessionID=<%=UniqueSessionID%>';
	}	
	
	var url = 'ReportCenterR165PJPOutletSummary.jsp?PJPID='+PjpID+'&UniqueSessionID=<%=UniqueSessionID%>';
	...
	<tr>
		...
		<td style="width: 30%" valign="top">
			...
			<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="c" data-count-theme="c" style="margin-top:-13px;">
				...
				ResultSet rs22 = s.executeQuery("SELECT distinct dbpav.id, label ,count(distinct outlet_id) outlet_count FROM distributor_beat_plan_all_view dbpav join common_outlets co on dbpav.outlet_id=co.id where 1=1 "+WhereDistributors+WhereHOD+WhereRSM+WhereDiscountedAll+WhereDiscountedFixed+WhereDiscountedPerCase+WhereNonDiscountedAll+WhereActive+WhereDeactivated+" group by dbpav.id order by outlet_count desc");
				int counter = 0;
				int TotalOutlets =0;
				...
				while(rs22.next()){
					...
					<li style="height: 35px" >
						<a data-ajax="false" id="PJPLi_<%=rs22.getString("id")%>" href="javascript: LoadPJPOutletReport(<%=rs22.getString("id")%>)" style="font-size:11px; font-weight:normal;"><%=rs22.getString("id")+" - "+rs22.getString("label")%></a>
					</li>
					<%
				...
				//System.out.println(AllPJPIDs);
					...
					%>	
				<li style="height: 35px" >
						<a data-ajax="false" id="PJPLiAll" href="javascript: LoadPJPOutletReport('x125')" style="font-size:12px; font-weight:bold;">All</a>
					</li>
			</ul>
