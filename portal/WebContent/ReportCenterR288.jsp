		...
		<td style="width: 30%" valign="top">
			...
			%>	
				...
				if(NoOrdercounter==0){
					...
					%>
				 <li data-role="list-divider" style="background:#4596ce;height:28px;color:white;font-size: 10px;">Prospect </li>
					<% 
					//System.out.println("SELECT id, outlet_id, (SELECT name FROM common_outlets where id=mobile_order_zero.outlet_id) outlet_name,created_on,mobile_order_no,mobile_timestamp FROM mobile_order_zero where  created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" "+OrderBookerIDsWher+WhereDistributors1+"  "+WhereOutlets+" order by date(created_on) desc, mobile_timestamp desc");
					
					
					ResultSet rs25 = s.executeQuery("SELECT id,outlet_name,created_on,mobile_timestamp,is_potential_outlet FROM mobile_order_sm_outlet_prospect where  created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" "+WhereASM+" order by date(created_on) desc, mobile_timestamp desc");
					int NoProspectcounter = 0;
					while(rs25.next()){
						long MobileOrderNumber = rs25.getLong("id");
						int IsPotentialOutlet = rs25.getInt("is_potential_outlet");	
						...
						if(IsPotentialOutlet==1){
							...
							PotentialStatus="Yes";
							PotentialStyle="color : green";
							
						}else {
							
							PotentialStatus="No";
							PotentialStyle="color : red";
						}
						
						
						Date MobileTimestamp = rs25.getTimestamp("mobile_timestamp");
								
						if(!DateUtils.isSameDay(LastDate, rs25.getDate("created_on")) ){
		            		%>
		            		<li data-role="list-divider" style="font-size: 10px;"><%=Utilities.getDisplayFullDateFormat(rs25.getDate("created_on"))%></li>
		            		<%
		            	}
						...
						<li style="height: 45px" >
							<a data-ajax="false" href="javascript: OrderBookerGetOrderActivityReport(3,<%=rs25.getString("id")%>,<%=MobileOrderNumber%>)" style="font-size:10px" <%if(Utilities.isAuthorized(31, SessionUserID) == true){%>ondblclick="DistReportsOrderBookerOrderListOutletDashboardRedirect(<%=rs25.getString("id")%>)"<%} %>><%=rs25.getString("outlet_name")%> <br>Mobile Timestamp: <%=Utilities.getDisplayDateTimeFormat(MobileTimestamp)  %><br>Potential : <span style="padding-bottom:1%;<%=PotentialStyle%>"><%=PotentialStatus%></span></a>
						</li>
						<%
						LastDate = rs25.getDate("created_on");
						NoProspectcounter++;
					}
					
				
					if(NoProspectcounter==0){
						%>
						<li style="height: 12px">&nbsp;</li>
						<%
					}
