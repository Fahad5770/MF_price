...
function redirect(ReceiptFromProdMainID){
	...
	//alert(SapOrderNo);
	var url = 'DistributorReportsReceiptFromProductionSummary.jsp?ReceiptFromProdMainID='+ReceiptFromProdMainID;
	/*if(OrderID == ''){
	...
	function OrderBookerGetOrderActivityReport(OrderID){
		...
		var url = 'DistributorReportsReceiptFromProductionSummary.jsp?OrderID='+OrderID;
		if(OrderID == ''){
			url = 'DistributorReportsReceiptFromProductionSummary.jsp';
		}
		$("#OrderActivityReportTD").html("<img src='images/snake-loader.gif'>");
	...
	<tr>
		<td style="width: 40%" valign="top">
			...
			<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="c" data-count-theme="c" style="margin-top:-13px;">
				...
				ResultSet rs22 = s.executeQuery("select * from inventory_production_receipts where created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" order by created_on desc");
				...
				while(rs22.next()){
					...
					%>
					<li><a data-ajax="false" href="#" onclick="redirect(<%=rs22.getString("id") %>)" style="font-size:13px; font-weight:normal;">Receipt# <b><%=rs22.getString("serial_no") %></b>&nbsp;&nbsp;<%if(rs22.getInt("is_received")==1){ %>[Received]<%}else{ %>[Pending]<%} %><br/><%=Utilities.getDisplayDateTimeFormat(rs22.getTimestamp("created_on"))  %>&nbsp;
