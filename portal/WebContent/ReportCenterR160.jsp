			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					...
					<tbody>
					    ...
					    <tr style="font-size:11px;">							
							...
							<th data-priority="1"  style="text-align:center;  width:15%">Customer</th>				
							<th data-priority="1"  style="text-align:center;  width:15%">Type</th>
							<th data-priority="1"  style="text-align:center;  width:15%">Account Type</th>
							<th data-priority="1"  style="text-align:center;  width:15%">Amount</th>
							<th data-priority="1"  style="text-align:center;  width:25%">Reason</th>
						...
						double Total=0;
						ResultSet rs = s.executeQuery("SELECT id,customer_id,(select name from common_distributors cd where cd.distributor_id=glcad.customer_id ) customer_name,type_id,(select label from gl_customer_account_adjustment_types glcaat where glcaat.id=glcad.type_id) type_name,amount,reason,customer_account_id,(select label from gl_accounts_types glat where glat.id=glcad.account_type_id) customer_account_name FROM gl_customer_account_adjustment glcad where created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate));
						while(rs.next()){
						...
						<tr>
							...
							<td style="text-align:left;"><%=rs.getString("customer_id") %> - <%=rs.getString("customer_name") %></td>			
							<td style="text-align:left;"><%=rs.getString("type_name") %></td>
							<td style="text-align:left;"><%=rs.getString("customer_account_name") %></td>
							<td style="text-align:left;"><%= Utilities.getDisplayCurrencyFormatTwoDecimalFixed(rs.getDouble("amount")) %></td>
							<td style="text-align:left;"><%=rs.getString("reason") %></td>
