			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					 <thead>
					    <tr style="font-size:11px;">
							...
							<th data-priority="1"  style="text-align:center; width: 30% ">Number of Outlets</th>
							<th data-priority="1"  style="text-align:center; width: 40% ">Hand to Hand Discount</th>							
					...
					<%
					String Query = "SELECT isa.distributor_id,(select cd.name from common_distributors cd where cd.distributor_id=isa.distributor_id) dist_name, sum(isap.hand_discount_amount) discount_amount, ("
					 +" SELECT count(distinct iisa.outlet_id) FROM inventory_sales_adjusted iisa join inventory_sales_adjusted_products iisap on iisa.id = iisap.id where iisa.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and hand_discount_amount != 0 and iisa.distributor_id = isa.distributor_id" 
					 +" ) outlet_count FROM inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id = isap.id where created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+WhereDistributors+WhereHOD+WhereRSM+WhereSM+WhereTDM+WhereASM+" group by distributor_id";
					...
					%>
						...
						<tr>
							<td style="text-align:left"><%=rs.getLong("distributor_id") %> - <%=rs.getString("dist_name") %></td>							
							<td style="text-align:left"><%=rs.getInt("outlet_count") %></td>
							<td style="text-align:left"><%=rs.getDouble("discount_amount") %></td>													
