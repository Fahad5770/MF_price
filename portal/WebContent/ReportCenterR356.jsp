...
function ExportToExcel(D1,D2,DistributorIDs,OrderBookersIDs,TSOIDs,ASMIDs,RegionIDs,ChannelIDs,CityIDs,BrandIDs,SKUIDs){
	//alert(D1);
...
function refreshExportDiv(){
	var mydiv = document.getElementById("aExcelFileReady");
	$(mydiv).html("<a onclick=\"ExportToExcel(<%=Utilities.getSQLDate(StartDate) %>, <%=Utilities.getSQLDate(EndDate) %>, '<%=DistributorIDs %>', '<%=OrderBookersIDs %>','<%=TSOIDs %>','<%=ASMIDs %>','<%=RegionIDs %>', '<%=ChannelIDs %>','<%=CityIDs %>', '<%=BrandIDs %>', '<%=SKUIDs %>')\" href=\"#\"    data-theme=\"c\" data-inline=\"true\" style=\"text-decoration: none;margin-top: 0px; margin-left: 0px; margin-bottom: 0px\">Export To Excel</a>");
}
...
<ul data-role="listview" data-inset="true"
	...
	data-icon="false">
	<li data-role="list-divider" data-theme="a">Channel Wise Sale Report</li>
	<li>
		...
		<table style="width: 100%; margin-top: -8px" cellpadding="0"
			...
			<select  name="cityid" onChange="refreshExportDiv()"  data-mini="true" id="reportTypeId" >
				<option value="-1">Select Report Type</option>
				        ...
				        <option value="3">Amount</option>
								...
								<td style="text-align: center;width:75%;">
   									...
   									<div id="aExcelFileReady" style=" margin-top: 6px;">
   									<p style="text-align: center;
   
    font-weight: bold;
    font-size: 17px;">Please select report type</p>
<%--    										<a onclick="ExportToExcel(<%=Utilities.getSQLDate(StartDate) %>, <%=Utilities.getSQLDate(EndDate) %>, '<%=DistributorIDs %>', '<%=OrderBookersIDs %>','<%=TSOIDs %>','<%=ASMIDs %>','<%=RegionIDs %>', '<%=ChannelIDs %>','<%=CityIDs %>', '<%=BrandIDs %>', '<%=SKUIDs %>')" href="#"    data-theme="c" data-inline="true" style="margin-top: 0px; margin-left: 0px; margin-bottom: 0px">Export To Excel</a>
 --%>   								</div>
