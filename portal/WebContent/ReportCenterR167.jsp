...
function ReportCenterR167LoadBrandsList(ParamDocumentID, ParamPackageID, ParamTypeID){
	...
	$.ajax({
	    url: "ReportCenterR167LoadBrandsList.jsp",
	    data: {
...
function ReportCenterR167LoadScopeList(ParamDocumentID){
	...
	$.ajax({
	    url: "ReportCenterR167LoadScopeList.jsp",
	    data: {
	    	DocumentID: ParamDocumentID
	    },
	    type: "POST",
	...
	<tr>
		...
		<td style="width: 100%" valign="top">
			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
				...
				<tbody>
				<%
				//System.out.println("SELECT id, label, valid_from, valid_to, activated_on FROM inventory_sales_promotions where id in ( SELECT product_promotion_id FROM inventory_sales_promotions_distributors where "+WhereDistributors+" union SELECT product_promotion_id FROM inventory_sales_promotions_regions where "+WhereRegion+" ) and is_active=1 ");
				ResultSet rs = s.executeQuery("SELECT id, label, valid_from, valid_to, created_on FROM inventory_sales_promotions where id in ( SELECT product_promotion_id FROM inventory_sales_promotions_distributors where "+WhereDistributors+" union SELECT product_promotion_id FROM inventory_sales_promotions_regions where 1=1 "+WhereRegion+" ) and is_active=1 and curdate() between valid_from and valid_to order by created_on desc");
				while(rs.next()){
					%>
						<tr>
							<td nowrap="nowrap"><%=rs.getString("label")%></td>
							...
							<%
							ResultSet rs2 = s2.executeQuery("SELECT package_id, raw_cases, units, (select package_label from inventory_products_view where package_id=inventory_sales_promotions_products.package_id limit 1) package_label FROM inventory_sales_promotions_products where id="+rs.getString("id")+" and type_id=1");
							if(rs2.first()){
								%>
							...
							<%
							ResultSet rs3 = s2.executeQuery("SELECT package_id, raw_cases, units, (select package_label from inventory_products_view where package_id=inventory_sales_promotions_products.package_id limit 1) package_label FROM inventory_sales_promotions_products where id="+rs.getString("id")+" and type_id=2");
							if(rs3.first()){
								%>
