		...
		<td style="width: 100%" valign="top">
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					...
					<tbody>
						...
						System.out.println("select pjp_id ,employee_id, (select DISPLAY_NAME from users where id=employee_id) as name from employee_targets_t2 where month=date_format("+Utilities.getSQLDate(StartDate)+",'%m') and year=date_format("+Utilities.getSQLDate(StartDate)+",'%Y')");
						ResultSet rs1 = s8.executeQuery("select pjp_id ,employee_id, (select DISPLAY_NAME from users where id=employee_id) as name from employee_targets_t2 where month=date_format("+Utilities.getSQLDate(StartDate)+",'%m') and year=date_format("+Utilities.getSQLDate(StartDate)+",'%Y')"); //distributor query
						...
						<td><%=pjpId%></td>
							...
							long qty=0;
								...
								//	ResultSet rs3 = s5.executeQuery("SELECT et.id,et.employee_id,etp.package_id,etp.quantity q1,etpb.brand_id,etpb.package_id, sum(etpb.quantity) q2 FROM employee_targets et  join employee_targets_packages etp on et.id=etp.id  join pep.employee_targets_packages_brands etpb on et.id=etpb.id and etp.package_id=etpb.package_id where et.month=date_format("+Utilities.getSQLDate(StartDate)+",'%m') and et.year=date_format("+Utilities.getSQLDate(StartDate)+",'%Y') and et.employee_id="+EmployeeID+ " and etp.package_id="+PackageID+" and  brand_id in (SELECT brand_id FROM pep.inventory_products_view where lrb_type_id="+lrb_type_id+" and package_id="+PackageID+" and category_id=1)");	
									ResultSet rs3 = s5.executeQuery("SELECT sum(etpb.quantity) q2 FROM employee_targets_t2 et  join employee_targets_packages_t2 etp on et.id=etp.id join employee_targets_packages_brands_t2 etpb on et.id=etpb.id and etp.package_id=etpb.package_id  where et.month=date_format("+Utilities.getSQLDate(StartDate)+",'%m') and et.year=date_format("+Utilities.getSQLDate(StartDate)+",'%Y') and et.pjp_id="+pjpId+" and  etp.package_id="+PackageID+" and  etpb.brand_id in (SELECT brand_id FROM inventory_products_view where lrb_type_id="+lrb_type_id+" and package_id="+PackageID+" and category_id=1)");
									while(rs3.next()){
									 qty = rs3.getLong("q2");
