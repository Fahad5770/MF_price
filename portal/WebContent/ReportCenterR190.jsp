...
if (session.getAttribute(UniqueSessionID+"_SR1SelectedDistributorType") != null){
	...
	DistributorTypeIDs = Utilities.serializeForSQL(SelectedDistributorTypeArray);
}

String WhereDistributorType = "";
if (DistributorTypeIDs.length() > 0){
	WhereDistributorType =" and is_active in ("+DistributorTypeIDs+")";

}
//System.out.println(WhereDistributorType);

//Active type
long Active=3;
if (session.getAttribute(UniqueSessionID+"_SR1SelectedDistributorType") != null){
    SelectedDistributorTypeArray = (long[])session.getAttribute(UniqueSessionID+"_SR1SelectedDistributorType");     
    Active=SelectedDistributorTypeArray[0];
} 


Date old30Days=Utilities.getDateByDays(StartDate, -30);
String ActiveWhere="";
if(Active==0){
	 ActiveWhere = " and distributor_id  in (select distinct distributor_id from inventory_delivery_note  where created_on between "+Utilities.getSQLDate(old30Days)+" and "+Utilities.getSQLDateNext(StartDate)+")";
}else if(Active==1){
	 ActiveWhere = " and distributor_id  not in (select distinct distributor_id from inventory_delivery_note  where created_on between "+Utilities.getSQLDate(old30Days)+" and "+Utilities.getSQLDateNext(StartDate)+")";
}
		...
		<td style="width: 100%" valign="top">
			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					...
					<tbody>
						...
						System.out.println("SELECT * FROM common_distributors where distributor_id in(select distinct distributor_id from ec_transactions) "+WhereHOD1+WhereRSM1+WhereTDM1+WhereRegion1+WhereCustomerID+ActiveWhere);
						boolean flag=true;
						ResultSet rs1 = s.executeQuery("SELECT * FROM common_distributors where distributor_id in(select distinct distributor_id from ec_transactions) "+WhereHOD1+WhereRSM1+WhereTDM1+WhereRegion1+WhereCustomerID+ActiveWhere);
						while(rs1.next()){
							int counter1=1;
							int ix1=0;
							%>
							<tr>
								...
								while(rs2.next()){
									...
									ResultSet rs3 = s3.executeQuery("Select sum(total_units_received), sum(total_units_issued),ip.unit_per_sku  FROM ec_transactions ect join inventory_products ip on ect.product_id=ip.id  where  ip.package_id= "+rs2.getLong("package_id")+" and ip.brand_id in ("+BrandIDModify+") and ect.created_on_date <= "+Utilities.getSQLDate(StartDate)+" and ect.type_id in(1,5,2,8,9,10,11,12,13,14,15) and ect.distributor_id="+rs1.getLong("distributor_id")+WhereHOD+WhereRSM+WhereTDM+WhereRegion+" order by ip.package_id desc");
									if(rs3.first()){
								...
								long Balance1=0;
								 int UnitPerSku1=0;
									...
									ResultSet rs12 = s6.executeQuery("Select sum(total_units_received), sum(total_units_issued),ip.unit_per_sku  FROM ec_transactions ect join inventory_products ip on ect.product_id=ip.id  where  ip.package_id= "+rs2.getLong("package_id")+" and ect.created_on_date <= "+Utilities.getSQLDate(StartDate)+" and ect.type_id in(1,5,2,8,9,10,11,12,13,14,15) and ect.distributor_id="+rs1.getLong("distributor_id")+WhereHOD+WhereRSM+WhereTDM+WhereRegion+" order by ip.package_id desc");
									if(rs12.first()){
									...
									%>
									<td style="text-align:right;background-color:#F6F6F6;"><%if(NewBalance!=0){ %><%=Utilities.convertToRawCasesAccounting(NewBalance,UnitPerSku1) %><%} %></td>
									<%
									...
									GrantTotalArray[GrandCounter]+=NewBalance;
