...
<script type="text/javascript">
	...
	var url = 'ReportCenterR242CensusSummary.jsp?CensusID='+PjpID+'&UniqueSessionID=<%=UniqueSessionID%>';
...
<table id="CensusNameTable" border="0" style="width: 100%">
	<tr>
		...
		<td style="width: 30%" valign="top">
			...
			<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="c" data-count-theme="c" style="margin-top:-13px;">
				<li data-role="list-divider"  data-theme="a">Census</li>
				...
				ResultSet rs22 = s.executeQuery("SELECT * FROM mrd_census where mobile_timestamp between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+WhereDistributors+WhereCensusUser+WhereDistributors+"and is_deleted=0 order by id desc");
				System.out.println("SELECT * FROM mrd_census where mobile_timestamp between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+WhereDistributors+WhereCensusUser+WhereDistributors+"  order by id desc");
				int counter = 0;
				int TotalOutlets =0;
				String OutletName="";
				while(rs22.next()){
					...
					OutletName = rs22.getString("outlet_id")+" - "+rs22.getString("outlet_name");
					...
					<li style="height: 35px" >
						<a data-ajax="false" id="Census_<%=rs22.getString("id")%>" href="javascript: LoadCensusDetailReport(<%=rs22.getString("id")%>)" style="font-size:11px; font-weight:normal;"><%=OutletName%></a>
					</li>
					<%
					TotalOutlets++;
					}				
