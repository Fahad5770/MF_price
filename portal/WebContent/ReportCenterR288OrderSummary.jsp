...
String OrderIDsQuery = "SELECT distinct mo.id FROM mobile_order_sm mo where mo.status_type_id in (1,2) and mo.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+"  "+WhereOrderBooker+" "+WhereOutlets+" "+WhereDistributors;
String outletId="";
...
if(OrderID > 0){
	IsMapShow = true;
	ResultSet rs3 = s.executeQuery("SELECT concat(co.id, ' - ', co.name) outlet_name, mo.created_on, co.id as outlet_id,mo.lat,mo.lng, co.lat olat, co.lng olng  FROM mobile_order_sm mo, common_outlets co where mo.outlet_id=co.id and mo.id="+OrderID);
	if(rs3.first()){
		outletId = rs3.getString("outlet_id");
	...
	//For no order
	IsMapShow2 = true;
	ResultSet rs32 = s.executeQuery("SELECT concat(co.id, ' - ', co.name) outlet_name , moz.created_on, co.id as outlet_id,moz.lat,moz.lng, co.lat olat, co.lng olng  FROM mobile_order_sm_zero moz, common_outlets co where moz.outlet_id=co.id and moz.id="+OrderID);
	if(rs32.first()){
		outletId = rs32.getString("outlet_id");
	...
	OrderIDsQuery = "SELECT distinct mo.id FROM mobile_order_sm mo where mo.status_type_id in (1,2,3) and mo.id="+OrderID;
	
	WherePackage = " and package_id in (";
	
	ResultSet rs = s.executeQuery("SELECT distinct(ipv.package_id) FROM mobile_order_sm_products mop, inventory_products_view ipv where mop.product_id=ipv.product_id and mop.id="+OrderID);
	int counter = 0;
	while(rs.next()){
		if(counter > 0){
			WherePackage += ", ";
	...
	ResultSet rs2 = s.executeQuery("SELECT distinct(ipv.brand_id) FROM mobile_order_sm_products mop, inventory_products_view ipv where mop.product_id=ipv.product_id and mop.id="+OrderID);
	int counter2 = 0;
	while(rs2.next()){
		if(counter2 > 0){
			WhereBrand += ", ";
	...
	//For no order
	IsMapShow3 = true;
	ResultSet rs323 = s.executeQuery("SELECT outlet_name, created_on,lat,lng  FROM mobile_order_sm_outlet_prospect  where id="+OrderID);
	if(rs323.first()){
		OutletName = rs323.getString("outlet_name") + " | "+ Utilities.getDisplayFullDateFormatShort(rs323.getDate(2));
		latt3 = rs323.getDouble("lat");
		lang3 = rs323.getDouble("lng");
						...
						<td style="width: 100%" valign="top">
							...
							<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
								...
								<%
									...
									while(rs.next()){
										...
										while(rs2.next()){
											...
											ResultSet rs3 = s3.executeQuery("SELECT sum(total_units) bottles, sum(if (mop.is_promotion=1, 0, mop.net_amount)) amount, ipv.unit_per_sku FROM mobile_order_sm mo, mobile_order_sm_products mop, inventory_products_view ipv where mo.id = mop.id and mop.product_id=ipv.product_id and mo.id in ("+OrderIDsQuery+") and ipv.package_id="+PackageID+" and ipv.brand_id="+BrandID +" and ipv.category_id = 1 "+WhereDistributors+" group by mop.product_id");
									...
									ResultSet rs21 = s.executeQuery("select sum(invoice_amount), sum(wh_tax_amount), sum(net_amount) from mobile_order_sm where id in ("+OrderIDsQuery+")");
									if (rs21.first()){
								...
								<table border=0 style="font-size:13px; margin-top:20px ; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
									...
									<% 
							         ResultSet rs = s.executeQuery("SELECT moz.comments,moz.no_order_reason_type_v2,(SELECT DISPLAY_NAME FROM users u where u.id=moz.created_by) order_booker_name,(select label from mobile_order_no_sm_order_reason_type where mobile_order_no_sm_order_reason_type.id=moz.no_order_reason_type_v2) name from mobile_order_sm_zero moz where moz.id="+OrderID );
							         while(rs.next()){
							          
							          String Reasonlabel = rs.getString("name");
							          String ReasonComments = rs.getString("comments");
							          String order_booker_name = rs.getString("order_booker_name");
							          int  Reasontype = rs.getInt("no_order_reason_type_v2");
							          
							          %>
							          <tr style="background:#ececec">
							                       <td align="left"><b>Outlet Order</b> : <%=order_booker_name %> </td>
...
%>
								...
								<table border=0 style="font-size:13px; margin-top:20px ; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
									...
									<% 
							         ResultSet rs = s.executeQuery("SELECT *,(SELECT psc.label FROM pci_sub_channel psc where channel=psc.id) ChannelName FROM mobile_order_sm_outlet_prospect where id="+OrderID );
							         while(rs.next()){
							          
							          String Outlet = rs.getString("outlet_name");
							          String ShopkeeprName = rs.getString("shop_keeper_name");
							          String channel = rs.getString("ChannelName");
							          String address = rs.getString("address");
							          String ContactNumber = rs.getString("contact_number");
							          String Comments = rs.getString("comments");
							          int IsPotentialOutlet = rs.getInt("is_potential_outlet");	
									  String PotentialStatus="";
									  String PotentialStyle="";
									
										
										if(IsPotentialOutlet==1){
							         ...
							         <tr style="background:#ececec">
							           	   ...
							           	   <th align="left"><b>Outlet </b> </th>
						                   <th align="left"><b>Shopkeeper </b> </th>
				                           <th align="left"><b>Channel </b></th>
				                           <th align="left"><b>Address </b> </th>
							         </tr> 
							    
							          <tr >
					                       <td align="left"> <%=Outlet %> </td>
						                   <td align="left"> <%=ShopkeeprName %> </td>
							           ...
							           <tr style="background:#ececec">
				                           ...
				                           <th align="left"><b>Potential </b></th>
				                            <th align="left" colspan="2"><b>Comments</b> </th>
							           </tr>  
							           
							            <tr >
					                     
						                   <td align="left"> <%=ContactNumber %> </td>
				                           <td align="left"> <%=PotentialStatus %> </td>
					...
					<tr>
						...
						<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
							 ...
							 <%
								...
								//double TotalOrderWHTax = 0;
								//double TotalOrderNetAmount = 0;
								 ResultSet rs2122 = s.executeQuery("select unedited_order_id from mobile_order_sm where id="+OrderID);
								if (rs2122.first()){
									UneditedOrderID= rs2122.getLong(1);
								}
								
									long retailerOrderID=0;
									ResultSet rs212233 = s.executeQuery("select mobile_order_no from mobile_order_sm_unedited where id="+UneditedOrderID);
									if (rs212233.first()){
									...
									String StockQuery="SELECT * FROM mobile_retailer_sm_stock mrs join inventory_products_view ipv on mrs.product_id=ipv.product_id where mrs.order_no="+retailerOrderID;
									String ProductQuery ="SELECT ipv.product_id FROM mobile_retailer_sm_stock mrs join inventory_products_view ipv on mrs.product_id=ipv.product_id where mrs.order_no="+retailerOrderID;
								if(Flag==1){
									StockQuery="SELECT * FROM mobile_retailer_sm_stock mrs join inventory_products_view ipv on mrs.product_id=ipv.product_id where mrs.order_no="+retailerOrderID;
									ProductQuery ="SELECT ipv.product_id FROM mobile_retailer_sm_stock mrs join inventory_products_view ipv on mrs.product_id=ipv.product_id where mrs.order_no="+retailerOrderID;
								}else if(Flag==2){
									
									StockQuery="SELECT * FROM mobile_retailer_sm_stock mrs join inventory_products_view ipv on mrs.product_id=ipv.product_id where mrs.order_no="+MobileOrder;
									ProductQuery ="SELECT ipv.product_id FROM mobile_retailer_sm_stock mrs join inventory_products_view ipv on mrs.product_id=ipv.product_id where mrs.order_no="+MobileOrder;
								}
							...
							<%
								...
								ResultSet rsAllProducts = s.executeQuery("SELECT brand_label , package_label FROM inventory_products_view where category_id=1 AND product_id not in ("+ProductQuery+")");
							 while(rsAllProducts.next()){
								 %>
								 <tr>
							 		<td><%=rsAllProducts.getString("package_label") %></td>
							 		<td><%=rsAllProducts.getString("brand_label") %></td>
							 		<td>No</td>
					...
					<tr><td>&nbsp;</td></tr>
				     ...
				     ResultSet rs51 = s2.executeQuery("SELECT * FROM mobile_order_sm_unedited where id in (select unedited_order_id from mobile_order_sm mo where mo.id="+OrderID+")");
				     while(rs51.next()){
			...
			if(IsMapShow3 && latt3 !=0 && lang3 !=0){
				%>
				<ul data-role="listview" data-inset="true"  style="font-size:10pt; font-weight: normal; margin-top:-5px;" data-icon="false">
				
					<li data-role="list-divider" data-theme="a">Prospect Location</li>
					<li>
				    	...
				    	function initMap(){			    		
				    		...
				    		map.setCenter(new google.maps.LatLng(<%=latt3%>,<%=lang3%>));
				    				  
				    			  
				    			  markers[0] = new google.maps.Marker({
				    			      position: new google.maps.LatLng(<%=latt3%>,<%=lang3%>),
				    			      map: map,
				    			      title: '<%=OutletName%>'
