...
<li data-role="list-divider" data-theme="a">Promotions Packages Distributors</li>
<li>
			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					 ...
					 <thead>
					    <tr style="font-size:11px;">
							...
							<th data-priority="1"  style="text-align:left">Address</th>
							<%
							int PackageCount = 0;
							...
							ResultSet rs = s2.executeQuery("SELECT  distinct package_id, package_label, unit_per_sku FROM inventory_products_view where product_id in (select isap.product_id from inventory_sales_adjusted isa join inventory_sales_adjusted_products isap ON isa.id = isap.id where isa.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+") "+WherePackage+" order by package_sort_order");
							int  PacakgeID=0;
							int OutletID=0;
							double GrandTotal=0;
							while(rs.next()){
					...
					<tbody>
						...
						ResultSet rs1 = s.executeQuery("select codv.outlet_id,co.name,co.address from common_outlets_distributors_view codv,common_outlets co where co.id = codv.outlet_id and codv.distributor_id in ("+DistributorIDs+")  "+WherePJP+WhereHOD+WhereRSM); //distributor query
						while(rs1.next()){
						...
						<tr>
							...
							PacakgeID=rs.getInt("package_id");
								...
								ResultSet rs3 = s3.executeQuery("select "+    
										"sum(if (isap.is_promotion = 0,total_units,0)) sale_amount "+
										",sum(if (isap.is_promotion = 1,total_units,0)) sale_amount_promotion "+								        
								",ipv.unit_per_sku "+
								"from "+
								    ...
								    "inventory_sales_adjusted_products isap ON isa.id = isap.id "+
								        "join "+
								    "inventory_products_view ipv ON isap.product_id = ipv.product_id "+
								"where isa.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and isa.outlet_id = "+OutletID+" and ipv.package_id ="+PacakgeID+WherePackage+WhereBrand);
								
								
								
								while(rs3.next()){
									long sales_amount = rs3.getLong("sale_amount");
									long sales_amount_promotion = rs3.getLong("sale_amount_promotion");
							...
							%>
								...
								<td style="text-align: right;background-color:#F6F6F6;"><%= Utilities.convertToRawCases(sales_amount_promotion,unit_per_sku) %></td>
								<td style="text-align: right;"><%= Utilities.convertToRawCases(sales_amount+sales_amount_promotion,unit_per_sku) %></td> 
							<%
								}
