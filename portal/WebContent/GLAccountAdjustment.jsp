...
ResultSet rs = s.executeQuery("SELECT id, label FROM gl_cash_receipts_types order by id");
while(rs.next()){
	AccountOptions += "<option value="+rs.getString("id")+">"+rs.getString("label")+"</option>";
}

String TypeOptions = "<option value='0'>Select</option>";
ResultSet rs2 = s.executeQuery("SELECT id, label FROM gl_customer_account_adjustment_types order by id");
while(rs2.next()){
	TypeOptions += "<option value="+rs2.getString("id")+">"+rs2.getString("label")+"</option>";
}
...
<head>
		...
		<script src="js/GLAccountAdjustment.js"></script>
		
</head>

<body>

<div data-role="page" id="GLAccountAdjustmentPage" data-url="GLAccountAdjustmentPage" data-theme="d">

    <jsp:include page="Header2.jsp" >
    	<jsp:param value="Account Adjustment" name="title"/>
    </jsp:include>
				...
				<ul data-role="listview" data-inset="false" data-divider-theme="c" style="margin-top:20px; margin-left:2px;">
				    ...
				    <li>
					    ...
					    <table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
							...
							<tr>
								<td style="width: 20%; border: 0px; padding-right: 2px">
									<label for="AccountTypeID" data-mini="true">Account</label>
									<select name="AccountTypeID" id="AccountTypeID" data-mini="true" >
										<%=AccountOptions%>
									</select>
								</td>
								<td style="width: 20%; border: 0px; padding-right: 2px">
			...
			<table style="width: 100%;">
				<tr>
					<td>
						...
						<button data-icon="check" data-theme="b" data-inline="true" onClick="javascript:window.location='GLAccountAdjustment.jsp'" >Reset</button>
					</td>
				</tr>
			</table>
		</div>
