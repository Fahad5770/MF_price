...
<html>
  ...
  $(".WorkflowAttachButton").on( "click", function( event, ui ) {
   var d = new Date(),     
      n = d.getMonth() + 1,
      y = d.getFullYear();
  $("#month1").val($("#Month option:selected").val());
  $("#year1").val($("#Year option:selected").val());
  $("#userid").val(<%=SessionUserID%>);
alert
  if($("#month1").val()==0 || $("#year1").val()==0){alert("Please Select Month and Year");}
  else {
   $("#popup_workflow_attach" ).popup( "open", { transition: "slidedown",positionTo: "window" } );
  }
  });
  
  localhost:8080/portal/AuthenticateUser
     var manualuploader = $('#manual-fine-uploader').fineUploader({
         ...
         request: {
          endpoint: 'employee/UploadFileDistributorTargets',
          paramsInBody: true,
             params: {
              RequestID: 123,
              Months: function(){
...
.on('complete', function(event, id, name, responseJSON){
      if(responseJSON.Success=='true'){
       ...
       window.location="DistributorTargetsUpload.jsp";
      }else{
...
<div data-role="page" id="PSRTargets" data-url="PSRTargets" data-theme="d">
    ...
    <jsp:include page="Header2.jsp" >
    	<jsp:param value="Distributor Primary Targets" name="title"/>
    </jsp:include>
    <!-- /header -->
		...
		<ul data-role="listview" data-inset="false" data-divider-theme="c">
			...
			<li>
			        ...
			        <table style="width:70%" border="0">
			        	...
			        	<tr>
			        		<td style="width:20px; font-weight:normal;">Select CSV file to upload Targets</td>
