					...
					<tbody>
					    ...
					    <tr style="font-size:11px;">							
							...
							<%
								...
								ResultSet rs2 = s3.executeQuery("select distributor_id,name from common_distributors where distributor_id in (select distinct(customer_id) from gl_cash_receipts where created_by="+SessionUserID+" and created_on between "+Utilities.getSQLDateLifting(StartDate)+" and "+Utilities.getSQLDateNextLifting(EndDate)+")  "+WhereDistributors+WhereASM+WhereHOD+WhereRSM+WhereSM+WhereTDM);
								while(rs2.next()){	
								...
								while(rs.next()){	
									...
									ResultSet rs1 = s2.executeQuery("SELECT glcr.id,glcr.customer_id,(select name from common_distributors cd where cd.distributor_id=glcr.customer_id) customer_name,glcr.receipt_type,glcr.created_on,"+
												...
												" on glcr.id=glcri.id where 1=1 and glcri.instrument_id="+rs.getLong("id")+" and glcr.customer_id="+rs2.getLong("distributor_id")+WhereWarehouse+" and created_on between "+Utilities.getSQLDateLifting(StartDate)+" and "+Utilities.getSQLDateNextLifting(EndDate)+" and glcr.created_by="+SessionUserID+" group by glcri.instrument_id ");
