...
function redirect(DistributorID, SapOrderNo){
	...
	//alert(SapOrderNo);
	var url = 'ReportCenterR111LiftingSummary.jsp?SapOrderNo='+SapOrderNo+'&UniqueSessionID=<%=UniqueSessionID%>';
	/*if(OrderID == ''){
	...
	function OrderBookerGetOrderActivityReport(OrderID){
		...
		var url = 'ReportCenterR111LiftingSummary.jsp?OrderID='+OrderID+'&UniqueSessionID=<%=UniqueSessionID%>';
		if(OrderID == ''){
			url = 'ReportCenterR111LiftingSummary.jsp?UniqueSessionID=<%=UniqueSessionID%>';
		}
		$("#OrderActivityReportTD").html("<img src='images/snake-loader.gif'>");
	...
	<tr>
		<td style="width: 40%" valign="top">
			...
			<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="c" data-count-theme="c" style="margin-top:-13px;">
				...
				//System.out.println("select idn.delivery_id,idn.is_delivered,idn.is_received, idn.distributor_id,sap_order_no,idn.created_on,created_by,concat(first_name,' ',last_name) name,sum(idnp.liquid_in_ml)/ipa.conversion_rate_in_ml converted_cases,idn.invoice_amount from users u  ,inventory_delivery_note idn,  common_distributors cd,inventory_delivery_note_products idnp, inventory_packages ipa,inventory_products ip where  idn.delivery_id=idnp.delivery_id and idnp.product_id=ip.id and ip.package_id=ipa.id and ip.category_id = 1 and u.id=idn.created_by and idn.distributor_id = cd.distributor_id and idn.created_on between "+Utilities.getSQLDateLifting(StartDate)+" and "+Utilities.getSQLDateNextLifting(EndDate)+ " and idn.distributor_id in ("+DistributorIDs+") group by idn.delivery_id order by created_on desc");
				...
				while(rs22.next()){
					...
					if(!DateUtils.isSameDay(LastDate, rs22.getDate("created_on")) ){
						...
						ResultSet rs3 = s2.executeQuery("select sum(invoice_amount) sub_total from inventory_delivery_note idn where idn.created_on between "+Utilities.getSQLDateLifting(rs22.getDate("created_on"))+" and "+Utilities.getSQLDateNextLifting(rs22.getDate("created_on"))+" and idn.distributor_id in ("+DistributorIDs+")");
	            		if(rs3.first()){
					...
					%>
					<li><a data-ajax="false" href="#" onclick="redirect(<%=rs22.getString("idn.distributor_id")%>, <%=rs22.getString("sap_order_no")%>)" style="font-size:13px; font-weight:normal;">Order# <%=rs22.getString("sap_order_no") %><br/><%=Utilities.getDisplayDateTimeFormat(rs22.getTimestamp("created_on"))  %>&nbsp;<%
		...
		<%}else{ %>
				<p style="padding:20px;">Please select at least one distributor</p>
			<%} %>	
	</tr>
</table>
