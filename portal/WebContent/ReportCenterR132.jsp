			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					 <thead>
					    <tr style="font-size:11px;">
							...
							ResultSet rs = s2.executeQuery("SELECT distinct package_id, package_label, unit_per_sku FROM inventory_products_view where  product_id in (SELECT distinct isap.product_id FROM inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id = isap.id where isa.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDate(EndDate)+") "+WherePackage+ "order by package_sort_order");
							int PacakgeID=0;
							int DistributorID=0;
							double GrandTotal=0;
							while(rs.next()){
					...
					<tbody>
						...
						<tr>
							...
							ResultSet rs6 = s3.executeQuery("select "+    
										"sum(if (isap.is_promotion = 0, ((total_units*ipv.liquid_in_ml)/ipv.conversion_rate_in_ml ) ,0)) sale_amount "+																		        
								...
								"where isa.adjusted_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and isa.distributor_id = "+DistributorID);
							...
							PacakgeID=rs.getInt("package_id");
								...
								"from "+
								    ...
								    "inventory_sales_adjusted_products isap ON isa.id = isap.id "+
								        "join "+
								    "inventory_products_view ipv ON isap.product_id = ipv.product_id "+
								"where isa.adjusted_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and isa.distributor_id = "+DistributorID+" and ipv.package_id ="+PacakgeID+WherePackage);
