		...
		<td style="width: 100%" valign="top">
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					...
					<tbody>
						...
						//System.out.println("select * from common_distributors where 1=1 "+WhereHOD+WhereRSM+WhereSM+WhereTDM+WhereASM+" and distributor_id in ("+DistributorIDs+") and distributor_id in (select distinct distributor_id from inventory_delivery_note where created_on between "+Utilities.getSQLDateLifting(StartDate)+" and "+Utilities.getSQLDateNextLifting(EndDate)+")");
						ResultSet rs1 = s.executeQuery("select * from common_distributors where 1=1 "+WhereCustomerID+" and distributor_id in ("+DistributorIDs+") and distributor_id in (select distinct distributor_id from primary_sales_orders where created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+")"); //distributor query
						while(rs1.next()){
						...
						<tr>
								...
								int UnitPerSKUCon=0;
								ResultSet rs23 = s3.executeQuery("select sum(total_units*ipv.liquid_in_ml)/6000 converted,ipv.unit_per_sku from primary_sales_orders pso join primary_sales_orders_products psop on pso.id = psop.id join inventory_products_view ipv on psop.product_id = ipv.product_id where pso.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and pso.distributor_id = "+DistributorID+" and ipv.category_id = 1");
								if(rs23.first()){
