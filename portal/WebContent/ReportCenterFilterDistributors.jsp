...
<script>
        ...
        <fieldset data-role="controlgroup" data-corners="true" style="margin-left: 0px; margin-top: -10px; padding-top: 0px;">
           ...
           if (session.getAttribute(UniqueVoID+"_SR1SelectedDistributors") != null){
            ...
            System.out.println("select distributor_id, name from common_distributors where distributor_id in (SELECT distinct distributor_id FROM inventory_delivery_note) and is_active=1 and distributor_id in ("+DistributorIds+") and type_id=2 "+WhereHOD+WhereRSM);
           //for(int x=0;x<UserDistributor.length;x++){
        	   ResultSet rs2 = s.executeQuery("select distributor_id, name from common_distributors where distributor_id in (SELECT distinct distributor_id FROM inventory_delivery_note)  and is_active=1 and distributor_id in ("+DistributorIds+") and type_id=2 "+WhereHOD+WhereRSM);
