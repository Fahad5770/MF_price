...
$( document ).delegate("#CreditSlipPage", "pageinit", function() {
	if(isEditCaseJS){
		//getOutletName();
...
function CreditSlipSubmit(){
	...
	$.mobile.loading( 'show');
	$.ajax({
	    url: "sampling/CreditSlipExecute",
	    ...
	    success: function( json ) {
	    	...
	    	if (json.success == "true"){
	    		window.location = 'SamplingCreditSlip.jsp';
	    	}else{
	    		alert(json.error);
	    	}
...
function getOutletName(){
	...
	$.ajax({
		...
		success:function(json){
			...
			if(json.exists == "true"){
				$('#OutletName').html(json.OutletName);
				$('#isValidOutlet').val('1');
				getOutletMasterInfo();
			}else{
				$.mobile.hidePageLoadingMsg();
				$('#isValidOutlet').val('0');
				$('#OutletID').val('Outlet not found');
				$('#OutletName').html('');
				$('#OutletAddress').html('');
			}
		},
...
function showSearchContent(){
	...
	$.get('SamplingCreditSlipSearch.jsp?FromDate='+$('#FromDate').val()+'&ToDate='+$('#ToDate').val(), function(data) {
...
function AddRow(){
	...
	for(var i = 0; i < RowLength; i++){
		if( $('input[name=OutletIDList]')[i].value == OutletID ){
	...
	$.ajax({
		...
		success:function(json){
			...
			if(json.exists == "true"){
				...
				content += "<td><input type='hidden' name='OutletIDList' id='OutletIDList' value='"+OutletID+"' >"+OutletID+" - "+json.OutletName+"</td>";
				content += "<td><input type='hidden' name='AmountList' id='AmountList' value='"+Amount+"' >"+Amount+"</td>";
...
function EditCreditSlip(EditIDParam){
	...
	$.mobile.loading( 'show');
	$.ajax({
	    url: "sampling/CreditSlipInfoJson",
	    data: {
	    	EditID: EditIDParam
	    },
	    ...
	    success: function( json ) {
	    	...
	    	if (json.success == "true"){
	    		...
	    		for(var i = 0; i < json.rows.length; i++){
	    			...
	    			var content = "<tr id='SamplingCreditSlip"+RowMaxID+"'>";
	    			content += "<td><input type='hidden' name='OutletIDList' id='OutletIDList' value='"+json.rows[i].OutletID+"' >"+json.rows[i].OutletID+" - "+json.rows[i].OutletName+"</td>";
...
<div data-role="page" id="CreditSlipPage" data-url="CreditSlipPage" data-theme="d">
    ...
    <div data-role="header" data-theme="c" data-position="fixed">
	    <div>
		    ...
		    <div style="float:left; width:90%;b1ackground-color:Red; text-align:center;">
		    	<h1 style="font-size: 14pt;float:left; margin-left:45%; text-align:center;">Credit Slip</h1>
			</div>
		</div>
	</div>
	...
	<div data-role="content" data-theme="d">
									...
									<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
										<tr>
											<td style="border: 0px" colspan="2" style="width: 50%">
												<label for="SlipDescription" data-mini="true">Slip Description (Printable)</label>
				...
				<table style="width: 100%">
					<tr>
						<td>
							...
							<button data-icon="check" data-theme="b" data-inline="true" id="CreditSlipReset" onClick="javascript:window.location='SamplingCreditSlip.jsp'" >Reset</button>
						</td>
        ...
        <div id="SearchContent">
		 ...
		 <jsp:include page="LookupOutletSearchPopup.jsp" > 
	    	...
	    	<jsp:param value="151" name="OutletSearchFeatureID" />
