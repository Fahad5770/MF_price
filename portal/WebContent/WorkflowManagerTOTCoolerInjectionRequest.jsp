...
<head>
	...
	<script src="js/WorkflowManagerTOTCoolerInjectionRequest.js?131=131"></script>
  ...
  long RequestID = Utilities.parseLong(request.getParameter("requestID"));
   ...
   %>
    ...
    <jsp:include page="Header2.jsp" >
    	<jsp:param value="Cooler Injection Request" name="title"/>
    </jsp:include>
     <!-- /header -->
 <script>
        ...
        <fieldset class="ui-grid-a">
		        ...
		        <form name="WorkflowContainerForm" class="WorkflowForm" id="WorkflowContainerForm" method="POST" action="#">
					<ul data-role="listview" data-inset="true" style="margin-left: 22px;margin-top: 6px;margin-right: 5px;">
						<li data-role="list-divider">Outlet Info</li>
						<li>
						...
						<table border="0" style="width: 100%; margin-top: 0px; padding: 0px;" class="workflow_form_font">
							...
							<tr>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">Outlet ID</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">Outlet Name</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">Outlet Address</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">Contact Number</th>	
								</tr>
								<tr style="b1ackground-color:#efedee">
									<td style="width:25%; padding:5px;" valign="top"><input type="text" name="NewOutletID" id="NewOutletID" value="<%=rs.getLong("outlet_id") %>" data-mini="true" <%if(bean.CURRENT_STEP.STEP_ID!=2){ %>readonly<%} %>/></td>
									<td style="width:25%; padding:5px;"><%=rs.getString("outlet_name") %></td>
									<td style="width:25%; padding:5px;"valign="top"><%=rs.getString("outlet_name") %></td>
									<td style="width:25%; padding:5px;"valign="top"><%=rs.getString("outlet_contact_number") %></td>
								</tr>
								<tr>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">Contact Name</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">CNIC</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">Channel</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">Region</th>	
								</tr>
								
								<tr style="b1ackground-color:#efedee">
									<td style="width:25%; padding:5px;" valign="top"><%=rs.getString("outlet_contact_name") %></td>
									<td style="width:25%; padding:5px;"><%=rs.getString("outlet_contact_cnic") %></td>
									<td style="width:25%; padding:5px;"valign="top"><%if(rs.getString("channel_name")!=null){%><%=rs.getString("channel_name") %><%} %></td>
									<td style="width:25%; padding:5px;"valign="top"><%=rs.getString("region_name") %></td>
								</tr>
								<tr>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">RSM</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">SND</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">Distributor</th>
									
								</tr>	
								<tr style="b1ackground-color:#efedee">
									<td style="width:25%; padding:5px;" valign="top"><%=rs.getString("outlet_rsm_name") %></td>
									<td style="width:25%; padding:5px;"><%=rs.getString("outlet_snd_name") %></td>
									<td style="width:25%; padding:5px;"valign="top"><%=rs.getString("outlet_distributor_name") %></td>
						...
						<li data-role="list-divider">Sales Volume-P/C: Per Year</li>
						<li>
							<table border="0" style="width: 100%; margin-top: 0px; padding: 0px;" class="workflow_form_font">
							...
							<thead>
								...
								<tr>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">LRB Types</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">Package</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px">Quantity</th>
							...
							<%
							ResultSet rs12 = s.executeQuery("SELECT id,type,(select label from inventory_products_lrb_types iplt where iplt.id=type) type_name,package_id,(select label from inventory_packages ip where ip.id=package_id) package_name,quantity FROM tot_issue_request_sales where id="+MasterParentTableID);
							while(rs12.next()){
							%>
							
							
									<tr style="b1ackground-color:#efedee">
									<td style="width:25%; padding:5px;"valign="top"><%=rs12.getString("type_name") %></td>
									<td style="width:25%; padding:5px;"><%=rs12.getString("package_name") %></td>
									<td style="width:25%; padding:5px;"valign="top"><%=rs12.getInt("quantity") %></td>
						...
						<li data-role="list-divider">Beat Plan</li>
						<li>
						<table border="0" style="width: 100%; margin-top: 0px; padding: 0px;" class="workflow_form_font">
						
							<tbody>
							<%
							boolean SundayCheckd=false;
							boolean MundayCheckd=false;
							boolean TuedayCheckd=false;
							boolean WeddayCheckd=false;
							boolean ThrsdayCheckd=false;
							boolean FriayCheckd=false;
							boolean SatdayCheckd=false;
							
							//System.out.println("SELECT * FROM pep.tot_issue_req_beat_plan where id="+MasterParentTableID);
							
							ResultSet rs14 = s.executeQuery("SELECT * FROM tot_issue_request_beat_plans where id="+MasterParentTableID);
							while(rs14.next()){
								
								if(rs14.getInt("day_number")==1){
									SundayCheckd=true;
								}
								
								if(rs14.getInt("day_number")==2){
									MundayCheckd=true;
								}
								
								if(rs14.getInt("day_number")==3){
									TuedayCheckd=true;
								}
								
								if(rs14.getInt("day_number")==4){
									WeddayCheckd=true;
								}
								
								if(rs14.getInt("day_number")==5){
									ThrsdayCheckd=true;
								}
								
								if(rs14.getInt("day_number")==6){
									FriayCheckd=true;
								}
								
								if(rs14.getInt("day_number")==7){
									SatdayCheckd=true;
								}
							...
							<tr>
							 	...
							 	<td style="width:14%;"> 
							 		<input type="checkbox" name="outletbeatplan" id="BeatplanD7" data-mini="true" value="7" <%if(SatdayCheckd){ %>checked<%} %>><label for="BeatplanD7">Saturday</label>
							 	</td>
							 	<td style="width:14%;"> 
							 		<input type="checkbox" name="outletbeatplan" id="BeatplanD1" data-mini="true" value="1" <%if(SundayCheckd){ %>checked<%} %>><label for="BeatplanD1">Sunday</label>
							 	</td>
							 	<td style="width:14%;"> 
							 		<input type="checkbox" name="outletbeatplan" id="BeatplanD2" data-mini="true" value="2" <%if(MundayCheckd){ %>checked<%} %>><label for="BeatplanD2">Monday</label>
							 	</td>
							 	<td style="width:14%;"> 
							 		<input type="checkbox" name="outletbeatplan" id="BeatplanD3" data-mini="true" value="3" <%if(TuedayCheckd){ %>checked<%} %>><label for="BeatplanD3">Tuesday</label>
							 	</td>
							 	<td style="width:14%;"> 
							 		<input type="checkbox" name="outletbeatplan" id="BeatplanD4" data-mini="true" value="4" <%if(WeddayCheckd){ %>checked<%} %>><label for="BeatplanD4">Wednesday</label>
							 	</td>
							 	<td style="width:14%;"> 
							 		<input type="checkbox" name="outletbeatplan" id="BeatplanD5" data-mini="true" value="5" <%if(ThrsdayCheckd){ %>checked<%} %>><label for="BeatplanD5">Thursday</label>
							 	</td>
							 	<td style="width:14%;"> 
							 		<input type="checkbox" name="outletbeatplan" id="BeatplanD6" data-mini="true" value="6" <%if(FriayCheckd){ %>checked<%} %>><label for="BeatplanD6">Friday</label>
							 	</td>
		 	</tr>
						...
						<li data-role="list-divider">TOT info</li>
						<li>
						<table border="0" style="width: 100%; margin-top: 0px; padding: 0px;" class="workflow_form_font">
		 	...
		 	<%
		 	String EmptyStatus1 ="";
		 	int ExistingTOTSize1=0;
		 	String TOTCode1="";
		 	
		 	ResultSet rs16 = s.executeQuery("select * from tot_issue_request where request_id="+RequestID);
		 	if(rs16.first()){
		 		EmptyStatus1 = rs16.getString("tot_empty_status");
		 		ExistingTOTSize1 = rs16.getInt("existing_tot");
		 		TOTCode1 = rs16.getString("tot_code");
		 	}
		 	
		 	
		 	%>
		 	
		 		<td><input type="text" name="EmptyStatus" data-mini="true" value="<%=EmptyStatus1%>"/></td>
		 		<td>
		 			<select name="ExistingTOTSize" data-mini="true">
		 				<%
		 				ResultSet rs15 = s.executeQuery("SELECT * FROM common_assets_tot_sizes");
		 				while(rs15.next()){
		 				%>
		 				<option value="<%=rs15.getInt("id")%>" <%if(ExistingTOTSize1==rs15.getInt("id")){ %>selected<%} %>><%=rs15.getString("label") %></option>
		 				<%
		 				}
		 				%>
		 		...
		 		<td><input type="text" name="TOTCode" data-mini="true" value="<%=TOTCode1%>"/></td>
						...
						<li data-role="list-divider">Required TOT</li>
						<li>
						<table border="0" style="width: 100%; margin-top: 0px; padding: 0px;" class="workflow_form_font">
							...
							<%
		 	ResultSet rs17 = s.executeQuery("SELECT id,tot_size_id,(select label from common_assets_tot_sizes cats where cats.id=tot_size_id) tot_size_name,quantity FROM tot_issue_request_sizes where id="+MasterParentTableID);
		 	while(rs17.next()){		 		
		 	...
		 	<tr>
		 		<td ><input type="checkbox" name="TOTSize" id="TOTSize<%=rs17.getInt("tot_size_id") %>" data-mini="true" value="<%=rs17.getInt("tot_size_id") %>" checked><label for="TOTSize<%=rs17.getInt("tot_size_id") %>"><%=rs17.getString("tot_size_name") %></label></td>
		 		<td><input type="text" data-mini="true" name="TOTSizeQuantity<%=rs17.getInt("tot_size_id") %>" value="<%=rs17.getInt("quantity") %>"/></td>
		 	</tr>
		 	...
		 	%>
			<tr>
		 	<td><input type="text" placeholder="Asset Number" data-mini="true" name="TOTAssetNumber" value="" <%if(bean.CURRENT_STEP.STEP_ID!=6){ %>readonly<%} %>/></td>
		 	<td></td>
