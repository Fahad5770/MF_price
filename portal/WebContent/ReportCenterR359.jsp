...
function ExportToExcel(D1,D2,DistributorIDs,OrderBookersIDs,TSOIDs,ASMIDs,RegionIDs,ChannelIDs,CityIDs){
	//alert(D1);
	
	$.mobile.showPageLoadingMsg();
	$.ajax({
		
		url: "reports/R359ExcelMain",
		data: {
			StartDate: D1,
			EndDate: D2,
...
<ul data-role="listview" data-inset="true"
	...
	data-icon="false">
	<li data-role="list-divider" data-theme="a">KPIs Report</li>
	<li>
							...
							<tr>
								...
								<td style="text-align: center">
									...
									<div id="aExcelFileReady">
										<a
											onclick="ExportToExcel(<%=Utilities.getSQLDate(StartDate)%>, <%=Utilities.getSQLDate(EndDate)%>, '<%=DistributorIDs%>', '<%=OrderBookersIDs%>','<%=TSOIDs%>','<%=ASMIDs%>','<%=RegionIDs%>', '<%=ChannelIDs%>','<%=CityIDs%>')"
