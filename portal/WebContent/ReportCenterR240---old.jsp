...
function DirectCreditFlow(ReqID,StDate,EnDate){
	alert(StDate);
	
	$.mobile.loading( 'show');	
	$.ajax({
    		url: "cash/PerCaseDiscountApprovalExecute",		    
    		data: {
    			RequestID: ReqID,
    			StartDate: StDate,
    			EndDate: EnDate
			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					   ...
					   <th data-priority="1"  style="text-align:center; ">&nbsp;</th>
					    ...
					    <tr style="font-size:11px;">
							...
							while(rs12.next()){
								...
								ResultSet rs2 = s2.executeQuery("SELECT distinct package_id, package_label FROM inventory_products_view where  package_id in (SELECT distinct package_id FROM inventory_primary_percase_request ippr join inventory_primary_percase_request_products ipprp on ippr.id=ipprp.id where ippr.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+") "+WherePackage+" and category_id = 1 and lrb_type_id="+rs12.getLong("lrb_type_id")+" order by package_sort_order");
								while(rs2.next()){
							%>
							...
							while(rs121.next()){
								...
								ResultSet rs212 = s2.executeQuery("SELECT distinct package_id, package_label FROM inventory_products_view where  package_id in (SELECT distinct package_id FROM inventory_primary_percase_request ippr join inventory_primary_percase_request_products ipprp on ippr.id=ipprp.id where ippr.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+") "+WherePackage+" and category_id = 1 and lrb_type_id="+rs121.getLong("lrb_type_id")+" order by package_sort_order");
								while(rs212.next()){
									%>
					...
					<tbody>
						...
						<tr>
							...
							<%
							ResultSet rs122 = s2.executeQuery("SELECT count(distinct package_id) package_count,lrb_type_id,(select label from inventory_products_lrb_types iplt where iplt.id=lrb_type_id) type_name FROM inventory_products_view where  package_id in (SELECT distinct package_id FROM inventory_primary_percase_request ippr join inventory_primary_percase_request_products ipprp on ippr.id=ipprp.id where ippr.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDate(EndDate)+") and category_id = 1 and lrb_type_id is not null group by lrb_type_id");
							while(rs122.next()){
								
								ResultSet rs2 = s3.executeQuery("SELECT distinct package_id, package_label FROM inventory_products_view where  package_id in (SELECT distinct package_id FROM inventory_primary_percase_request ippr join inventory_primary_percase_request_products ipprp on ippr.id=ipprp.id where ippr.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDate(EndDate)+") "+WherePackage+" and category_id = 1 and lrb_type_id="+rs122.getLong("lrb_type_id")+" order by package_sort_order");
								while(rs2.next()){
							...
							<td><input type="button" value="Release" onClick="DirectCreditFlow('<%=RequestID %>',<%=Utilities.getSQLDate(StartDate) %>,<%=Utilities.getSQLDate(EndDate) %>)" data-mini="true"/></td>
