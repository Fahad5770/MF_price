...
<script>
           ...
           //System.out.println(DistributorIds);
            ResultSet rs = s.executeQuery("select * from users where IS_ACTIVE=1");
            ...
            if (session.getAttribute(UniqueSessionID+"_SR1UserSelectedEmployees") != null){
            	SelectedEmployeeArray = (long[])session.getAttribute(UniqueSessionID+"_SR1UserSelectedEmployees");           	
            }
            while(rs.next()){
            	...
            	%>    		
           			<input type="checkbox" name="UserEmployeeCheckBox" id="EmployeeID_<%=rs.getString("id") %>" value="<%=rs.getString("id") %>" <%if(IsEmployeeSelected){ %>checked<%} %> data-mini="true">
   					<label for="EmployeeID_<%=rs.getString("id") %>" ><span style="font-size:9pt; font-weight:normal;"><%=rs.getString("id") %> - <%=rs.getString("DISPLAY_NAME") %></span></label> 
