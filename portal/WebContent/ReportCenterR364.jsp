...
// System.out.println("ASM Filter : "+ASMIDs);
...
function ExportToExcel(D1,D2,DistributorIDs,OrderBookersIDs,ASMIDs,TSOIDs,RegionIDs,CityIDs, BrandIDs, SKUIDs){
	//alert(D1);
	...
	$.ajax({
		...
		url: "reports/R364ExcelMain",
		data: {
			StartDate: D1,
			EndDate: D2,
			Distributors : DistributorIDs,
			OrderBooker :OrderBookersIDs,
			ASM : ASMIDs,
			TSO :TSOIDs,
			Region :RegionIDs,
	        City :CityIDs,
							...
							<tr>
								<td style="text-align: center;width:75%;">
   									...
   									<div id="aExcelFileReady">
   										<a onclick="ExportToExcel(<%=Utilities.getSQLDate(StartDate) %>, <%=Utilities.getSQLDate(EndDate) %>, '<%=DistributorIDs %>', '<%=OrderBookersIDs %>',
   										 '<%=ASMIDs %>','<%=TSOIDs %>','<%=RegionIDs %>','<%=CityIDs %>', '<%=BrandIDs %>', '<%=SKUIDs %>'
