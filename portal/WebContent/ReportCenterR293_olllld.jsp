...
<script type="text/javascript">
	...
	var url = 'ReportCenterR238CensusSummary.jsp?CensusID='+PjpID+'&UniqueSessionID=<%=UniqueSessionID%>';
	...
	<tr>
		...
		<td style="width: 30%" valign="top">
			...
			<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="c" data-count-theme="c" style="margin-top:-13px;">
				...
				ResultSet rs22 = s.executeQuery("SELECT * FROM mrd_census where mobile_timestamp between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+WhereCensusUser+" and is_deleted!=1 order by id desc");
				int counter = 0;
				int TotalOutlets =0;
				String OutletName="";
				while(rs22.next()){
					...
					if(rs22.getInt("outlet_id")==0 || rs22.getString("outlet_name").equals("No Outlet Found")){
						if(!rs22.getString("outlet_name_actual").equals("")){
							OutletName = rs22.getString("outlet_id")+" - "+rs22.getString("outlet_name_actual");
						}else{
							OutletName = rs22.getString("outlet_id")+" - "+rs22.getString("outlet_board_name");
						}
