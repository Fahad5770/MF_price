...
String WhereHOD = "";
if (HODIDs.length() > 0){
	WhereHOD = " and cache_distributor_id in (SELECT distributor_id FROM common_distributors where snd_id in ("+HODIDs+")) ";	
}
...
String WhereRSM = "";
if (RSMIDs.length() > 0){
	WhereRSM = " and cache_distributor_id in (SELECT distributor_id FROM common_distributors where rsm_id in ("+RSMIDs+")) ";	
}
							...
							<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
									 <thead>
									    <tr style="font-size: 11px;">
											...
											<th data-priority="1"  style="text-align:center; width: 10%" >Last Order</th>
								...
								ResultSet rs = s.executeQuery("select outlet_id, max_date, datediff(now(),max_date) diff,co.name,co.address, concat(cache_contact_name,' ',cache_contact_number) outlet_contact , (select concat(id,'-',display_name) from users where id = co.cache_orderbooker_id) order_booker from ("+
																"select outlet_id, max(imo.created_on) max_date from mobile_order imo join common_outlets ico on imo.outlet_id = ico.id  where 1=1 "+WhereDistributors+WhereHOD+WhereRSM+" group by outlet_id"+
