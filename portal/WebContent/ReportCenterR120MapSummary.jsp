...
/////////////ResultSet rs = s.executeQuery("select mo.mobile_timestamp,mo.outlet_id,(select outlet_name from outletmaster om where mo.outlet_id=om.outlet_id) outlet_name, mo.lat,mo.lng,mo.net_amount from mobile_order mo where mo.lat !=0 and mo.lng !=0 and mo.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateTimeNext(EndDate)+WhereOrderBooker+" order by mo.created_on desc ");

//Query changed by Zulqurnan , zero outlet code added on 14/09/2020

ResultSet rs = s.executeQuery("Select * from ("
		+"select mo.mobile_timestamp,mo.outlet_id,(select outlet_name from outletmaster om where mo.outlet_id=om.outlet_id) outlet_name, mo.lat,mo.lng,mo.net_amount, 0 as zero, mo.created_on from mobile_order mo where mo.lat !=0 and mo.lng !=0 and mo.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateTimeNext(EndDate)+WhereOrderBooker
		+"union "
		+"select moz.mobile_timestamp,moz.outlet_id,(select outlet_name from outletmaster om where moz.outlet_id=om.outlet_id) outlet_name, moz.lat,moz.lng,0 as net_amount, 1 as zero, moz.created_on from mobile_order_zero moz   where created_by=935 and moz.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateTimeNext(EndDate)+WhereOrderBooker
