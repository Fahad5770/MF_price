...
System.out.println("OrderBookerIDs : " + OrderBookerIDs);


String OrderBookerIDsWher="";
if(OrderBookerIDs.length()>0){
	OrderBookerIDsWher =" and dbpav.assigned_to in ("+OrderBookerIDs+") ";
}
	...
	<tr>
		...
		<td style="width: 30%" valign="top">
			...
			<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="c" data-count-theme="c" style="margin-top:-13px;">
				...
				<%
				String checkHibernation = (SessionUserID != 204220064) ? " and dbpav.outlet_active=1" : "";
                System.out.println("SELECT distinct dbpav.id, label ,count(distinct outlet_id) outlet_count,assigned_to,assigned_to_name FROM distributor_beat_plan_all_view dbpav join common_outlets co on dbpav.outlet_id=co.id where 1=1 and outlet_id!=131100002  "+OrderBookerIDsWher+" "+checkHibernation+WhereDistributors+WhereHOD+WhereRSM+WhereDiscountedAll+WhereDiscountedFixed+WhereDiscountedPerCase+WhereNonDiscountedAll+WhereActive+WhereDeactivated+" group by dbpav.id order by outlet_count desc");
