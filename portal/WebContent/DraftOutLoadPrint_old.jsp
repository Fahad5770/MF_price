...
<body style="font-family:Helvetica,Arial,sans-serif" >
	<%
		 ...
		 SQLMain = "SELECT idol.id, idol.created_on, idol.distributor_id, (select name from common_distributors where distributor_id=idol.distributor_id) distributor_name,(SELECT DISPLAY_NAME FROM users where id=idol.created_by) user_name,idol.remarks,idol.invoice_no,idol.uvid,idol.sap_order_no from inventory_draft_outlet_load idol where is_printed=0 and idol.sap_order_no="+SapOrderNo;
	...
	%>
                ...
                <tr>
			  	...
			  	<table style="width: 100%;  padding-left: 10px;padding-right: 10px; font-size: 12px" border="0" cellpadding="0" cellspacing="0">
			  		<tr style="font-weight: 700; font-size:12px">
			  			...
			  			<td class="formattedRow" nowrap style="text-align: right;">Batch Code</td>
			  			
			  		</tr>
			  	<%
			  	//int PromotionCounter = 0;
			  	
			  	ResultSet rs2 = s2.executeQuery("SELECT idolp.draft_id, idolp.product_id, ipv.sap_code, ipv.package_id, ipv.package_label, ipv.brand_id, ipv.brand_label, idolp.total_units, idolp.raw_cases,idolp.batch_code, ipv.unit_per_sku FROM inventory_draft_outlet_load_products idolp, inventory_products_view ipv where idolp.product_id=ipv.product_id and idolp.draft_id="+InvoiceID+" order by  ipv.package_label, ipv.brand_label");
				while (rs2.next()){
			  	...
			  	%>
			  		<tr>
			  			...
			  			<td class="formattedRow" style="text-align: right; padding-right:5px;"><%= Utilities.convertToRawCases(Bottles, UnitPerSKU) %></td>
			  			<td class="formattedRow" style="text-align: right; padding-right:5px;"><%= Bottles%></td>
			  ...
			  out.println("This document print has been generated already.");
		  }
