			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					 <thead>
					    <tr style="font-size:11px;">
							...
							<th data-priority="1"  style="text-align:center; width: 25% ">Outlet Contact</th>
							<th data-priority="1"  style="text-align:center; width: 15% ">Complaint Type</th>
							<th data-priority="1"  style="text-align:center; width: 25% ">Description</th>							
					...
					ResultSet rs1 = null;
					ResultSet rs = s.executeQuery("SELECT * FROM crm_tasks ct where created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and ifnull((select '1' from crm_tasks_list ctl where ctl.id = ct.id and ((ctl.type_id >= 15 and ctl.type_id <= 20)) limit 1),0) = 1");
					while(rs.next()){
					%>
						...
						<tr>
							...
							<%
							ResultSet rs2 = s2.executeQuery("SELECT ctt.english_label FROM crm_tasks_list ctl join crm_tasks_types ctt on ctl.type_id = ctt.id where ctl.id = "+rs.getLong("id")+" and ctt.id >= 15 and ctt.id <= 20");
							while(rs2.next()){
							%>
