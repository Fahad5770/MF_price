...
function ExportToExcel(D1,D2,DistributorIDs, RegionIDs, ChannelIDs, CityIDs , OrderBookersIDs, ASMIDs, TSOIDs){
	//alert(D1);
	$.mobile.showPageLoadingMsg();
	$.ajax({
		
		url: "reports/R373ExcelMain",
		data: {
			StartDate: D1,
			EndDate: D2,
			...
			Channels : ChannelIDs,
			CityIDs : CityIDs,
			OrderBookersIDs : OrderBookersIDs,
			ASMIDs : ASMIDs,
			TSOIDs : TSOIDs,
		},	
		type:"POST",
							...
							<tr>
								<td style="text-align: center;width:75%;">
   									...
   									<div id="aExcelFileReady">
   										<a onclick="ExportToExcel(<%=Utilities.getSQLDate(StartDate) %>, <%=Utilities.getSQLDate(EndDate) %>, '<%=DistributorIDs %>', '<%=RegionIDs %>', '<%=ChannelIDs %>', '<%=CityIDs %>', '<%=OrderBookersIDs %>', '<%=ASMIDs %>','<%=TSOIDs %>',)" href="#"    data-theme="c" data-inline="true" style="margin-top: 0px; margin-left: 0px; margin-bottom: 0px">Export To Excel</a>
   								</div>
