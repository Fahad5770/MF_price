...
<script>
        ...
        <fieldset data-role="controlgroup" data-corners="true" style="margin-left: 0px; margin-top: -10px; padding-top: 0px;">
           ...
           if (session.getAttribute(UniqueVoID+"_SR1SelectedFromDistributors") != null){
           	SelectedPackagesArray = (long[])session.getAttribute(UniqueVoID+"_SR1SelectedFromDistributors");           	
           }
			...
			String WhereRSM = "";
			if (RSMIDs.length() > 0){
				WhereRSM = " and distributor_id in (SELECT distributor_id FROM common_distributors where snd_id in ("+RSMIDs+")) ";	
			}
            
            
            
            if (OrderBookerIds.length() == 0){
            System.out.println("select distributor_id, name from common_distributors where distributor_id in (SELECT distinct distributor_id FROM inventory_delivery_note) and distributor_id in ("+DistributorIds+") and type_id=2 "+WhereHOD+WhereRSM);
           //for(int x=0;x<UserDistributor.length;x++){
        	   ResultSet rs2 = s.executeQuery("select distributor_id, name from common_distributors where distributor_id in (SELECT distinct distributor_id FROM inventory_delivery_note) and distributor_id in ("+DistributorIds+") and type_id=2 "+WhereHOD+WhereRSM);
        	   ...
        	   if (session.getAttribute(UniqueVoID+"_SR1SelectedFromDistributors") != null){
            	...
            	%>    		
           			<input type="checkbox" name="FromDistributorCheckBox" id="DistributorID_<%=DistributorID %>" value="<%=DistributorID %>" <%if(CheckBoxSelected){ %>checked<%} %> onClick="ChangeFromLabelDistributor('<%=ClickIDD%>')" data-mini="true">
            	...
            	System.out.println("select distributor_id, name from common_distributors where distributor_id in("+
            	...
            	while(rs2.next()){
            		...
            		%>
           			<input type="checkbox" name="FromDistributorCheckBox" id="DistributorID_<%=rs2.getLong(1) %>" value="<%=rs2.getLong(1) %>" <%if(CheckBoxSelected){ %>checked<%} %> onClick="ChangeFromLabelDistributor('<%=ClickIDD%>')" data-mini="true">
