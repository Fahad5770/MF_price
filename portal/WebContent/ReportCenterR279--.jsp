			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					 <thead>
					    <tr style="font-size:11px;">
							...
							ResultSet rs = s2.executeQuery("select product_id,package_label from inventory_products_view ");
					...
					//	System.out.println("SELECT id FROM pep.common_outlets co where id in (select outlet_id from mobile_order_unedited where created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+")"+WherePJP+WhereDistributors);
						...
						while(rs1.next()){
							...
							while(rs11.next()){
								...
								ResultSet rs22 = s4.executeQuery("SELECT sum(raw_cases) rawcase FROM pep.mobile_retailer_stock mrs join mobile_order_unedited mou on mrs.order_no=mou.mobile_order_no join inventory_products_view ipv on mrs.product_id=ipv.product_id where outlet_id="+OutleID+" and created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and package_id="+PackID+OrderBookerIDsWhere); //distributor query
								while(rs22.next()){
