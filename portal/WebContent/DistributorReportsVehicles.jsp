...
<script>
           ...
           //System.out.println(DistributorIds);
            ResultSet rs = s.executeQuery("select * from distribtuor_vehicles where distributor_id in ("+DistributorIds+")");
            ...
            while(rs.next()){
            	...
            	if (session.getAttribute("SR1SelectedVehicles") != null){
                	...
                	{            		
              			if(rs.getLong("id")==SelectedVehicleArray[i])
              			{
            	...
            	%>    		
           			<input type="checkbox" name="VehicleCheckBox" id="VehicleID_<%=rs.getString("id") %>" value="<%=rs.getString("id") %>" <%if(IsVehicleSelected){ %>checked<%} %> data-mini="true">
   					<label for="VehicleID_<%=rs.getString("id") %>" ><span style="font-size:9pt; font-weight:normal;"><%=rs.getString("vehicle_no") %></span></label> 
