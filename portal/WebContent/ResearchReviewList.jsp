...
<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="c" data-count-theme="c" style="margin-top: -13px;" id="datathememw">
				...
				//ResultSet rs22 = s.executeQuery("SELECT mr.id, mr.outlet_id, mr.outlet_name, mr.distributor_id, (select name from common_distributors cd where cd.distributor_id=cmw.distributor_id) distributor_name FROM mrd_research mr where created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+WhereDistributors+" order by cmw.distributor_id");
				ResultSet rs22 = s.executeQuery("SELECT mr.id, mr.outlet_id, mr.outlet_name, mr.distributor_id, (select name from common_distributors cd where cd.distributor_id=mr.distributor_id) distributor_name FROM mrd_research mr where created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate));
				...
				while(rs22.next()){
					...
					<li style="height: 40px; " >
						...
						<a data-ajax="false" id="mw_outlet_id_<%=rs22.getString("id")%>" href="#"  style=" " onclick="OpenMarketWatchForm('<%=rs22.getString("id")%>');" style="font-size:10px" >
							<span style="font-size: 12px; font-weight: bold">
							<%=rs22.getString("outlet_id")%> - <%=rs22.getString("outlet_name")%></span><br/>
							<span style="font-size: 11px; font-weight: normal">
							<%=rs22.getString("distributor_id")%> - <%=rs22.getString("distributor_name")%>
							</span>
						</a>
					</li>
