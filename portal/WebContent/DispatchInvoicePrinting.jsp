...
<body style="font-family: Helvetica, Arial, sans-serif"
	...
	<%
		...
		SQLMain = "SELECT isi.shelf_rent,isi.brand_discount_amount,isi.order_id,isi.id, isi.created_on, isi.outlet_id, om.name as outlet_name,  om.address as outlet_address, om.cache_contact_name as owner, om.cache_contact_number as telephone, isi.uvid, isi.invoice_amount, isi.wh_tax_amount, isi.total_amount, isi.discount, isi.net_amount, isi.distributor_id,  (select name from common_distributors where distributor_id=isi.distributor_id) distributor_name , (SELECT DISPLAY_NAME FROM users where id=isi.created_by) user_name, (SELECT DISPLAY_NAME FROM users where id=isi.booked_by) booked_by_name, fraction_adjustment FROM inventory_sales_invoices isi, common_outlets om where isi.outlet_id=om.id and isi.id in (select sales_id from inventory_sales_dispatch_invoices where id="+DispatchID+") order by isi.id ";
		...
		while (rs.next()) {
			...
			SQLMain1 = "select STN , NTN from common_outlets where id = "+rs.getLong("outlet_id")+" ";
			ResultSet rs10 = s2.executeQuery(SQLMain1);
			if (rs10.first()) {
				STN = rs10.getString("STN");
				NTN = rs10.getString("NTN");
			}
			...
			WHTaxAmount += rs.getDouble("wh_tax_amount");
				...
				<div style="width: 5.5in; border: solid 1px; margin-top: 5px;"
					...
					<table style="width: 100%; margin: 10px; font-size: 12px"
						...
						<tr>
							...
							<td><%= (NTN != null && !NTN.trim().isEmpty()) ? NTN : "" %></td>
						</tr>
						<tr>
							<td><b>STRN</b></td>
						</tr>
						<tr>
							<td><%= (STN != null && !STN.trim().isEmpty()) ? STN : "" %></td>
						</tr>
						...
						<%
							double TotalAmountBase = 0;
								...
								while (rs2.next()) {
									...
									int raw_cases = rs2.getInt("raw_cases");
									//long order_id =  rs2.getInt("order_id");
									...
									/* if (LrbTypeID == 3) { //For Only Gur
										ResultSet rs3 = s3
												.executeQuery("select * from inventory_price_list_products_base where product_id="
														+ ProductID + " and " + Utilities.getSQLDate(CreatedOn)
 ...
 */
									...
									TotalAmountBase += Rate * raw_cases;
									////////////////////////////////////////////////////////////////////////
						...
						<tr>
							...
							<td class="formattedRow"
								style="text-align: right; padding-right: 5px;"><%=Utilities.convertToRawCases(TotalUnits, UnitPerSKU)%></td>
						...
						<tr>
							<td colspan="3"
								style="font-weight: bold; border-bottom: 1px solid #000; padding-top: 5px; padding-bottom: 5px; text-align: left;">Brand
								Name</td>
							<td colspan="2"
						...
						<%
							if (rs.getDouble("brand_discount_amount") != 0) {
									...
									ResultSet rsBrandDiscount = s2.executeQuery(
											"select (select label from inventory_products_lrb_types lrb where lrb.id=mobd.brand_id) as brand, mobd.cartons, mobd.brand_discount_amount from mobile_order_brand_discount mobd  where mobd.id="
													+ orderID);
						...
						<tr>
							<td colspan="7" style="text-align: right; padding-right: 5px">Tax
								:</td>
			  		...
			  		<tr>
			  			...
			  			<td style="text-align:right; padding-right:5px;"><%=Utilities.getDisplayCurrencyFormat(totalDiscount)%></td>
			  		</tr> -->
						<tr>
							<td colspan="7" style="text-align: right; padding-right: 5px">Brand
								Discount Amount :</td>
							<td style="text-align: right; padding-right: 5px;"><%=rs.getDouble("brand_discount_amount")%></td>
						</tr>
						
						<% if (rs.getDouble("shelf_rent") != 0) { %>

						<tr>
							<td colspan="7" style="text-align: right; padding-right: 5px">Shelf
								Rent Amount :</td>
							<td style="text-align: right; padding-right: 5px;"><%=rs.getDouble("shelf_rent")%></td>
						</tr>
						...
						<tr>
							<td colspan="7" style="text-align: right; padding-right: 5px">Total
								Discount:</td>
							<td style="text-align: right; padding-right: 5px;"><%=Utilities.getDisplayCurrencyFormat(totalDiscount)%></td>
						</tr>
						<tr>
