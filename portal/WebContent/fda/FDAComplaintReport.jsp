	...
	<tr>
		<td valign="top" style="width: 30%">
			<ul data-role="listview" data-inset="true"  style="font-size:10pt; font-weight: normal; amargin-top:-10px;" data-icon="false">
				...
				<%
				int i = 0;
				ResultSet rs2 = s.executeQuery("SELECT *, (SELECT count(case_no) FROM fda_complaints where date_of_entry between "+Utilities.getSQLDate(FromDate)+" and "+Utilities.getSQLDate(ToDate)+" and type_id=id and status_id=1 ) count, (SELECT count(case_no) FROM fda_complaints where date_of_entry between "+Utilities.getSQLDate(FromDate)+" and "+Utilities.getSQLDate(ToDate)+" and status_id=1 ) count_all FROM fda_complaints_types order by count desc");
				while(rs2.next()){
					
					if(i == 0){
						%>
						<li><a href="FDAComplaintReport.jsp?FromDate=<%=request.getParameter("FromDate")%>&ToDate=<%=request.getParameter("ToDate")%>" style="font-size: 12px; font-weight: bold" data-ajax="false">All<span class="ui-li-count" style="font-size:13px"><%=rs2.getString("count_all")%></span></a></li>
						<%
					}
					
					%>
					<li><a href="FDAComplaintReport.jsp?Type=<%=rs2.getString("id")%>&FromDate=<%=request.getParameter("FromDate")%>&ToDate=<%=request.getParameter("ToDate")%>" style="font-size: 12px; font-weight: bold" data-ajax="false"><%=rs2.getString("label")%><span class="ui-li-count" style="font-size:13px"><%=rs2.getString("count")%></span></a></li>
					<%
					i++;
				}
				%>
						...
						<td style="width: 100%" valign="top">
							...
							<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
								<thead>
									...
									ResultSet rs = s.executeQuery("SELECT *, (SELECT label FROM fda_complaints_types where id=type_id) complaint_label FROM fda_complaints where date_of_entry between "+Utilities.getSQLDate(FromDate)+" and "+Utilities.getSQLDate(ToDate)+" and status_id=1 "+Where);
									while(rs.next()){
										...
										String ExpectedDate = Utilities.getDisplayDateFormat(Utilities.parseDateYYYYMMDD(rs.getString("expected_date")));
										if(rs.getString("expected_date") == null){
											ExpectedDate = "";
										}
