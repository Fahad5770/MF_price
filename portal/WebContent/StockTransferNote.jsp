...
<head>
        ...
        <script src="js/StockTransferNote.js?111111111112=1112"></script>
...
%>
<div data-role="page" id="StockTransferNote" data-url="StockTransferNote" data-theme="d">

    <jsp:include page="Header3.jsp" >
    	<jsp:param value="Stock Transfer Note" name="title"/>
	...
	<tr align='left'>
		...
		<tr> 
			...
			<td colspan="2">
				<input type="text" placeholder="Remarks" id="DeliveryNoteRemarks2" name="DeliveryNoteRemarks2" data-mini="true" onChange="DeliveryNoteUpdateChangeStatus()">
			</td>
				<td></td> 
		</tr>
	...
	<tr align='left'>
		...
		<tr>
			...
			<td colspan="2">
				<input type="text" placeholder="Distributor ID" id="DeliveryNoteDistributorID3" onChange="getDistributorName2()" name="DeliveryNoteDistributorID3"  data-mini="true" >
			</td>
			<td colspan="2">
				<input type="text"  placeholder="Distributor Name" id="DistributorName3" name="DistributorName3" readonly="readonly" data-mini="true" >
			</td>  
			<td style="padding-left:10px">
				<div id="DeliveryNoteDistributorName3" ></div>
			</td>
    ...
    <input type="hidden"  id="DeliveryNoteDistributorID" name="DeliveryNoteDistributorID"  data-mini="true" >
        <input type="hidden"  id="DeliveryNoteDistributorID4" name="DeliveryNoteDistributorID4"  data-mini="true" >
    ...
    <tr>
		...
		//select raw_case from inventory_price_list_products where product_id=
		ResultSet rs1 = s2.executeQuery("select raw_case,unit from inventory_price_list_products where product_id="+rs.getString("product_id"));
		while(rs1.next()){
			Raw_cases_price = rs1.getDouble("raw_case");
			Unit_Price = rs1.getDouble("unit");
		...
		<tr>
			<td>
				...
				<input type='hidden' name='DeliveryNoteMainFormLiquidInML' value='<%=rs.getDouble("liquid_in_ml")%>'>
...
<%-- 			<td><input  type="text" value="0" name="DeliveryNoteMainFormRawCases" id="rawcases<%=i%>" /></td>
 --%>				<td><input type="text" value="0" name="DeliveryNoteMainFormUnits" id="bottles<%=i%>" /></td>	
							<td><input type="text" name="Raw_cases_price" id="Raw_cases_price"  value="<%=Unit_Price%>" readonly/></td>
		...
		<table style="width: 100%;">
			<tr>
				<td>
					...
					<a data-icon="check" data-theme="a" data-role="button" data-inline="true" id="DeliveryNoteSave" href="#"  
					<%if (true){%>
					 onClick="DeliveryNoteSubmit1();" 
					 <%}   %>>
					 Save</a>
					
					<a data-icon="check" data-theme="b" data-role="button" data-inline="true" id="DeliveryNotePrint" href="StockTransferNotePrint.jsp?DeliveryID=<% if( isEditCase ){ out.print(EditID); }else if( isPrintCase ){ out.print(DeliveryIDPrint); } %>" target="_blank" <% if( isEditCase == false && isPrintCase == false ){ %> class="ui-disabled" <% } %>  >Print</a>
                    
                   
                    <button data-icon="check" data-theme="b" data-inline="true" id="DeliveryNoteReset" onClick="javascript:window.location='StockTransferNote.jsp'" >Reset</button>
...
{ 
	var DistributorID = document.getElementById("DeliveryNoteDistributorID3").value;
	$.ajax({
	    url: "common/GetDistributorInfoJson",
	    data: {
	    	DistributorID:DistributorID,
	    ...
	    success: function( json ) {
	    	if (json.exists == "true"){
	    		$("#DistributorName3").val(json.DistributorName);
	    		
	    	}else{
	    		alert("Invalid Distributor ID");
	    		$("#DistributorName3").val('');
	    	}
	    },
	    error: function( xhr, status ) {
	    	alert("Server could not be reached.");
