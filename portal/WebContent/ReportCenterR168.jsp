...
String WarehouseIDs1="";
         ...
         //Sampling Credit Slip Types
           ...
           String SamplingCreditSlipTypesIDs="";
           ...
           if (session.getAttribute(UniqueSessionID+"_SR1SelectedSamplingCreditSlipTypes") != null){
           	...
           	SamplingCreditSlipTypesIDs = Utilities.serializeForSQL(SelectedSamplingCreditSlipTypesArray);
           }

           String WhereSamplingCreditSlipTypes = "";
           if (SamplingCreditSlipTypesIDs.length() > 0){
           	WhereSamplingCreditSlipTypes = " and scsd.type_id in ("+SamplingCreditSlipTypesIDs+") ";	
           }
		...
		<td style="width: 100%" valign="top">
			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					    ...
					    <tr style="font-size:11px;">
							...
							<th style="width: 10%;">Printed On</th>
						...
						//;
						//System.out.println("SELECT scsd.month,scsd.year,scsd.distributor_id,(select name from common_distributors cd where scsd.distributor_id=cd.distributor_id ) distributor_name,scsd.slip_description,scsd.internal_description ,scsd.type_id,(select label from sampling_credit_slip_types scst where scst.id=scsd.type_id) type_name,scsd.is_received,scsd.received_on,scsd.is_printed,scsd.printed_on,(select sum(amount) from sampling_credit_slip_outlets scso where scso.id=scsd.id) amount FROM sampling_credit_slip_distributor scsd where scsd.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" order by scsd.year,scsd.month desc");
						ResultSet rs = s.executeQuery("SELECT scsd.month,scsd.year,scsd.distributor_id,(select name from common_distributors cd where scsd.distributor_id=cd.distributor_id ) distributor_name,scsd.slip_description,scsd.internal_description ,scsd.type_id,(select label from sampling_credit_slip_types scst where scst.id=scsd.type_id) type_name,scsd.is_received,scsd.received_on,scsd.is_printed,scsd.printed_on,(select sum(amount) from sampling_credit_slip_distributor_outlets scso where scso.id=scsd.id) amount,scsd.created_on,scsd.id FROM sampling_credit_slip_distributor scsd where scsd.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+WhereSamplingCreditSlipTypes+" order by scsd.year,scsd.month desc");
						...
						<tr>
							...
							<td ><%=Utilities.getMonthNameByNumber(rs.getInt("month")) %> <%=rs.getLong("year") %></td>
							<td ><%=rs.getLong("distributor_id") %> - <%=rs.getString("distributor_name") %></td>
							<td ><%=rs.getString("slip_description") %></td>
							<td ><%=rs.getString("internal_description") %></td>
							<td ><%=rs.getString("type_name") %></td>
							<td ><%if(rs.getInt("is_received")==1) {%>Yes<%}else{ %>No<%} %></td>
							<td ><%if(rs.getDate("received_on")!=null){%><%=Utilities.getDisplayDateFormat(rs.getDate("received_on")) %><%} %></td>
							<td ><%if(rs.getInt("is_printed")==1) {%>Yes<%}else{ %>No<%} %></td>
							<td ><%if(rs.getDate("printed_on")!=null){%><%=Utilities.getDisplayDateFormat(rs.getDate("printed_on")) %><%} %></td>
						...
						<td></td>
						<td><b>Total</b></td>
						<td style="text-align: right;"><%=Utilities.getDisplayCurrencyFormat(TotalAmount) %></td>
