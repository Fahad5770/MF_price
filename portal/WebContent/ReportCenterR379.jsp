...
<script type="text/javascript">
	...
	if(PjpID == "x125"){
		var url = 'ReportCenterR379NewOutletSummary.jsp?PJPID=x125&UniqueSessionID=<%=UniqueSessionID%>';
	}	
	
	var url = 'ReportCenterR379NewOutletSummary.jsp?CensusID='+PjpID+'&UniqueSessionID=<%=UniqueSessionID%>';
	...
	<tr>
		...
		<td style="width: 30%" valign="top">
			...
			<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="c" data-count-theme="c" style="margin-top:-13px;">
				...
				System.out.println("SELECT * FROM rental_discount where created_on BETWEEN "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and  approved_1=1 and approved_2=0 and is_declined=0 order by id desc");
				ResultSet rs22 = s.executeQuery("SELECT * FROM rental_discount where created_on BETWEEN "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and  approved_1=1 and approved_2=0 and is_declined=0 order by id desc");
				int counter = 0;
				int TotalOutlets =0;
				String OutletName="";
				while(rs22.next()){
