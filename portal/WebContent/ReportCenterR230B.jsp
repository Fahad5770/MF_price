...
<br/>
			...
			<table border="0" id="mytable" style="font-size:13px; font-weight: 400; width:700px" cellpadding="0" cellspacing="0"  >
			
			<tr style="font-size:11px;">
				<th colspan="6" style="background-color:#A2A2A3; color:white;"></th>
				<th colspan="9" style="text-align:center;background-color:#5F9599; color:white; border-left:2px solid #C1DAD7;">ORDER KPI</th>
				
				<th colspan="15" style="text-align:center;background-color:#805F99; color:white; border-left:2px solid #C1DAD7;">DELIVERY KPI</th>
			</tr>
			
			<tr style="font-size:13px;">
				
				<th style="width:110px; background-color:#E3E3E3;">Distributor</th>
				
				<!-- <th style="width:400px;">Order Booker</th> -->
				<th style="width:20px;background-color:#E3E3E3;">Total Outlets</th>
				<th style="width:20px;background-color:#E3E3E3;"># of Avg. Daily Sched. Calls</th>
				<th style="width:20px;background-color:#E3E3E3;"># of Scheduled Calls</th>
				<th style="width:20px;background-color:#E3E3E3;"># of Actual Sched. Calls</th>
				<th style="width:20px;background-color:#E3E3E3;">Call Completion Rate %</th>
				
				
				<th style="width:20px; border-left:2px solid #C1DAD7;"># of Unplan. Calls/ Orders</th>
				...
				<th style="width:20px; border-left:2px solid #C1DAD7; background-color:#F0E6F7;">Actual Sale (r/c)</th>
				<th style="width:20px; background-color:#F0E6F7;">Avg. Daily Target (r/c)</th>
				
				
				
				<th style="width:20px;background-color:#F0E6F7;">Avg. Daily Actual Sale (r/c)</th>
				<th style="width:20px;background-color:#F0E6F7;">BOM Avg. Daily Sale Req. (r/c)</th>
				<th style="width:20px;background-color:#F0E6F7;">Prior Day Ordered Qty (r/c)</th>
				<th style="width:20px;background-color:#F0E6F7;">Delivery Vs. Order %</th>
				<th style="width:20px;background-color:#F0E6F7;">Load Out (r/c)</th>
				<th style="width:20px;background-color:#F0E6F7;">Full In (r/c)</th>
				<th style="width:20px;background-color:#F0E6F7;">Full In %</th>
				<th style="width:20px;background-color:#F0E6F7;">Drop Size</th>
				<th style="width:20px;background-color:#F0E6F7;">Total # of SKU</th>
				<th style="width:20px;background-color:#F0E6F7;">Avg. SKU / Invoice</th>
				<th style="width:20px;background-color:#F0E6F7;"># of Zero Sale Outlets</th>
				<th style="width:20px;background-color:#F0E6F7;">Whole-sale % of total</th>
			...
			ResultSet rs1211 = s3.executeQuery("select count(distinct product_id) from mobile_order mo join mobile_order_products mop on mo.id = mop.id where mop.is_promotion = 0 and mo.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+"");
			if(rs1211.first()){
				GrandTotalUniqueSKUs = rs1211.getDouble(1);
			}
			
			ResultSet rs2 = s.executeQuery("select distinct mo.distributor_id, (select name from common_distributors cd where cd.distributor_id=mo.distributor_id) distributor_name, (select count(distinct outlet_id) from distributor_beat_plan dbp join distributor_beat_plan_schedule dbps on dbp.id = dbps.id where dbp.distributor_id = mo.distributor_id) outlet_count, (select group_concat(distinct outlet_id) from distributor_beat_plan dbp join distributor_beat_plan_schedule dbps on dbp.id = dbps.id where dbp.distributor_id = mo.distributor_id) total_outlet_ids from mobile_order mo where mo.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" ");
			while(rs2.next()){
				...
				*/
				...
				ResultSet rs121 = s3.executeQuery("select count(distinct product_id) from mobile_order mo join mobile_order_products mop on mo.id = mop.id where mop.is_promotion = 0 and mo.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and distributor_id = "+rDistributorID);
				if(rs121.first()){
				...
				while(true){
					...
					////////	System.out.println("Current Date : "+CurrentDate);
						...
						ResultSet rs5 = s3.executeQuery("select count(distinct outlet_id), group_concat(distinct outlet_id) from distributor_beat_plan_log where log_date between "+Utilities.getSQLDate(CurrentDate)+" and "+Utilities.getSQLDate(CurrentDate)+" and distributor_id = "+rDistributorID+" and day_number = dayofweek(log_date) and assigned_to is not null");
						if (rs5.first()){
				...
				if (rDistributorID == 100953){
					//System.out.println("select count(distinct outlet_id) from distributor_beat_plan_log where log_date between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDate(EndDate)+" and distributor_id = "+rDistributorID+" and day_number in ("+DaysOfWeek+") and outlet_id not in (select distinct outlet_id from mobile_order where distributor_id = "+rDistributorID+" and created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+")");
				}
				
				double ZeroSalesCount = 0;
				ResultSet rs50 = s2.executeQuery("select count(distinct outlet_id) from distributor_beat_plan_log where log_date between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDate(EndDate)+" and distributor_id = "+rDistributorID+" and day_number in ("+DaysOfWeek+") and assigned_to is not null and outlet_id not in (select distinct outlet_id from mobile_order where distributor_id = "+rDistributorID+" and created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+")");
				if (rs50.first()){
					ZeroSalesCount = rs50.getDouble(1);
				}
				GrandZeroSalesCount +=ZeroSalesCount;
				...
				ResultSet D_rs51 = s3.executeQuery("select count(distinct isap.product_id) from inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id = isap.id where isap.is_promotion = 0 and isa.order_created_on_date between "+Utilities.getSQLDate(StartDate1)+" and "+Utilities.getSQLDate(EndDate1)+" and isa.distributor_id = "+rDistributorID);
				if(D_rs51.first()){
					D_UniqueSKUs = D_rs51.getDouble(1);
				}
				
				ResultSet D_rs511 = s3.executeQuery("select count(distinct isap.product_id) from inventory_sales_adjusted isa join inventory_sales_adjusted_products isap on isa.id = isap.id where isap.is_promotion = 0 and isa.order_created_on_date between "+Utilities.getSQLDate(StartDate1)+" and "+Utilities.getSQLDate(EndDate1));
				if(D_rs511.first()){
				...
				while(true){
					if(Utilities.getDayOfWeekByDate(CurrentDate1)!=6){ //if it is not friday
						...
						ResultSet rs5 = s3.executeQuery("select count(distinct outlet_id), group_concat(distinct outlet_id) from distributor_beat_plan_log where log_date between "+Utilities.getSQLDate(CurrentDate1)+" and "+Utilities.getSQLDate(CurrentDate1)+" and distributor_id = "+rDistributorID+" and day_number = dayofweek(log_date) and assigned_to is not null");
						if (rs5.first()){
							PlannedOutletsCount = rs5.getDouble(1);
							PlannedOutletIDs = rs5.getString(2);
						}
						...
						*/
						...
						ResultSet D_rs4 = s3.executeQuery("select count(id) from inventory_sales_adjusted where order_id in ("+OrderIDs+")  and net_amount != 0");
						if(D_rs4.first()){
			...
			<tr style="font-size:14px;">
				...
				<td><%=rDistributorID %> - <%=Utilities.truncateStringToMax(rDistributorName, 22)  %></td>
				...
				<td style="text-align: center;background-color:<%=AltRowColorGray%>;"><%=Utilities.getDisplayCurrencyFormatRounded(AvgDailySchCall) %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatRounded(ScheduledCalls) %></td>
				<td style="text-align: center;background-color:<%=AltRowColorGray%>;"><%=Utilities.getDisplayCurrencyFormatRounded(TotalOrdersFromPlannedOutlets) %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(OrderProductivity) %>%</td>
				
				
				<td style="text-align: center; border-left:2px solid #C1DAD7; background-color:<%=AltRowColorBlue%>;"><%=Utilities.getDisplayCurrencyFormatRounded(TotalUnplannedCalls) %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(UnplannedCallsPercentage) %>%</td>
				<td style="text-align: center;background-color:<%=AltRowColorBlue%>;"><%=Utilities.getDisplayCurrencyFormatRounded(TotalOrdersFromPlannedOutlets) %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatRounded(TotalOrders) %></td>
				<td style="text-align: center;background-color:<%=AltRowColorBlue%>;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(OrderProductivity) %>%</td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatRounded(TotalTotalQuantitySold) %></td>
				<td style="text-align: center;background-color:<%=AltRowColorBlue%>;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(DropSize) %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatRounded(TotalUniqueSKUs) %></td>
				<td style="text-align: center;background-color:<%=AltRowColorBlue%>;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(SKUPerOrder) %></td>
				...
				<td style="background-color:<%=AltRowColorPink%>;"></td>
				...
				<td style="text-align: center;background-color:<%=AltRowColorPink%>;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(DeliveryOrderPercentage) %>%</td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatRounded(D_TotalQuantityDispatched) %></td>
				
				<td style="text-align: center;background-color:<%=AltRowColorPink%>;"><% if (D_Returns > 0){out.print(Utilities.getDisplayCurrencyFormatRounded(D_Returns));} %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(D_ReturnsPercentage) %>%</td>
				<td style="text-align: center;background-color:<%=AltRowColorPink%>;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(D_DropSize) %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(D_UniqueSKUs) %></td>
				<td style="text-align: center;background-color:<%=AltRowColorPink%>;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(SKUPerInvoice) %></td>
				...
				<td style="text-align: center;background-color:<%=AltRowColorPink%>; "><%if (TotalWholeSalePercent != 0){out.print(Utilities.getDisplayCurrencyFormatRounded(TotalWholeSalePercent)+"%");} %></td>
			...
			<tr style="font-size:14px;">
				...
				<td style="text-align: center;background-color:<%=AltRowColorGray%>;"><%=Utilities.getDisplayCurrencyFormatRounded(GrandAvgDailySchCallTotal) %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatRounded(GrandTotalScheduledCalls) %></td>
				<td style="text-align: center;background-color:<%=AltRowColorGray%>;"><%=Utilities.getDisplayCurrencyFormatRounded(GrandTotalOrdersFromPlannedOutlets) %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(GrandOrderProductivity) %>%</td>
				
				
				<td style="text-align: center; border-left:2px solid #C1DAD7;background-color:<%=AltRowColorBlue%>;"><%=Utilities.getDisplayCurrencyFormatRounded(GrandTotalUnplannedCalls) %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(GrandUnplannedCallsPercentage) %>%</td>
				<td style="text-align: center;background-color:<%=AltRowColorBlue%>;"><%=Utilities.getDisplayCurrencyFormatRounded(GrandTotalOrdersFromPlannedOutlets) %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatRounded(GrandTotalOrders) %></td>
				<td style="text-align: center;background-color:<%=AltRowColorBlue%>;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(GrandOrderProductivity) %>%</td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatRounded(GrandTotalTotalQuantitySold) %></td>
				<td style="text-align: center;background-color:<%=AltRowColorBlue%>;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(TotalDropSize) %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatRounded(GrandTotalUniqueSKUs) %></td>
				<td style="text-align: center;background-color:<%=AltRowColorBlue%>;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(TotalSKUPerOrder) %></td>
				...
				<td style="text-align: center;background-color:<%=AltRowColorPink%>;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(GrandD_DeliveryOrderPercentage) %>%</td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatRounded(GrandD_TotalQuantityDispatched) %></td>
				
				<td style="text-align: center;background-color:<%=AltRowColorPink%>;"><% if (GrandD_Returns > 0){out.print(Utilities.getDisplayCurrencyFormatRounded(GrandD_Returns));} %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(GrandD_ReturnsPercentage) %>%</td>
				<td style="text-align: center;background-color:<%=AltRowColorPink%>;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(D_TotalDropSize) %></td>
				<td style="text-align: center;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(Grand_UniqueSKUs) %></td>
				<td style="text-align: center;background-color:<%=AltRowColorPink%>;"><%=Utilities.getDisplayCurrencyFormatOneDecimal(TotalSKUPerInvoice) %></td>
				...
				<td style="text-align: center;background-color:<%=AltRowColorPink%>;"><%=Utilities.getDisplayCurrencyFormatRounded(GrandWholeSalePercent) %>%</td>
