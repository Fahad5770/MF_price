			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					...
					<tbody>
					    ...
					    <tr style="font-size:11px;">							
							...
							<th style="width: 13%;">Credit Limit Utilized</th>
							<th style="width: 13%;">Available Credit Limit</th>
							<th style="width: 13%;">One Time Credit</th>
							<th style="width: 13%;">Total Credit</th>
						...
						ResultSet rs = s.executeQuery("SELECT cd.distributor_id, cd.name, (select ifnull(sum(debit),0) - ifnull(sum(credit),0) from gl_transactions_accounts where account_id in (select id from gl_accounts where customer_id = cd.distributor_id and type_id = 3)) one_time_credit_balance, (select ifnull(sum(debit),0) - ifnull(sum(credit),0) from gl_transactions_accounts where account_id in (select id from gl_accounts where customer_id = cd.distributor_id and type_id = 1)) ledger_balance, ifnull((SELECT credit_limit FROM gl_customer_credit_limit where customer_id = cd.distributor_id and is_active = 1 and curdate() between valid_from and valid_to),0) credit_limit FROM pep.common_distributors cd where 1=1 "+WhereASM+WhereHOD+WhereRSM+WhereSM+WhereTDM+" having one_time_credit_balance != 0 or ledger_balance > 0 or credit_limit != 0");
						while(rs.next()){
						...
						<tr>
							...
							<td style="text-align:right"><%if(CreditLimitUtilized!=0){%><%=Utilities.getDisplayCurrencyFormatTwoDecimalFixedAccounting(CreditLimitUtilized) %><%} %></td>
						...
						<tr>
						<th>Total</th>
						<td style="text-align:right"><%=Utilities.getDisplayCurrencyFormatTwoDecimalFixedAccounting(TotalCreditLimit) %></td>
						<td style="text-align:right"><%=Utilities.getDisplayCurrencyFormatTwoDecimalFixedAccounting(TotalLadgerBalance) %></td>
						<td style="text-align:right"><%=Utilities.getDisplayCurrencyFormatTwoDecimalFixedAccounting(TotalCreditLimitUtilized) %></td>
