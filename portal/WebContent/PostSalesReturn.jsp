...
if(Utilities.isAuthorized(364, SessionUserID) == false){
	response.sendRedirect("AccessDenied.jsp");
}

System.out.println("here");
Distributor UserDistributor[] = UserAccess.getUserFeatureDistributor(SessionUserID, 364);
...
<head>
        ...
        <script src="js/PostSalesReturn.js?22111=221"></script>
		...
		function updateReturn(){
			selectElement = 
                document.querySelector('#returnIds');
          output = selectElement.value;
		//	alert(output);
			  document.querySelector('#returnId').textContent = output;
			  $('#returnId').val(output);
			//document.getElementById("returnIds")
			//$('#returnId').val($('select[name=returnIds]').value);
		}
...
<div data-role="page" id="DeskSale" data-url="DeskSale" data-theme="d">
    ...
    <jsp:include page="Header3.jsp" >
    	<jsp:param value="Damage / Expired Stock" name="title"/>
	...
	<table border="0" style="width: 100%">
		...
		<tr>
			...
			<td style="width: 20%;">
					<select    data-mini="true" id="returnIds" name="returnIds"  onChange="updateReturn()" >
						 <option value="1">Sales Return</option>
				       <option value="2">Damage Return</option>
				       <option value="3">Expired Return</option>
			  </select> 
			</td>
	...
	<form action="test2.jsp" name="DeskSaleMainForm" id="DeskSaleMainForm">
    	...
    	<input type="hidden" name="returnId" id="returnId" value="1" >
			...
			<tr>
				<td>
                   ...
                   <!--   <a data-icon="check" data-theme="b" data-role="button" data-inline="true" id="DeskSalePrint" href="DeskSalePrint.jsp?DeliveryID=<% if( isEditCase ){ out.print(EditID); }else if( isPrintCase ){ out.print(DeskSaleIDPrint); } %>" target="_blank" <% if( isEditCase == false && isPrintCase == false ){ %> class="ui-disabled" <% } %>  >Print</a> -->
                    ...
                    <button data-icon="check" data-theme="b" data-inline="true" id="DeskSaleReset" onClick="javascript:window.location='PostSalesReturn.jsp'" >Reset</button>
                    <% if( isEditCase ) { %>
