...
String WhereHOD = "";
if (HODIDs.length() > 0){
	WhereHOD = " and isd.distributor_id in (SELECT distributor_id FROM common_distributors where snd_id in ("+HODIDs+")) ";	
}
...
String WhereRSM = "";
if (RSMIDs.length() > 0){
	WhereRSM = " and isd.distributor_id in (SELECT distributor_id FROM common_distributors where rsm_id in ("+RSMIDs+")) ";	
}
...
String WhereSM = "";
if (SMIDs.length() > 0){
	WhereSM = " and isd.distributor_id in (SELECT distributor_id FROM distributor_beat_plan where sm_id in ("+SMIDs+")) ";	
}
...
String WhereTDM = "";
if (TDMIDs.length() > 0){
	WhereTDM = " and isd.distributor_id in (SELECT distributor_id FROM distributor_beat_plan where tdm_id in ("+TDMIDs+")) ";	
}
...
String WhereASM = "";
if (ASMIDs.length() > 0){
	WhereASM = " and isd.distributor_id in (SELECT distributor_id FROM distributor_beat_plan where asm_id in ("+ASMIDs+")) ";	
}
...
ResultSet rs10 = s2.executeQuery("SELECT sales_id FROM inventory_sales_dispatch isd join inventory_sales_dispatch_invoices_view isdi on isd.id = isdi.id join inventory_sales_adjusted isa on isa.id = isdi.sales_id where isd.is_adjusted = 1 and isd.adjusted_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" "+WhereDistributors+" "+WhereOrderBooker+WhereHOD+WhereRSM+WhereSM+WhereTDM+WhereASM);
while(rs10.next()){
	//if (!rs10.isFirst()){
		InvoiceIDsQuery += ",";
	//}
...
//System.out.println("SELECT sales_id FROM inventory_sales_dispatch isd join inventory_sales_dispatch_invoices_view isdi on isd.id = isdi.id join inventory_sales_adjusted isa on isa.id = isdi.sales_id where isd.is_adjusted = 1 and isd.adjusted_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" "+WhereDistributors+" "+WhereOrderBooker+WhereHOD+WhereRSM+WhereSM+WhereTDM+WhereASM);
