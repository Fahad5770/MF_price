...
function ExportToExcel(D1,D2,DistributorIDs, OrderBookersIDs, TSOIDs, ASMIDs, RegionIDs, ChannelIDs, BrandIDs, SKUIDs){
	//alert(D1);
	$.mobile.showPageLoadingMsg();
	$.ajax({
		
		url: "reports/R360ExcelMain",
		data: {
			StartDate: D1,
			EndDate: D2,
							...
							<tr>
								<td style="text-align: center;width:75%;">
   									...
   									<div id="aExcelFileReady">
   										<a onclick="ExportToExcel(<%=Utilities.getSQLDate(StartDate) %>, <%=Utilities.getSQLDate(EndDate) %>, '<%=DistributorIDs %>', '<%=OrderBookersIDs %>', '<%=TSOIDs %>', '<%=ASMIDs %>', '<%=RegionIDs %>', '<%=ChannelIDs %>', '<%=BrandIDs %>', '<%=SKUIDs %>')" href="#"    data-theme="c" data-inline="true" style="margin-top: 0px; margin-left: 0px; margin-bottom: 0px">Export To Excel</a>
   								</div>
