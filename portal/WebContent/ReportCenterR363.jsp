...
function ExportToExcel(D1,D2,DistributorIDs,BrandIDs,OrderBookersIDs,SKUIDs,ASMIDs,TSOIDs,ChannelIDs,RegionIDs,CityIDs){
	//alert(D1);
	$.mobile.showPageLoadingMsg();
	$.ajax({
		
		url: "reports/R363ExcelMain",
		data: {
			StartDate: D1,
			EndDate: D2,
...
<ul data-role="listview" data-inset="true"
	...
	data-icon="false">
	<li data-role="list-divider" data-theme="a">Orders Punching Status
		Report</li>
	<li>
		<%
		//if(IsUserKpo){
							...
							<tr>
								<td style="text-align: center; width: 75%;">
									...
									<div id="aExcelFileReady">
										<a
											onclick="ExportToExcel(<%=Utilities.getSQLDate(StartDate)%>, <%=Utilities.getSQLDate(EndDate)%>, '<%=DistributorIDs%>',
   										'<%=BrandIDs%>','<%=OrderBookersIDs%>','<%=SKUIDs%>','<%=ASMIDs%>','<%=TSOIDs%>','<%=ChannelIDs%>','<%=RegionIDs%>',
   										'<%=CityIDs%>' 
   										)"
