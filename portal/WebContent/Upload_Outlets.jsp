  ...
  $("#userid").val(<%=SessionUserID%>);
   ...
   $("#popup_workflow_attach" ).popup( "open", { transition: "slidedown",positionTo: "window" } );
     ...
     var manualuploader = $('#manual-fine-uploader').fineUploader({
         ...
         request: {
          endpoint: 'myscripts/Upload_Outlets',
          paramsInBody: true,
             params: {             
...
.on('complete', function(event, id, name, responseJSON){
      if(responseJSON.Success=='true'){
       ...
       window.location="Upload_Outlets.jsp";
      }else{
		...
		<ul data-role="listview" data-inset="false" data-divider-theme="c">
			...
			<li>
			        ...
			        <table style="width:70%" border="0">
						<tr>
						<td>Distributor</td><td>City</td></tr>
						<tr> 
						<td style="width: 20%;">
				<select id="Distributors" name="Distributors" data-mini="true" >
					<%
											while (rs7.next()) {
										%>
										<option value="<%=rs7.getLong("distributor_id")%>"><%=rs7.getString("name")%></option>
										<%
											}
										%>
				</select>
			...
			<td style="width: 30%;" >
				<select id="City" name="City" data-mini="true" >
										<%
											while (rs8.next()) {
										%>
										<option value="<%=rs8.getLong("id")%>"><%=rs8.getString("label")%></option>
										<%
											}
										%>
								</select>
