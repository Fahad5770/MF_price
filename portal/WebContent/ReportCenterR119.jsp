		...
		<td style="width: 100%" valign="top">
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					   ...
					   <tr style="font-size:11px;">
					    ...
					    <tr style="font-size:11px;">
							...
							while(rs12.next()){
								...
								ResultSet rs2 = s2.executeQuery("SELECT distinct package_id, package_label FROM inventory_products_view where  product_id in ("+ProductsLifted+") "+WherePackage+" and category_id = 1 and lrb_type_id="+rs12.getLong("lrb_type_id")+" order by package_sort_order");
					...
					<tbody>
						...
						ResultSet rs1 = s.executeQuery("select * from common_distributors where 1=1 "+WhereHOD+WhereRSM+WhereSM+WhereTDM+WhereASM+" and distributor_id in ("+DistributorIDs+") and distributor_id in (select distinct distributor_id from inventory_delivery_note where created_on between "+Utilities.getSQLDateLifting(StartDate)+" and "+Utilities.getSQLDateNextLifting(EndDate)+")"); //distributor query
						while(rs1.next()){
						...
						%>
						<tr>
								<td><%=Utilities.truncateStringToMax((DistributorID + " - "+ rs1.getString("name")),30) %></td>
							<%
							//Converted
							long ConvertedCases=0;
							
							
							ResultSet rs311 = s4.executeQuery("select sum(total_units*ipv.liquid_in_ml)/6000 cqty,ipv.unit_per_sku from inventory_delivery_note idn join inventory_delivery_note_products idnp on idn.delivery_id = idnp.delivery_id join inventory_products_view ipv on idnp.product_id = ipv.product_id where idn.created_on between "+Utilities.getSQLDateLifting(StartDate)+" and "+Utilities.getSQLDateNextLifting(EndDate)+" and idn.distributor_id = "+DistributorID+" and ipv.category_id = 1 "+WherePackage+" "+WhereBrand+WhereLiftingType);	
								while(rs311.next()){
									ConvertedCases = rs311.getLong("cqty");
								}
							...
							while(rs31.next()){								
								ResultSet rs4 = s2.executeQuery("SELECT distinct package_id, package_label FROM inventory_products_view where  product_id in ("+ProductsLifted+") "+WherePackage+" and category_id = 1 and lrb_type_id="+rs31.getLong("lrb_type_id")+" order by package_sort_order");
								while(rs4.next()){
							...
							int unit_per_sku=0;
									...
									ResultSet rs3 = s4.executeQuery("select sum(total_units) qty,ipv.unit_per_sku from inventory_delivery_note idn join inventory_delivery_note_products idnp on idn.delivery_id = idnp.delivery_id join inventory_products_view ipv on idnp.product_id = ipv.product_id where idn.created_on between "+Utilities.getSQLDateLifting(StartDate)+" and "+Utilities.getSQLDateNextLifting(EndDate)+" and idn.distributor_id = "+DistributorID+" and ipv.category_id = 1 and ipv.package_id ="+rs4.getLong("package_id")+" and ipv.lrb_type_id="+rs31.getLong("lrb_type_id")+WherePackage+" "+WhereBrand+WhereLiftingType);	
								while(rs3.next()){
						...
						<td style="text-align: right;"><%if(TotalConverted!=0){%><%=Utilities.getDisplayCurrencyFormatRounded(TotalConverted) %><%} %></td>
						<%
