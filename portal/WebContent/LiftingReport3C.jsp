...
String DeliveryStatus = Utilities.filterString(request.getParameter("LiftingReportMainDeliveryStatus"), 1, 100);
...
int DistributorGroup = Utilities.parseInt(Utilities.filterString(request.getParameter("LiftingReportMainDistributorGroup"), 1, 100));
...
<form id="PassParams" action="LiftingReport3C.jsp" method="post">
	...
	<input type="hidden" name="LiftingReportMainDeliveryStatus" value="<%=DeliveryStatus%>" >
	...
	<input type="hidden" name="LiftingReportMainDistributorGroup" value="<%=DistributorGroup%>" >
...
<table border=0 style="width:100%">
	<tr>
		<td style="width:30%" valign="top">
			...
			<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="a" data-count-theme="c">
	            ...
	            if (DeliveryStatus.equals("Pending")){
	            	whereWithoutDistributor += " and idn.is_delivered = 0 and idn.is_received = 0 ";
	            }
	            if (DeliveryStatus.equals("Delivered")){
	            	whereWithoutDistributor += " and idn.is_delivered = 1 ";
	            }			            
	            if (DeliveryStatus.equals("Received")){
	            	whereWithoutDistributor += " and idn.is_received = 1 ";
	            }		            
	            ...
	            if (DistributorGroup != 0){
	            	whereWithoutDistributor += " and cd.distributor_id in (select distributor_id from common_distributor_groups_list where id = "+DistributorGroup+") ";
	            }
				...
				while(rs_distributor.next()){
					...
					<li <%if(Distributor == rs_distributor.getLong("idn.distributor_id")){ %> data-theme="b" <%} %>><a data-ajax="false" href="#" onclick="redirect(<%=rs_distributor.getString("idn.distributor_id")%>, 0)" style="font-size:13px"><%=rs_distributor.getString("distributor_name")%> <span class="ui-li-count" style="font-size:13px"><%=Math.round(rs_distributor.getDouble("converted_cases"))%></span></a></li>
					
					<%
				}
				%>
				
				<li><a data-ajax="false" href="#" onclick="redirect(0, 0)" style="font-size:13px">All<span class="ui-li-count" style="font-size:13px"><%=Utilities.getDisplayCurrencyFormat(TotalDistributorConvertedCases)%></span></a></li>
		...
		<%
			            ...
			            if (DeliveryStatus.equals("Pending")){
			            	where += " and idn.is_delivered = 0 and idn.is_received = 0 ";
			            }
			            if (DeliveryStatus.equals("Delivered")){
			            	where += " and idn.is_delivered = 1 ";
			            }			            
			            if (DeliveryStatus.equals("Received")){
			            	where += " and idn.is_received = 1 ";
			            }
			            ...
			            if (DistributorGroup != 0){
			            	where += " and cd.distributor_id in (select distributor_id from common_distributor_groups_list where id = "+DistributorGroup+") ";
			            }
			...
			<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="a" data-count-theme="c">
				...
				while(rs_orders.next()){%>
					<li <% if(SapOrderNumber == rs_orders.getLong("sap_order_no")){%>data-theme="b"<%} %> ><a data-ajax="false" href="#" onclick="redirect(<%=rs_orders.getString("idn.distributor_id")%>, <%=rs_orders.getString("sap_order_no")%>)" style="font-size:13px; font-weight:normal;">Order# <%=rs_orders.getString("sap_order_no") %> | Vehicle# <%=rs_orders.getString("vehicle_no")%><br/><%=Utilities.getDisplayDateTimeFormat(rs_orders.getTimestamp("created_on"))  %>&nbsp;<%
			...
			<ul data-role="listview" data-inset="true" class="ui-icon-alt" data-theme="d" data-divider-theme="a" data-count-theme="b">
				...
				<table border=0 style="font-size:13px; width:100%"  data-role="table" id="SamplingSummary" data-mode="reflow" class="ui-responsive table-stroke">
					  <thead>
			            ...
			            if (DeliveryStatus.equals("Pending")){
			            	where1 += " and idn.is_delivered = 0 and idn.is_received = 0 ";
			            }
			            if (DeliveryStatus.equals("Delivered")){
			            	where1 += " and idn.is_delivered = 1 and idn.is_received = 0 ";
			            }			            
			            if (DeliveryStatus.equals("Received")){
			            	where1 += " and idn.is_received = 1 ";
			            }
			            ...
			            if (DistributorGroup != 0){
			            	where1 += " and cd.distributor_id in (select distributor_id from common_distributor_groups_list where id = "+DistributorGroup+") ";
			            }
			            ...
			            while(rs1.next()){
			            	...
			            	TotalRawCases +=rs1.getDouble("bottles")/rs1.getDouble("ipa.unit_per_case");
			            ...
			            %>
			                ...
			                <tr style="background:#ececec">
			                	<td colspan="1">&nbsp;</td>
			                	<td style="text-align:right; padding-right:10px"><b><%=Utilities.getDisplayCurrencyFormatRounded(TotalRawCases)%></b></td>
