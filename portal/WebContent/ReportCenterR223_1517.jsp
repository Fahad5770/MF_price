					   ...
					   <th data-priority="1"  style="text-align:center; ">&nbsp;</th>
					    ...
					    <tr style="font-size:11px;">
							...
							while(rs12.next()){
								...
								ResultSet rs2 = s2.executeQuery("SELECT distinct package_id, package_label, lrb_type_id FROM inventory_products_view where  package_id in (SELECT distinct package_id FROM inventory_primary_percase_request_products) "+WherePackage+" and category_id = 1 and lrb_type_id="+rs12.getLong("lrb_type_id")+" and package_id != 24  order by package_sort_order");
								while(rs2.next()){
							%>
								<th data-priority="1"  style="text-align:center; " colspan="4"><%if (rs2.getInt(1) == 2 && rs2.getInt(3) == 1){out.print("1500/1750ML");}else{out.print(rs2.getString("package_label"));} %></th>
							<%
							ArrayCount++;
							PackageCount++;
								}
							...
							while(rs121.next()){
								...
								ResultSet rs212 = s2.executeQuery("SELECT distinct package_id, package_label FROM inventory_products_view where  package_id in (SELECT distinct package_id FROM inventory_primary_percase_request_products) "+WherePackage+" and category_id = 1 and lrb_type_id="+rs121.getLong("lrb_type_id")+" and package_id != 24  order by package_sort_order");
								while(rs212.next()){
									%>
					...
					<tbody>
						...
						<tr>
							...
							while(rs122.next()){
								...
								ResultSet rs2 = s3.executeQuery("SELECT distinct package_id, package_label FROM inventory_products_view where  package_id in (SELECT distinct package_id FROM inventory_primary_percase_request_products) "+WherePackage+" and category_id = 1 and lrb_type_id="+rs122.getLong("lrb_type_id")+" and package_id != 24  order by package_sort_order");
								while(rs2.next()){
									...
									String sPackageID = ""+PacakgeID;
									if (PacakgeID == 2){
										sPackageID = ""+PacakgeID+",24";
									}
									...
									ResultSet rs4 = s4.executeQuery("select sum(idnp.total_units) total_units,unit_per_sku  from inventory_delivery_note idn join inventory_delivery_note_products idnp on idn.delivery_id = idnp.delivery_id join inventory_products_view ipv on idnp.product_id = ipv.product_id where ipv.category_id = 1 and distributor_id = "+DistributorID+" and created_on between "+Utilities.getSQLDateLifting(LiftingStartDate)+" and "+Utilities.getSQLDateNextLifting(LiftingEndDate)+" and package_id in ("+sPackageID+") and lrb_type_id="+rs122.getLong("lrb_type_id"));
									if(rs4.first()){
										TotalUnitsLifting = rs4.getLong("total_units");
										UnitPerSKU = rs4.getInt("unit_per_sku");
