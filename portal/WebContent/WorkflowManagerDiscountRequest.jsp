...
<head>
	...
	<script src="js/WorkflowManagerDiscountRequest.js?1242121=1242121"></script>
  ...
  long RequestID = Utilities.parseLong(request.getParameter("requestID"));
   ...
   %>
    ...
    <jsp:include page="Header2.jsp" >
    	<jsp:param value="Product Discount Request" name="title"/>
    </jsp:include>
     <!-- /header -->
 <script>
        ...
        <fieldset class="ui-grid-a">
		        ...
		        <form name="WorkflowContainerForm" class="WorkflowForm" id="WorkflowContainerForm" method="POST" action="#">
					<ul data-role="listview" data-inset="true" style="margin-left: 22px;margin-top: 6px;margin-right: 5px;">
						...
						<li data-role="list-divider">Discount Product</li>
						<li>
						...
						<table border="0" style="width: 100%; margin-top: 0px; padding: 0px;" class="workflow_form_font">
							...
							<%
							ResultSet rs6 = s1.executeQuery("SELECT isprp.id,isprp.package_id,(select label from inventory_packages ip where ip.id=isprp.package_id) package_labele_id,isprp.raw_cases,isprp.units,isprp.total_units,isprp.type_id,isprp.quantity,isprp.discount_rate,isprp.selling_price,isprp.variable_cost,isprp.other_cost,isprp.promotion_cost,isprp.marginal_contribution,isprp.serial_no FROM inventory_sales_discounts_request_products isprp join inventory_sales_discounts_request ispr on ispr.id=isprp.id where ispr.request_id="+RequestID+" and isprp.type_id=1");
							...
							while(rs6.next()){
								...
								Quantity=rs6.getDouble("quantity");
								DiscountRate=rs6.getDouble("discount_rate");
								SellingPrice=rs6.getDouble("selling_price");
								VariableCost=rs6.getDouble("variable_cost");
								OtherCost=rs6.getDouble("other_cost");
								PromotionCost=rs6.getDouble("promotion_cost");
								MarginalContribution= (SellingPrice - VariableCost - PromotionCost);
								
								
								
								//out.print(SellingPrice + " " + VariableCost + " " + PromotionCost);
								
								PackageID = rs6.getInt("package_id");
								ProductTableID = rs6.getInt("id");
								MasterSerialID = rs6.getInt("serial_no");
								//System.out.println("MasterSerialID");
								RowCounter++;
								
								
								ResultSet rs2 = s2.executeQuery("SELECT isprp.id,isprp.package_id,(select label from inventory_packages ip where ip.id=isprp.package_id) package_label,isprpb.brand_id,(select label from inventory_brands ib where ib.id=isprpb.brand_id) brand_label,isprp.raw_cases,isprp.units,isprp.total_units,isprp.type_id,isprpb.serial_no FROM inventory_sales_discounts_request_products isprp join inventory_sales_discounts_request_products_brands isprpb on isprp.id=isprpb.id and isprp.serial_no=isprpb.serial_no and isprp.package_id=isprpb.package_id and isprp.type_id=isprpb.type_id join inventory_sales_discounts_request ispr on ispr.id=isprp.id where ispr.request_id="+RequestID+" and isprp.type_id=1 and isprp.package_id="+rs6.getLong("package_id")+" and isprp.serial_no="+MasterSerialID+" group by isprp.type_id,isprpb.brand_id");
								
								//System.out.println("SELECT isprp.id,isprp.package_id,(select label from inventory_packages ip where ip.id=isprp.package_id) package_label,isprpb.brand_id,(select label from inventory_brands ib where ib.id=isprpb.brand_id) brand_label,isprp.raw_cases,isprp.units,isprp.total_units,isprp.type_id,isprpb.serial_no FROM inventory_sales_discounts_request_products isprp join inventory_sales_discounts_request_products_brands isprpb on isprp.id=isprpb.id and isprp.package_id=isprpb.package_id and isprp.type_id=isprpb.type_id join inventory_sales_discounts_request ispr on ispr.id=isprp.id where ispr.request_id="+RequestID+" and isprp.type_id=1 and isprp.package_id="+rs6.getLong("package_id")+" and isprp.serial_no="+MasterSerialID+" group by isprp.type_id,isprpb.brand_id");
								...
								<tr style="b1ackground-color:#efedee">								
									<td style=" padding:5px;" valign="top"><%=PackageLabel %></td>
									<td style=" padding:5px;"><%=BrandLabel %></td>
								</tr>
								<tr>	
									<th  style="background-color:#efedee; padding:5px;font-size:12px; ">Quantity</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px; ">Distributor Rate</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px; ">Selling Price</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px; ">Variable Cost</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px; ">Other Cost</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px; ">Promotion Cost</th>
									<th  style="background-color:#efedee; padding:5px;font-size:12px; ">Marginal Contribution</th>
									
								</tr>	
								
								<tr>
									<td>
										<input type='hidden' name='ProductPromotionsMainFormPackage' value="<%=PackageID %>" />
										<input type='text' placeholder='Quantity' id='EstimatedSalesVolumeRawCases<%=RowCounter %>' name='Quantity' data-mini='true' value="<%=Quantity%>" <%if(bean.CURRENT_STEP.STEP_ID!=2){ %>readonly<%} %>/>
									</td>
									<td><input type='text' placeholder='Discount Rate' id='SalesSKUPrice<%=RowCounter %>' name='DiscountRate' data-mini='true' onKeyup='CalculateMarginalContribution(<%=RowCounter %>);' value="<%=DiscountRate%>" <%if(bean.CURRENT_STEP.STEP_ID!=2){ %>readonly<%} %>/></td>
									<td><input type='text' placeholder='Selling Price' id='FreeSKUPriceBottles<%=RowCounter %>' name='SellingPrice' data-mini='true' onKeyup='CalculateMarginalContribution(<%=RowCounter %>);' value="<%=SellingPrice%>" <%if(bean.CURRENT_STEP.STEP_ID!=2){ %>readonly<%} %>/></td>
									<td><input type='text' placeholder='Variable Cost + Taxes' id='VariableCost<%=RowCounter %>' name='VariableCost' data-mini='true' onKeyup='CalculateMarginalContribution(<%=RowCounter %>)' value="<%=VariableCost%>" <%if(bean.CURRENT_STEP.STEP_ID!=2){ %>readonly<%} %>/></td>
									<td><input type='text' placeholder='Other Cost' id='OtherCost<%=RowCounter %>' name='OtherCost' data-mini='true' value="<%=OtherCost%>" <%if(bean.CURRENT_STEP.STEP_ID!=2){ %>readonly<%} %>/></td>
									<td><input type='text' placeholder='Promotion Cost' id='PromotionCost<%=RowCounter %>' name='PromotionCost' data-mini='true'  value="<%=PromotionCost%>" <%if(bean.CURRENT_STEP.STEP_ID!=2){ %>readonly<%} %>/></td>
									<td><input type='text' placeholder='Marginal Contribution' id='MarginalContribution<%=RowCounter %>' name='MarginalContribution' data-mini='true' class='ui-disabled' value="<%=MarginalContribution%>" <%if(bean.CURRENT_STEP.STEP_ID!=2){ %>readonly<%} %>/></td>
						...
						<li data-role="list-divider">Discount Reason</li>
						<li>
						<table border="0" style="width: 100%; margin-top: 0px; padding: 0px;" class="workflow_form_font">
							...
							<tbody>
							<%
							ResultSet rs = s.executeQuery("select * from inventory_sales_discounts_request where request_id="+RequestID);
							while(rs.next()){
							%>
							<tr>
						...
						<table>
							...
							<tbody id="UpfrondDisocuntCustomer">
							</tbody>
						</table>
						<table border="0" style="width: 100%; margin-top: 0px; padding: 0px;" class="workflow_form_font">
							<thead>
									<tr><th  style="background-color:#efedee; padding:5px;font-size:12px; ">Customer Name</th></tr>
									
							</thead>
							<tobdy>
						<%
						ResultSet rs41 = s.executeQuery("SELECT isdrd.distributor_id,(select name from common_distributors cd where cd.distributor_id=isdrd.distributor_id) name FROM inventory_sales_discounts_request isdr join inventory_sales_discounts_request_distributors isdrd on isdr.id=isdrd.product_promotion_id where request_id="+RequestID);
						while(rs41.next()){
						%>
						
							
								<tr>
									<td><%=rs41.getString("distributor_id") +" - "+rs41.getString("name") %></td>
								</tr>
					...
					<input type="hidden" name="ProductTableID" id="uvid" value="<%=ProductTableID%>">
