			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					 <thead>
					    <tr style="font-size: 11px;">
							...
							<th data-priority="1" style="text-align:center; width: 10%">Dispatch#</th>
							<th data-priority="1" style="text-align:center; width: 30%">Distributor</th>
							<th data-priority="1" style="text-align:center; width: 10%">Vehicle</th>
							<th data-priority="1" style="text-align:center; width: 20%">Driver</th>
							<th data-priority="1" style="text-align:center; width: 10%">Dispatch Amount</th>
							<th data-priority="1" style="text-align:center; width: 10%">Adjusted Amount</th>
					    </tr>
						<tr style="background:#ececec">
							...
							ResultSet rs = s.executeQuery("SELECT isd.id, isd.adjusted_on, isd.vehicle_id, (select vehicle_no from distribtuor_vehicles where id = isd.vehicle_id) vehicle_label, isd.driver_id, (select name from distributor_employees where id = isd.driver_id) driver_name, sum(isi.net_amount) dispatch_amount, sum(isa.net_amount) adjusted_amount, isd.distributor_id, (select name from common_distributors where distributor_id = isd.distributor_id) distributor_name FROM inventory_sales_dispatch isd join inventory_sales_dispatch_invoices_view isdi on isd.id = isdi.id join inventory_sales_adjusted isa on isdi.sales_id = isa.id join inventory_sales_invoices isi on isa.id = isi.id where isd.distributor_id in ("+DistributorIDs+") and isd.is_adjusted = 1 and isd.adjusted_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" "+WhereVehicle+" "+WhereEmployee+" group by isd.id");
							while(rs.next()){
							...
							<tr style="font-size: 12px;">
		    					...
		    					<td style="text-align:left; border-left: 1px solid #ececec; border-right: 1px solid #ececec"><%=DistributorID + " - "+DistributorName %></td>
						...
						<tr style="background:#ececec">
	   	            		<td align="left" colspan="2">Unadjusted (All)</td>
	   	            		<td style="text-align:right; padding-right:10px; border-left: 1px solid #ececec; border-right: 1px solid #ececec" colspan="5">&nbsp;</td>
	   	            	</tr>	
							<%
							double TotalDispatchAmountUnadjusted =0;
							
							ResultSet rs2 = s.executeQuery("SELECT isd.id, isd.created_on, isd.vehicle_id, (select vehicle_no from distribtuor_vehicles where id = isd.vehicle_id) vehicle_label, isd.driver_id, (select name from distributor_employees where id = isd.driver_id) driver_name, sum(isi.net_amount) dispatch_amount, isd.distributor_id, (select name from common_distributors where distributor_id = isd.distributor_id) distributor_name FROM inventory_sales_dispatch isd join inventory_sales_dispatch_invoices_view isdi on isd.id = isdi.id join inventory_sales_invoices isi on isdi.sales_id = isi.id where isd.distributor_id in ("+DistributorIDs+") and isd.is_adjusted = 0 /* and isd.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" */ "+WhereVehicle+" "+WhereEmployee+" group by isd.id");
							while(rs2.next()){
								long DistributorID = rs2.getLong("distributor_id");
								String DistributorName = rs2.getString("distributor_name");
