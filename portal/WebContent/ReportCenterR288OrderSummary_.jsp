...
if(OrderID > 0){
	IsMapShow = true;
	ResultSet rs3 = s.executeQuery("SELECT concat(co.id, ' - ', co.name) outlet_name, mo.created_on,mo.lat,mo.lng, co.lat olat, co.lng olng  FROM mobile_order_sm mo, common_outlets co where mo.outlet_id=co.id and mo.id="+OrderID);
	if(rs3.first()){
	...
	//For no order
	IsMapShow2 = true;
	ResultSet rs32 = s.executeQuery("SELECT concat(co.id, ' - ', co.name) outlet_name, moz.created_on,moz.lat,moz.lng, co.lat olat, co.lng olng  FROM mobile_order_sm_zero moz, common_outlets co where moz.outlet_id=co.id and moz.id="+OrderID);
	if(rs32.first()){
						...
						<td style="width: 100%" valign="top">
							...
							<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
								...
								<table border=0 style="font-size:13px; margin-top:20px ; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
									...
									<% 
							         ResultSet rs = s.executeQuery("SELECT moz.comments,moz.no_order_reason_type_v2,(select label from mobile_order_no_sm_order_reason_type where mobile_order_no_sm_order_reason_type.id=moz.no_order_reason_type_v2) name from mobile_order_sm_zero moz where moz.id="+OrderID );
							         while(rs.next()){
