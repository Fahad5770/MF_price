...
<script>
           ...
           if(SelectedWarehouseArray != null){
		      ...
		      // System.out.println("SELECT * from gl_accounts where  "+WhereWarehouse);
		            ...
		            boolean IsTransactionAccountSelected=false;
		            long SelectedTransactionAccountArray[] = null;
		            if (session.getAttribute(UniqueVoID+"_SR1SelectedTransactionAccount") != null){
		            	SelectedTransactionAccountArray = (long[])session.getAttribute(UniqueVoID+"_SR1SelectedTransactionAccount");           	
		            }
		            while(rs.next()){
		            	IsEmptyResultSet = false;
		            	if (session.getAttribute(UniqueVoID+"_SR1SelectedTransactionAccount") != null){
		            		for(int i=0;i<SelectedTransactionAccountArray.length;i++)
		                	{            		
		              			if(rs.getLong("id")==SelectedTransactionAccountArray[i])
		              			{
		              				IsTransactionAccountSelected = true;            			
		              			}            		
		                	}
		            	}
		            	...
		            	%>  
		           			...
		           			<input type="radio" name="TransactionAccountRadio" id="TransactionAccount_<%=rs.getString("id") %>" value="<%=rs.getString("id") %>" <%if(IsTransactionAccountSelected){ %>checked<%} %> data-mini="true">
		   					<label for="TransactionAccount_<%=rs.getString("id") %>" ><span style="font-size:9pt; font-weight:normal;"><%=rs.getString("label") %></span></label> 
		            	...
		            	IsTransactionAccountSelected=false;
		            }
		       }else{%>
		       <p></p>
