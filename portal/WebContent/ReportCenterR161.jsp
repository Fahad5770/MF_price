...
String WhereDistID="";
String WhereDistIDSAP="";
if(DistributorID!=0){

	WhereDistID ="  and distributor_id="+DistributorID;
	WhereDistIDSAP = "vbrk.kunag = "+DistributorID+" and ";

}

//System.out.println(WhereDistID);
%>
			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					 <thead>
					    <tr style="font-size:11px;">
							...
							<th data-priority="1"  style="text-align:center; width: 20% ">Invoice Amount</th>							
					...
					sMySQL.executeUpdate("create temporary table temp_invoices_list (transaction_date date, distributor_id bigint(11), distributor_name varchar(255),  invoice_number bigint(11), invoice_amount double )");	
					double TotalInvoiceAmount = 0;
					
					
					String Query = "select vbrk.kurrf_dat /*Transaction Date*/, vbrk.fkdat /*Billing Date*/, vbrk.erdat, vbrk.aedat /*Changed On*/, vbrk.vbeln, vbrk.netwr+vbrk.mwsbk invoice_amount, vbrk.fkart, vbrk.kunag /*Customer*/, kna1.name1, vbuk.gbstk "+
							" from sapsr3.vbrk vbrk join sapsr3.vbuk vbuk on vbrk.vbeln = vbuk.vbeln join sapsr3.kna1 kna1 on vbrk.kunag = kna1.kunnr"+
							" where "+WhereDistIDSAP+" vbrk.kurrf_dat between "+Utilities.getSQLDateOracle(StartDate)+" and "+Utilities.getSQLDateOracle(EndDate)+" and vbrk.fksto != 'X' /* Invoice not Cancelled */ and vbrk.ZZCDS_STATUS != 'X' "+ 
					...
					while(rs.next()){
						sMySQL.executeUpdate("insert into temp_invoices_list values("+Utilities.getSQLDate(Utilities.parseDateYYYYMMDDWithoutSeparator(rs.getString("kurrf_dat")))+", "+rs.getLong("kunag")+",'"+Utilities.filterString(rs.getString("name1"), 1, 200)+"("+rs.getString("gbstk")+")"+"',"+rs.getLong("vbeln")+","+rs.getDouble("invoice_amount")+")");
					}
					
					ResultSet rsMySQL = sMySQL.executeQuery("select * from temp_invoices_list where invoice_number not in (SELECT invoice_no FROM gl_invoice_posting) and invoice_number not in (select invoice_no from inventory_delivery_note)  "+WhereDistID+" order by invoice_amount");
					while(rsMySQL.next()){
						TotalInvoiceAmount += rsMySQL.getDouble("invoice_amount");
						/*
						String OrderNo = "";
						ResultSet rs2 = s2.executeQuery("select vgbel from sapsr3.vbrp where vbeln = "+rsMySQL.getString("invoice_number")+" and rownum = 1");
						if(rs2.first()){
							OrderNo = rs2.getString(1);
						}
						*/
					%>
