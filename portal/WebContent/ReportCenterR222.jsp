...
if(UserAccess.isAuthorized(283, SessionUserID, request) == true){
...
if (DistributorID1 != 0){
	WhereCustomerID = " and pso.distributor_id in ("+DistributorID1+") ";
			...
			<table border=0 style="font-size:13px; font-weight: 400; width:100%" cellpadding="0" cellspacing="0"  adata-role="table" class="GridWithBorder">
					 <thead>
					   <tr style="font-size:11px;">
							...
							ResultSet rs21 = s.executeQuery("SELECT count(distinct package_id) package_count,lrb_type_id,(select label from inventory_products_lrb_types iplt where iplt.id=lrb_type_id) type_name FROM inventory_products_view where  product_id in (SELECT distinct product_id FROM primary_sales_orders_products) and category_id = 1 and lrb_type_id is not null group by lrb_type_id");
							while(rs21.next()){
								%>
					    ...
					    <tr style="font-size:11px;">
							...
							while(rs12.next()){
								...
								ResultSet rs2 = s2.executeQuery("SELECT distinct package_id, package_label FROM inventory_products_view where  product_id in (SELECT distinct product_id FROM primary_sales_orders_products) "+WherePackage+" and category_id = 1 and lrb_type_id="+rs12.getLong("lrb_type_id")+" order by package_sort_order");
								while(rs2.next()){
							%>
					...
					<tbody>
						...
						//System.out.println("select * from common_distributors where 1=1 "+WhereCustomerID+" and distributor_id in ("+DistributorIDs+") and distributor_id in (select  distributor_id from primary_sales_orders where created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+")");
						ResultSet rs1 = s.executeQuery("select  pso.id,pso.distributor_id,cd.name, pso.plant_id,(select cd1.name from common_distributors cd1 where cd1.distributor_id=pso.plant_id) plant_name from primary_sales_orders pso join common_distributors cd on cd.distributor_id=pso.distributor_id where pso.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" "+WhereCustomerID+WherePlantID+" order by pso.plant_id"); //distributor query
						while(rs1.next()){
							DistributorID = rs1.getInt("distributor_id");
							
							int PackageIndex = 0;
							long SrID=rs1.getLong("id");
							PlantID = rs1.getInt("plant_id");
							
							String PlantName ="";
							
							if(PlantID!=0){
								PlantName = PlantID+" - "+rs1.getString("plant_name");
							}
						...
						<tr>
								...
								int UnitPerSKUCon=0;
								ResultSet rs23 = s3.executeQuery("select sum(total_units*ipv.liquid_in_ml)/6000 converted from primary_sales_orders_products psop join inventory_products_view ipv on psop.product_id = ipv.product_id where psop.id="+SrID);
								if(rs23.first()){
								...
								<td  style="text-align: right;"><%=Utilities.getDisplayCurrencyFormatRounded(ConvertedSales)%></td>
							<%
							
							ResultSet rs31 = s3.executeQuery("SELECT count(distinct package_id) package_count,lrb_type_id,(select label from inventory_products_lrb_types iplt where iplt.id=lrb_type_id) type_name FROM inventory_products_view where  product_id in (SELECT distinct product_id FROM primary_sales_orders_products) and category_id = 1 and lrb_type_id is not null group by lrb_type_id");
							while(rs31.next()){								
								ResultSet rs4 = s2.executeQuery("SELECT distinct package_id, package_label FROM inventory_products_view where  product_id in (SELECT distinct product_id FROM primary_sales_orders_products) "+WherePackage+" and category_id = 1 and lrb_type_id="+rs31.getLong("lrb_type_id")+" order by package_sort_order");
								while(rs4.next()){
							...
							long qty=0;
									ResultSet rs3 = s4.executeQuery("select sum(total_units) qty,ipv.unit_per_sku from primary_sales_orders pso join primary_sales_orders_products psop on pso.id = psop.id join inventory_products_view ipv on psop.product_id = ipv.product_id where pso.created_on between "+Utilities.getSQLDate(StartDate)+" and "+Utilities.getSQLDateNext(EndDate)+" and pso.distributor_id = "+DistributorID+" and ipv.category_id = 1 and ipv.package_id ="+rs4.getLong("package_id")+" and ipv.lrb_type_id="+rs31.getLong("lrb_type_id")+WherePackage+" "+WhereBrand+" and psop.id="+SrID);	
								while(rs3.next()){
									 qty = rs3.getLong("qty");
							...
							ResultSet rs231 = s4.executeQuery("SELECT delivery_id FROM inventory_delivery_note where outsourced_primary_sales_id="+SrID);
							if(rs231.first()){
								DeliveryID = rs231.getLong("delivery_id");
							}
							
							
							%>
							
							<td style="text-align:left; border-left: 1px solid #ececec; border-right: 1px solid #ececec"><a data-role="button" data-mini="true" data-theme="c" id="DeliveryNoteButton_<%=row %>" onClick="OpenDeliveryNoteVoucherPrint(<%=DeliveryID%>)">View</a></td>
							<td style="text-align:left; border-left: 1px solid #ececec; border-right: 1px solid #ececec"><%if (isDeleteAllowed) {%><a data-role="button" data-mini="true" data-theme="c" id="DeliveryNotDeleteeButton_<%=row %>" onClick="DeleteOutsourceEntry(<%=SrID%>,<%=row%>)">Delete</a><%} %></td>
						</tr>
						<%
						row++;
						}
